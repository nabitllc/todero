...
type ServiceCheckDependencies = {
  ...
  probe: (config: ToderoConfig) => Promise<HealthResult>;
  shimPresent: (executablePath: string) => Promise<boolean>;
};

async function probeHealth(config: ToderoConfig): Promise<HealthResult> {
  try {
...
export async function serviceHealthChecks(
  ...
  results.push(
    definitionCurrent
      ...
      : {
          ...
          repairHint: "Run `todero service install` to regenerate the service definition",
        },
  );
        ...
        ? {
            ...
            repairHint:
              path.resolve(serviceExecutable) === path.resolve(resolveInstallStorePaths().shimPath)
                ? "Run `todero install` to restore the managed payload and shim, then `todero service start`"
                : `Restore the executable at ${serviceExecutable}, or unset PAPERCLIP_SHIM_PATH and run \`todero install\` followed by \`todero service install\` to re-point the service at the managed shim`,
          }
        : health.ok
          ? {
              name: "Service runtime",
              status: "fail",
              message: `${status.serviceName} is inactive but the configured port is serving another Todero process`,
              repairHint: "Run `todero service start`, or stop the conflicting foreground process first",
            }
          : {
              name: "Service runtime",
              status: "fail",
              message: `${status.serviceName} is ${status.detail ?? "inactive"}`,
              repairHint: "Run `todero service start`; inspect `todero service logs` if it does not stay up",
            },
  );
  ...
  results.push(
    !health.ok
      ? {
          ...
          repairHint: "Inspect `todero service status` and `todero service logs`",
        }
      : expectedVersion && health.version !== expectedVersion
        ? {
            ...
            repairHint: "Run `todero service restart --expected-version " + expectedVersion + "`",
          }
        : status.active
          ? {
              name: "Service health",
  ...
  if (status.enabled && status.linger === false) {
    results.push({
      ...
      repairHint: "Re-run `todero service install --enable-linger` if the service must survive logout",
    });
  }
