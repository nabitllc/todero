...
function writeBaseConfig(configPath: string) {
  const base: ToderoConfig = {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/todero-storage" },
      s3: {
        bucket: "todero",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/todero-secrets/master.key" },
    },
  };
...
describe("configure command", () => {
  it("sets a failing exit code for unknown sections", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "todero-configure-"));
  ...
  it("sets a failing exit code when no config exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "todero-configure-missing-"));
  ...
  it("backs up invalid config bytes and refuses non-interactive replacement", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "todero-configure-invalid-"));
    const configPath = path.join(root, "config.json");
