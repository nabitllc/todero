...
export class ToderoApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-todero-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  lines.push(
    "",
    "This usually means the Todero server is not running, the configured URL is wrong, or the request is being blocked before it reaches Todero.",
    "",
    "Try:",
    "- Start Todero with `pnpm dev` (from a source checkout) or `npx todero run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Todero is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`PAPERCLIP_API_URL\`.`,
  );
  return lines.join("\n");
}
