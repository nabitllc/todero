...
import { readRuntimeInfo, type ToderoRuntimeInfo } from "./runtime-info.js";
import {
  readInstallManifest,
...
type OnboardServiceDashboardConfig = {
  auth: Pick<ToderoConfig["auth"], "baseUrlMode" | "publicBaseUrl">;
  server: Pick<ToderoConfig["server"], "host" | "port">;
};

type OnboardServiceDashboardDependencies = {
  isInteractive: () => boolean;
  waitUntilReady: () => Promise<ToderoRuntimeInfo | null>;
...
async function waitUntilDashboardReady(timeoutMs = 60_000): Promise<ToderoRuntimeInfo | null> {
...
export function resolveOnboardServiceDashboardUrl(
  ...
  runtime?: Pick<ToderoRuntimeInfo, "dashboardUrl"> | null,
): string {
...
export async function handoffToOnboardedService(
  ...
  deps.info(`Todero dashboard: ${pc.cyan(dashboardUrl)}`);

  if (!runtime) {
    deps.warn(
      `The background service started, but the dashboard is not ready yet. ` +
        `Open ${dashboardUrl} after checking \`todero service logs\`.`,
    );
    return;
  }
  ...
  if (await deps.openDashboard(dashboardUrl)) {
    deps.success("Sent the Todero dashboard to your browser.");
  } else {
...
const defaultDependencies: OnboardServiceDependencies = {
      ...
      if (manifest?.source === "git" && manifest.repo) {
        ...
        return {
          ...
          reason:
            ...
            "run `todero install` (or `todero install --repo <repo> --ref <ref>` for source builds) first",
        };
      }
    } catch (error) {
      return {
  ...
  confirm: async () => {
    const answer = await p.confirm({
      message: "Install Todero as a background service?",
      initialValue: true,
    });
  ...
  confirmLinger: async () => {
    const answer = await p.confirm({
      message: "Allow Todero to keep running after logout? This may request system authorization.",
      initialValue: false,
    });
    return !p.isCancel(answer) && answer === true;
  },
...
export async function handleOnboardService(
  ...
  if (!explicitlyRequested && !canPrompt) {
    deps.info(
      "Background service not installed. Use `todero onboard --install-service` or `todero service install` to opt in.",
    );
    return false;
  }
  ...
  if (!shim.ok) {
    deps.warn(
      `Background service not installed: ${shim.reason ?? "the managed install could not be completed"}. ` +
        "Run `todero install`, then `todero service install`.",
    );
    return false;
  }
  if (shim.installedNow) {
    deps.success("Installed the managed todero payload and command shim for the service.");
  }
