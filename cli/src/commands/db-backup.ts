...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@todero/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://todero:todero@127.0.0.1:${port}/todero`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" todero db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveToderoInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "todero";
