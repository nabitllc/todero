...
const DEFAULT_AGENT_JWT_ISSUER = "todero";
const DEFAULT_AGENT_JWT_AUDIENCE = "todero-api";
...
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveToderoInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" todero env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: ToderoConfig | null = null;
...
function collectDeploymentEnvRows(config: ToderoConfig | null, configPath: string): EnvVarRow[] {
