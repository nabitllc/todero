...
import { isSupportedNodeVersion, MINIMUM_NODE_VERSION } from "@todero/shared/node-version";
import {
  addManagedPathBlock,
...
const DEFAULT_GITHUB_REPO = "nabitllc/todero";
  ...
  const visit = (packageName: string): void => {
    ...
    for (const section of ["dependencies", "optionalDependencies", "peerDependencies"] as const) {
      ...
      for (const dependencyName of Object.keys(dependencies)) {
        if (dependencyName.startsWith("@todero/")) visit(dependencyName);
      }
    }
...
export async function resolvePublishedVersion(spec: string, runCommand: CommandRunner): Promise<string> {
  const result = await runCommand(
    "npm",
    ["view", `todero@${spec}`, "version", "--json", `--registry=${PUBLIC_NPM_REGISTRY}`],
    { maxBuffer: 1024 * 1024 },
  );
...
async function runGitHubCurl(
  ...
  const configDir = fs.mkdtempSync(path.join(os.tmpdir(), "todero-gh-"));
...
export async function resolveGitHubRef(repo: string, ref: string, runCommand: CommandRunner): Promise<string> {
  const result = await runGitHubCurl(["--fail", "--silent", "--show-error", "--location", "--header", "Accept: application/vnd.github+json", "--header", "User-Agent: todero-install", `https://api.github.com/repos/${repo}/commits/${encodeURIComponent(ref)}`], runCommand, { maxBuffer: 4 * 1024 * 1024 });
  let sha: unknown;
...
function payloadEntrypoint(payloadPath: string): string {
  return path.join(payloadPath, "node_modules", "todero", "dist", "index.js");
}
...
export async function installGitPayload(repo: string, sha: string, runCommand: CommandRunner, paths = resolveInstallStorePaths()): Promise<{ payloadPath: string; reused: boolean; version: string }> {
  ...
  if (fs.existsSync(payloadPath)) {
    const metadata = JSON.parse(fs.readFileSync(path.join(payloadPath, "node_modules", "todero", "package.json"), "utf8")) as { version: string };
  ...
  try {
    ...
    await runCommand("corepack", ["pnpm", "-r", "--filter", "@todero/server...", "--if-present", "run", "build"], { cwd: checkoutPath, env: buildEnv(), maxBuffer: 32 * 1024 * 1024 });
    ...
    const cliTarball = tarballs.find((entry) => entry === `todero-${metadata.version}.tgz`);
...
async function ensureShimOnPath(options: InstallOptions): Promise<void> {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY || !rcPath) {
    console.log(pc.yellow(`Add Todero to PATH for this shell:\n  ${manualInstruction}`));
    return;
  }
...
export async function installCommand(
  ...
  if (gitRequest) {
    ...
    console.log(pc.green(`${installed.reused ? "Activated cached" : "Installed"} todero git payload ${sha.slice(0, 12)}.`));
    return;
  }
  const request = resolveNpmInstallRequest(options);
  console.log(`Resolving todero@${request.spec} from ${PUBLIC_NPM_REGISTRY}...`);
  const version = await resolvePublishedVersion(request.spec, runCommand);
  console.log(`Installing todero@${version}...`);
  ...
  console.log(pc.green(`${installed.reused ? "Activated cached" : "Installed"} todero ${version} (${request.channel}).`));
  console.log(pc.dim(`Payload: ${installed.payloadPath}`));
  console.log(`Run ${pc.cyan("todero --version")} to verify the managed install.`);
}
