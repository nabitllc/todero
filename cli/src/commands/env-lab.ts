...
export function resolveEnvLabSshStatePath(instanceId?: string): string {
  ...
  return path.resolve(
    resolveToderoInstanceRoot(resolvedInstanceId),
    "env-lab",
    "ssh-fixture",
    "state.json",
  );
...
export async function envLabDoctorCommand(opts: { instance?: string; json?: boolean }) {
  ...
  //
  // The doctor diagnoses the instance that `resolveToderoInstanceId` selects
  ...
  const cleanupInstance = resolveToderoInstanceId(opts.instance);
...
export function registerEnvLabCommands(program: Command) {
  ...
  envLab
    ...
    .option("-i, --instance <id>", "Todero instance id (default: current/default)")
