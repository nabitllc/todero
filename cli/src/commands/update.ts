...
import { resolveToderoInstanceId, resolveToderoInstanceRoot } from "../config/home.js";
...
function hasToderoInstanceData(): boolean {
  return Boolean(process.env.DATABASE_URL?.trim())
    || fs.existsSync(resolveConfigPath())
    || fs.existsSync(resolveToderoInstanceRoot());
}
...
async function runPreUpdateBackup(options: UpdateOptions, backup: () => Promise<void>, hasInstanceData = hasToderoInstanceData): Promise<void> {
  if (!hasInstanceData()) {
    const message = "Skipping the pre-update backup because this Todero instance has not been onboarded and has no data to back up.";
  ...
  try {
    ...
    if (isDatabaseUnreachableError(error)) {
      throw new Error(
        "The Todero database is not running or reachable, so the pre-update backup cannot be taken. Start the service with `todero service start` and retry, or skip the backup with `todero update --no-backup`.",
        { cause: error },
      );
    }
    throw error;
...
export function detectInstallMode(executablePath = process.argv[1] ?? "", paths = resolveInstallStorePaths()): InstallMode {
  ...
  if (normalized.includes("/node_modules/todero/")) return "global-npm";
...
export async function updateCommand(options: UpdateOptions, overrides: Partial<Dependencies> = {}): Promise<void> {
  ...
  if (options.rollback) {
    ...
    emit(options, { mode, action: "rollback", version: next.version, restarted }, pc.green(`Rolled back to todero ${next.version}${restarted ? " and restarted the active service" : ""}. Database migrations are not reversed; restore the pre-update backup if needed.`));
    return;
  }
  if (mode === "npx") { emit(options, { mode, action: "install" }, "This is an ephemeral npx install. Run `todero install`, then use `todero update` from the managed shim."); return; }
  if (mode === "source" || mode === "unknown") { emit(options, { mode, action: "manual" }, "This appears to be a source checkout. Update it with `git pull` followed by `pnpm install`; Todero will not mutate the repository."); return; }
  ...
  if (options.check) { emit(options, { mode, currentVersion: currentVersion ?? null, targetVersion, updateAvailable: comparison > 0, downgrade: comparison < 0, channel: request.channel }, comparison > 0 ? `Update available: ${targetVersion}` : comparison < 0 ? `Target ${targetVersion} is older than ${currentVersion}.` : `todero ${targetVersion} is current.`); if (comparison > 0) process.exitCode = 10; return; }
  if (mode === "global-npm") {
    if (comparison < 0 && options.yes !== true) { const confirmed = await (overrides.confirm ?? defaultConfirm)(`Downgrade todero from ${currentVersion} to ${targetVersion}?`); if (!confirmed) throw new Error("Downgrade cancelled. Re-run with --yes to confirm explicitly."); }
    const args = ["install", "-g", `todero@${targetVersion}`, `--registry=${PUBLIC_NPM_REGISTRY}`, `--@todero:registry=${PUBLIC_NPM_REGISTRY}`]; console.log(`Running: npm ${args.join(" ")}`);
    if (!options.dryRun) {
      const npmConfigDir = fs.mkdtempSync(path.join(os.tmpdir(), "todero-npm-"));
      const npmUserConfigPath = path.join(npmConfigDir, "npmrc");
      try {
        fs.writeFileSync(npmUserConfigPath, `registry=${PUBLIC_NPM_REGISTRY}\n@todero:registry=${PUBLIC_NPM_REGISTRY}\n`, { mode: 0o600 });
  ...
  if (comparison === 0) { emit(options, { mode, currentVersion, targetVersion, changed: false }, `todero ${targetVersion} is already active.`); return; }
  if (comparison < 0 && options.yes !== true) { const confirmed = await (overrides.confirm ?? defaultConfirm)(`Downgrade todero from ${currentVersion} to ${targetVersion}?`); if (!confirmed) throw new Error("Downgrade cancelled. Re-run with --yes to confirm explicitly."); }
  if (options.dryRun) { emit(options, { mode, currentVersion, targetVersion, action: comparison < 0 ? "downgrade" : "update", backup: options.backup !== false, dryRun: true }, `Would ${comparison < 0 ? "downgrade" : "update"} todero ${currentVersion} → ${targetVersion}${options.backup === false ? " without a backup" : " after a database backup"}.`); return; }
  if (options.backup !== false) await runPreUpdateBackup(options, overrides.backup ?? (() => dbBackupCommand({})), overrides.hasInstanceData);
  const installed = await withInstallStoreLock(async () => {
  ...
  emit(options, { mode, currentVersion, targetVersion, changed: true, reused: installed.reused, restarted }, pc.green(`Updated todero ${currentVersion} → ${targetVersion}${restarted ? " and restarted the active service" : ""}. Run \`todero update --rollback\` for an instant payload rollback.`));
}
