...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className={cn("todero-markdown-issue-ref", "font-normal underline")}
      title={title}
...
function MarkdownCaseLink({
  ...
  return (
    <Link
      ...
      className={cn("todero-markdown-case-ref", "font-normal underline")}
...
function MarkdownExternalLink({
  ...
  return (
    <a
      ...
      className="todero-markdown-external-ref"
    >
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-todero-wiki-link": "true",
        "data-todero-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="todero-markdown-codeblock" data-wrap-lines={wrapLines || undefined}>
      <pre
        {...preProps}
        ref={preRef}
        style={{
      ...
      <div
        className="todero-markdown-codeblock-actions"
      ...
      >
        <button
          ...
          title={wrapLabel}
          className="todero-markdown-codeblock-action todero-markdown-codeblock-wrap"
          style={wrapLines
            ? {
        ...
        <button
          ...
          title={copyLabel}
          className="todero-markdown-codeblock-action todero-markdown-codeblock-copy"
        ...
        >
          ...
          <span className="todero-markdown-codeblock-action-label">{copyLabel}</span>
        </button>
      </div>
    </div>
  );
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`todero-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="todero-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("todero-mermaid-status", error && "todero-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="todero-mermaid-source">
...
function MarkdownBodyImpl({
  ...
  const components = useMemo<Components>(() => {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="todero-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-todero-wiki-link"] === "true";
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className={cn(
              "todero-mention-chip",
              `todero-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "todero-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "todero-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
