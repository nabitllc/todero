...
import type { Agent, AttentionDetailImage, AttentionItem } from "@todero/shared";
  ...
  const decision = useMutation<unknown, Error, CompactDecisionAction>({
    mutationFn: (action: CompactDecisionAction) => {
      ...
      if (item.sourceKind === "issue_thread_interaction") {
        ...
        if (typeof issueId !== "string") throw new Error("Missing task reference for this decision.");
...
function InlineResolver({
  ...
  if (item.sourceKind === "issue_thread_interaction") {
    ...
    if (!issueId) {
      return <p className="text-xs text-muted-foreground">This decision is missing a link to its task.</p>;
    }
    return (
      <>
