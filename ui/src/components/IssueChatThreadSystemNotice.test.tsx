...
import type { Agent, SuccessfulRunHandoffState } from "@todero/shared";
...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
    ...
    expect(container.textContent).toContain("Todero needs a disposition");
  ...
  it("labels system notice source as Todero when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("Todero");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to Todero in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("Todero");
  });
  ...
  it("folds stale successful-run disposition warnings into the activity log disclosure style", () => {
    ...
    expect(row?.textContent).not.toContain("Todero needs a disposition before this issue can continue.");
  ...
  it("keeps the required disposition warning loud when no live continuation exists", () => {
    ...
    expect(container.textContent).toContain("Todero needs a disposition before this issue can continue.");
  });
