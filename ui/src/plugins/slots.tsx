 ...
 *
 * Plugin bundles are built with `external: ["@todero/plugin-sdk/ui", "react", "react-dom"]`,
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        ...
        if (!R) {
          throw new Error("Todero plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
    ...
    case "sdk-ui":
      source = `
        ...
        function missing(name) {
          return function MissingToderoSdkUiComponent() {
            throw new Error('Todero plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@todero/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@todero/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@todero/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@todero/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@todero/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
 ...
 * self-register via `window.toderoPlugins.registerReactComponent()`.
 ...
 * of `@todero/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
