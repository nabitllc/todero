...
import type { Agent, IssueComment } from "@todero/shared";
...
function authorNameForComment(
  ...
  if (authorAgentId) {
    return agentMap?.get(authorAgentId)?.name ?? (options?.isSystemNotice ? "Todero" : authorAgentId.slice(0, 8));
  }
  const authorUserId = comment.authorUserId ?? null;
  if (!authorUserId) return options?.isSystemNotice ? "Todero" : "You";
