...
describe("TeamCatalog routes", () => {
  it("round-trips file paths containing literal tildes", () => {
    const route = teamRoute("todero/bundled/test/team", "agents/a~b/AGENTS.md");

    expect(route).toBe("/teams-catalog/todero%2Fbundled%2Ftest%2Fteam/files/agents%7Ea~b%7EAGENTS.md");
    expect(parseTeamRoute(route.replace("/teams-catalog/", ""))).toEqual({
      catalogRef: "todero/bundled/test/team",
...
function makeTeam(overrides: Partial<CatalogTeam> = {}): CatalogTeam {
  return {
    id: "team-no-deps",
    key: "todero/bundled/company-defaults/team-no-deps",
    kind: "bundled",
    ...
    mockTeamCatalogApi.installed.mockResolvedValue([
      {
        ...
        catalogKey: "todero/bundled/company-defaults/team-no-deps",
        present: true,
