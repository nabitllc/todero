...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes Todero config", async () => {
    ...
    expect(document.body.textContent).toContain("from Todero only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Todero"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
  ...
  it("writes a new value through the provider for external reference secrets", async () => {
    const externalSecret = makeCompanySecret({
      ...
      name: "todero-cloud/prod/provider/neon/admin-api-key",
      provider: "aws_secrets_manager",
      managedMode: "external_reference",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:todero-cloud/prod/provider/neon/admin-api-key",
      providerConfigId: "vault-aws",
    });
  ...
  it("explains AWS managed secret creation failures with actionable safe details", async () => {
    const rawProviderMessage =
      "AccessDeniedException: arn:aws:sts::123456789012:assumed-role/prod/Todero is not authorized";
  ...
  it("discovers AWS provider vault candidates and applies selected values as prefill", async () => {
    ...
    await act(async () => {
      ...
      setInputValue(prefixInput!, "todero");
    });
    await flushReact();
    ...
    expect((document.getElementById("provider-vault-secret-name-prefix") as HTMLInputElement).value).toBe("todero");
    expect((document.getElementById("provider-vault-kms-key-id") as HTMLInputElement).value).toBe("alias/todero-secrets");
  ...
  it("shows AWS discovery errors without replacing manual vault form values", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Todero is not authorized";
    ...
    expect(errorBanner?.textContent).toContain("Todero server runtime/provider credential path");
