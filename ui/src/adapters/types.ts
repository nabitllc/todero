...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@todero/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@todero/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@todero/adapter-utils").TranscriptEntry[];
