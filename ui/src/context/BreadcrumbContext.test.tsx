...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Todero", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Todero",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Todero");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Todero");
    expect(buildDocumentTitle([], null)).toBe("Todero");
  });
});
