...
description: >
  Convert Todero plans into executable issue graphs. Use when asked to plan,
  scope, or break down Todero company work into assigned tasks with specialty
  fit, dependencies, blockers, and parallelization.
---

# Todero — Converting Plans to Tasks

A companion skill for turning a plan into executable Todero work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of Todero works for you.

For the **mechanics** of recording a plan (issue document with key `plan`, comment links, approval gating, who to reassign back to), follow the _Planning_ section of the `todero` skill. This skill covers planning method, not the API surface.
...
- Not a replacement for the `todero` skill's planning mechanics. Use both.
