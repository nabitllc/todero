...
Use the helper bundled with this skill. From an installed `todero` skill directory, the helper lives at `scripts/todero-upload-artifact.sh`:

```bash
scripts/todero-upload-artifact.sh path/to/output.webm \
...
If the helper is unavailable, use the Todero API directly:
