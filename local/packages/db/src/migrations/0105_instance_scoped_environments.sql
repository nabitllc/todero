  ...
  WITH ranked AS (
    SELECT
      ...
      CASE
        ...
        WHEN e.driver = 'sandbox' AND (e.metadata ->> 'managedByTodero')::boolean = true
...
SELECT
  ...
  '{}'::jsonb,
  '{}'::jsonb,
  '{"managedByTodero": true, "defaultForInstance": true}'::jsonb,
  now(),
  now()
WHERE NOT EXISTS (
  SELECT 1
...
UPDATE "environments"
SET
  "metadata" = COALESCE("metadata", '{}'::jsonb) || '{"managedByTodero": true, "defaultForInstance": true}'::jsonb,
  "updated_at" = now()
WHERE "driver" = 'local';
--> statement-breakpoint
...
CREATE UNIQUE INDEX "environments_managed_sandbox_idx"
  ON "environments" USING btree ("driver")
  WHERE "driver" = 'sandbox' AND ("metadata" ->> 'managedByTodero')::boolean = true;
--> statement-breakpoint
