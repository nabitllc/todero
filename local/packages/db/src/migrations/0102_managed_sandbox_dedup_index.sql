...
-- Keeps the oldest row per (company_id) where driver='sandbox' AND metadata.managedByTodero is true.
-- Scoped to Todero-managed rows only: tenant-created sandbox envs (without the marker) are untouched.
DO $$
DECLARE
...
BEGIN
  WITH ranked AS (
    ...
    WHERE driver = 'sandbox'
      AND (metadata ->> 'managedByTodero')::boolean = true
  ),
  deleted AS (
