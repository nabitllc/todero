...
const CATALOG_PACKAGE_NAME = "@todero/skills-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @todero/skills-catalog build:manifest.");
    }
  }
...
function skillIdForCandidate(candidate: BaseSkillCandidate) {
  return `todero:${candidate.kind}:${candidate.category}:${candidate.slug}`;
}
