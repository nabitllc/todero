...
function entry(overrides: Partial<ToderoSkillEntry> & Pick<ToderoSkillEntry, "key">): ToderoSkillEntry {
  return {
...
describe("buildSkillLibraryManifestMarkdown", () => {
  ...
  it("renders enabled, not-enabled, and broken states deterministically and key-sorted", () => {
    const entries = [
      ...
      entry({ key: "nabitllc/todero/todero" }),
      entry({
    ...
    const desiredSkillKeys = new Set(["nabitllc/todero/todero", "acme/tools/broken"]);
    ...
    expect(manifest).toContain("- nabitllc/todero/todero — enabled");
    ...
    const coreIndex = manifest!.indexOf("nabitllc/todero/todero —");
