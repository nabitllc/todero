...
export interface AdapterEnvironmentTestContext {
   ...
   *
   * If omitted (or `kind === "local"`), the adapter tests on the Todero
...
export interface AdapterRuntimeCommandSpec {
  /**
   * The command Todero should execute for this adapter in the current config.
   */
  command: string;
  /**
...
export interface ServerAdapterModule {
  ...
  /** How this adapter receives Todero's run-scoped control tools. */
   ...
   * rather than reading config.toderoRuntimeSkills.
   */
