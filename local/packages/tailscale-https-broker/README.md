# Todero Tailscale HTTPS broker

Least-privilege host broker that manages **only** Todero-owned, tailnet-only,
same-number HTTPS-to-loopback listeners for managed branch runtimes.

It exists so the Todero app/agent account never gains Tailscale operator
...
for the configured Todero service UID. Connection deadlines destroy the
...
## One-time host installation (`todero-dev`)

These steps require **root** and must be run by CloudOps/host owner, not the
Todero agent account. They install the broker as a dedicated
Tailscale-operator service account distinct from the Todero app account.
...
2. **Create the dedicated operator account and socket group.**
   ...
   ```sh
   sudo useradd --system --home /var/lib/todero-tailscale-broker \
     --shell /usr/sbin/nologin todero-tsbroker
   sudo groupadd --system todero-tsbroker-sock
   # The Todero *app* service account must have this as its PRIMARY group so
   # its SO_PEERCRED gid matches the socket group (supplemental membership is
   # intentionally NOT accepted).
   sudo usermod -g todero-tsbroker-sock <todero-app-account>
   ```

3. **Grant Tailscale operator authority to the broker account only.**

   ```sh
   sudo tailscale set --operator=todero-tsbroker
   ```

   Do **not** grant `--operator` to the Todero app/agent account (that grant
   was explicitly rejected in PAP-16989).

4. **Create state directories (not writable by the Todero app).** The
   packaged unit creates these automatically; for a manual install use:

   ```sh
   sudo install -d -o todero-tsbroker -g todero-tsbroker-sock -m 0750 /run/todero-tailscale-broker
   sudo install -d -o todero-tsbroker -g todero-tsbroker-sock -m 0700 /var/lib/todero-tailscale-broker
   sudo install -d -o todero-tsbroker -g todero-tsbroker-sock -m 0700 /var/log/todero-tailscale-broker
   ```

   The broker refuses to start if the registry path's parent is group/other
   writable.

5. **Build, install the package under `/opt/todero`, and install the
   packaged systemd unit.** The unit's `ExecStart` (and the doctor command
   below) run the build output from
   `/opt/todero/packages/tailscale-https-broker/dist`, so copy it there
   ...
   ```sh
   pnpm --filter @todero/tailscale-https-broker build
   sudo install -d -m 0755 /opt/todero/packages/tailscale-https-broker
   sudo cp -r packages/tailscale-https-broker/dist \
     /opt/todero/packages/tailscale-https-broker/
   sudo install -D -m 0644 \
     packages/tailscale-https-broker/deploy/todero-tailscale-https-broker.service \
     /etc/systemd/system/todero-tailscale-https-broker.service
   sudo install -d -m 0750 /etc/todero
   sudoedit /etc/todero/tailscale-https-broker.env
   ```
   ...
   ```ini
   [Unit]
   Description=Todero Tailscale HTTPS broker
   After=tailscaled.service
   Requires=tailscaled.service

   [Service]
   Type=simple
   User=todero-tsbroker
   # Socket must end up 0660 todero-tsbroker:todero-tsbroker-sock. Set the group here and
   # the broker chmods the socket to 0660 on bind.
   Group=todero-tsbroker-sock
   EnvironmentFile=/etc/todero/tailscale-https-broker.env
   ExecStart=/usr/bin/node /opt/todero/packages/tailscale-https-broker/dist/main.js
   Restart=on-failure
   NoNewPrivileges=true
   ProtectSystem=strict
   ReadWritePaths=/run/todero-tailscale-broker /var/lib/todero-tailscale-broker /var/log/todero-tailscale-broker
   ...
   `PAPERCLIP_TAILSCALE_BROKER_SOCKET=/run/todero-tailscale-broker/broker.sock`
   on the Todero service only if overriding its default.
   ...
   | `BROKER_SERVICE_UID` | yes | — | UID of the Todero **app** account allowed to connect |
   | `BROKER_SERVICE_GID` | yes | — | GID of the dedicated socket group (caller's primary GID) |
   | `BROKER_RUNTIME_UID` | yes | — | UID that owns Todero-managed runtime processes (normally the Todero app service account); only its loopback listeners are eligible |
   | `BROKER_TAILSCALE_BIN` | no | `/usr/bin/tailscale` | absolute path to the Tailscale CLI |
   | `BROKER_SOCKET_PATH` | no | `/run/todero-tailscale-broker/broker.sock` | Unix socket path |
   | `BROKER_REGISTRY_PATH` | no | `/var/lib/todero-tailscale-broker/registry.json` | root-owned `0600` ownership registry |
   | `BROKER_AUDIT_PATH` | no | `/var/log/todero-tailscale-broker/audit.log` | append-only security audit log |
   ...
   *provenance-blind*: it protects only entries the broker has no lease for. It
   ...
   ```sh
   sudo -u todero-tsbroker \
     env $(cat /etc/todero/tailscale-https-broker.env | xargs) \
     node /opt/todero/packages/tailscale-https-broker/dist/main.js --doctor
   ```
...
7. **Enable.** `sudo systemctl daemon-reload && sudo systemctl enable --now
   todero-tailscale-https-broker`. Confirm the socket is `0660
   todero-tsbroker:todero-tsbroker-sock`.

## Upgrade

Deploy new package output to
`/opt/todero/packages/tailscale-https-broker/dist`, then
`sudo systemctl restart todero-tailscale-https-broker`. On
...
1. Disable the exposure flag on the project runtime (Todero stops requesting
   `expose`). Existing previews drain on runtime stop.
2. Drain owned listeners: stop each managed runtime so Todero issues `remove`
   for its own leases (proven by handle).
3. `sudo systemctl disable --now todero-tailscale-https-broker`.
...
```sh
pnpm --filter @todero/tailscale-https-broker test        # 72 tests
pnpm --filter @todero/tailscale-https-broker typecheck
pnpm --filter @todero/tailscale-https-broker build
```
