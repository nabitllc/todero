...
const EXPECTED_BUNDLED_KEYS = [
  "todero/bundled/company-defaults/core-exec-team",
  "todero/bundled/product/product-design",
  "todero/bundled/software-development/product-engineering",
];

const EXPECTED_OPTIONAL_KEYS = [
  "todero/optional/content/content-machine",
];
...
describe("shipped teams catalog", () => {
  ...
  it("uses canonical todero keys derived from kind/category/slug", () => {
    ...
    for (const team of catalogTeams) {
      const expectedKey = `todero/${team.kind}/${team.category}/${team.slug}`;
      const expectedId = `todero:${team.kind}:${team.category}:${team.slug}`;
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@todero/teams-catalog");
  ...
  it("resolves shipped teams by id, key, and unique slug", () => {
    const sample = catalogTeams.find((team) => team.key === "todero/bundled/company-defaults/core-exec-team");
