...
export const PLUGIN_ID = "todero.plugin-llm-wiki";
...
export const PAPERCLIP_DISTILL_SKILL_KEY = "todero-distill";
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/todero-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded Todero issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). Todero-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space Todero ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up Todero issue with the exact files and evidence.
...
const manifest: PaperclipPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, Todero project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    },
    {
      ...
      displayName: "Todero Distill",
      slug: PAPERCLIP_DISTILL_SKILL_KEY,
      description: "Turn Todero cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
