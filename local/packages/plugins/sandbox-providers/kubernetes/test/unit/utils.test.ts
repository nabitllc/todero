...
import { deriveCompanySlug, deriveNamespaceName, newRunUlidDns, toderoLabels } from "../../src/utils.js";
...
describe("deriveNamespaceName", () => {
  it("concatenates prefix and slug", () => {
    expect(deriveNamespaceName("todero-", "acme-co")).toBe("todero-acme-co");
  });
});
...
describe("toderoLabels", () => {
  it("returns canonical label map", () => {
    const labels = toderoLabels({ runId: "r1", agentId: "a1", companyId: "c1", adapterType: "claude_local" });
    expect(labels["todero.io/run-id"]).toBe("r1");
    expect(labels["todero.io/agent-id"]).toBe("a1");
    expect(labels["todero.io/company-id"]).toBe("c1");
    expect(labels["todero.io/adapter"]).toBe("claude_local");
    expect(labels["todero.io/managed-by"]).toBe("todero-k8s-plugin");
  });
});
