# @todero/plugin-kubernetes (alpha)

First-party Todero sandbox-provider plugin for Kubernetes.

**Alpha:** the default backend (`sandbox-cr`) is built on `kubernetes-sigs/agent-sandbox` v1alpha1 — expect breaking changes as that CRD evolves toward Beta. A stable fallback backend (`job`, using `batch/v1` Job) is available for clusters without agent-sandbox installed, but it does NOT support multi-command exec (todero-server's adapter-install pattern requires sandbox-cr).
...
3. Todero-server running with access to the cluster (in-cluster via `inCluster: true` or external via `kubeconfig`)

### For `job` backend (stable fallback)

1. A Kubernetes cluster running k8s 1.27+
2. Todero-server with cluster access — no additional controllers or CRDs required

## Installation

```bash
todero plugin install @todero/plugin-kubernetes
```

Or, for local development:

```bash
todero plugin install --local /path/to/todero/packages/plugins/sandbox-providers/kubernetes
```
...
**`sandbox-cr` (default):** Creates a `Sandbox` CR (`agents.x-k8s.io/v1alpha1`) whose controller provisions a long-lived pod running `sleep infinity`. todero-server execs individual commands into the running pod — this is the multi-command adapter-install pattern. When you `releaseLease`, the Sandbox CR is deleted and the controller tears down the pod.

**`job` (stable fallback):** Creates a `batch/v1` Job. The container entrypoint runs once and exits — no multi-command exec possible. Use this when you cannot install agent-sandbox, or when you need strictly stable Kubernetes APIs. Note: todero-server's adapter-install pattern will not work in job mode.
...
- `inCluster: true` — use the in-pod ServiceAccount credentials (when todero-server runs inside the same cluster).
- `kubeconfig: <YAML>` — inline kubeconfig (stored as a company secret).
- `kubeconfigSecretRef: <secret-uuid>` — reference to an existing Todero secret.
...
| `namespacePrefix` | `"todero-"` | Prefix for the per-company tenant namespace. |
...
```
Namespace          todero-{companySlug}        (PSS: restricted enforce + audit)
ServiceAccount     todero-tenant-sa
Role               todero-tenant-role          (only get pods/log)
RoleBinding        todero-tenant-rb
ResourceQuota      todero-quota                (pods, requests/limits cpu+memory)
LimitRange         todero-limits               (container max/min/default/defaultRequest)
NetworkPolicy      todero-deny-all             (deny ingress + egress baseline)
NetworkPolicy      todero-egress-allow         (DNS + todero-server callback + user CIDRs)
                   OR CiliumNetworkPolicy todero-egress-fqdn if egressMode=cilium
```
...
- `readOnlyRootFilesystem: true` with explicit `emptyDir` mounts for `/workspace`, `/home/todero`, `/home/todero/.cache`, `/tmp`
...
- `automountServiceAccountToken: true` (for the agent shim's todero-server callback)

Plus per-namespace `pod-security.kubernetes.io/enforce: restricted` and a deny-all NetworkPolicy baseline with explicit egress allow-list (DNS, todero-server, configured FQDNs/CIDRs).
...
To run the kind-cluster integration test (requires `kubectl --context kind-todero` and a pre-loaded alpine image; see `test/integration/end-to-end-run.test.ts`):
