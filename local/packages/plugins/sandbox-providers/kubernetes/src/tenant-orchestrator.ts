...
const SERVICE_ACCOUNT_NAME = "todero-tenant-sa";
const ROLE_NAME = "todero-tenant-role";
const ROLE_BINDING_NAME = "todero-tenant-rb";
const RESOURCE_QUOTA_NAME = "todero-quota";
const LIMIT_RANGE_NAME = "todero-limits";
 ...
 * old todero-egress-allow NetworkPolicy in place alongside the new
...
async function ensureNetworkPolicies(clients: KubeClients, input: EnsureTenantInput): Promise<void> {
  const [denyAll, egressStd] = buildNetworkPolicyManifests({
    namespace: input.namespace,
    toderoServerNamespace: input.toderoServerNamespace,
