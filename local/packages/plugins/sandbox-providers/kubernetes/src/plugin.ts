...
function generateBootstrapToken(): string {
  ...
  // callback auth scheme; todero-server's callback auth is out of scope for
...
const plugin = definePlugin({
  ...
  async onEnvironmentAcquireLease(
    ...
    // it in todero-server logs even if they missed the validation step.
    ...
    await ensureTenant(clients, {
      ...
      toderoServerNamespace: PAPERCLIP_SERVER_NAMESPACE,
  ...
  async onEnvironmentExecute(
    ...
    if (leaseBackend === "sandbox-cr") {
      ...
      // The container entrypoint is baked into the Job spec (Tini + todero-agent-shim).
