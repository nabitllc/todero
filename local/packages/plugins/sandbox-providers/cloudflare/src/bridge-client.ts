...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-Todero-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-Todero-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-Todero-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/todero-sandbox/v1";
