...
This Worker is the operator-facing bridge used by `@todero/plugin-cloudflare-sandbox`.

It exposes a small authenticated JSON API under `/api/todero-sandbox/v1` and translates Todero lease and command requests into Cloudflare Sandbox SDK calls.
...
- clean up timed-out sessions so Todero does not inherit wedged background processes
...
After deploy, configure Todero with:
