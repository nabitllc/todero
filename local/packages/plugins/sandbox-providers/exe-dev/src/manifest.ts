...
const PLUGIN_ID = "todero.exe-dev-sandbox-provider";
...
const manifest: PaperclipPluginManifestV1 = {
  ...
  description:
    "Sandbox provider plugin that provisions exe.dev VMs as Todero execution environments.",
  author: "Todero",
  ...
  environmentDrivers: [
    {
      ...
      description:
        "Provisions exe.dev VMs through the HTTPS API, then runs commands over direct SSH for long-lived Todero workloads.",
      configSchema: {
        type: "object",
        properties: {
          ...
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Paste your exe.dev API token, or pick a saved Todero secret. Create one at exe.dev → Settings → API tokens with `/exec` scope (`new`, `ls`, `rm`).",
          },
          sshPrivateKey: {
            type: "string",
            format: "secret-ref",
          ...
          sshUser: {
            ...
            "x-todero-advanced": true,
            "x-todero-group": "SSH access",
          },
          sshIdentityFile: {
            type: "string",
            description:
              "Absolute path to a private key on the Todero host. Used only when SSH Private Key is empty.",
            "x-todero-advanced": true,
            "x-todero-group": "SSH access",
          },
          sshPort: {
