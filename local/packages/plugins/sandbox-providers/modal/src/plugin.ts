...
const DEFAULT_WORKDIR = "/workspace/todero";
...
function resolveAuth(config: ModalDriverConfig): { tokenId: string; tokenSecret: string } | null {
  ...
  // environment config, which Todero stores as company secrets.
...
function warnIfUnsupportedNode(logger: { warn: (msg: string) => void } | undefined): void {
  ...
  if (Number.isFinite(major) && major < 24) {
    const message = `Modal sandbox provider is running on Node ${process.versions.node}; Todero requires Node 24+. Upgrade the host runtime before using this plugin.`;
    logger?.warn(message);
  }
}
  ...
  async onEnvironmentProbe(
    ...
    try {
      ...
      try {
        ...
        const proc = await sandbox.exec(["sh", "-lc", "printf todero-probe"]);
        const { stdout, exitCode } = await readProcessStreams(proc);
        if (exitCode !== 0 || stdout.trim() !== "todero-probe") {
          return {
            ok: false,
  ...
  async onEnvironmentExecute(
    ...
    try {
      ...
      const stdinPath = params.stdin != null
        ? `/tmp/todero-stdin-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
        : null;
      try {
