...
Official TypeScript SDK for Todero plugin authors.

- **Worker SDK:** `@todero/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@todero/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@todero/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@todero/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@todero/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@todero/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@todero/plugin-sdk/ui` | UI entry: hooks, host navigation, HTTP-safe clipboard copy, shared components, and slot prop types |
| `@todero/plugin-sdk/ui/hooks` | Hooks only |
| `@todero/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@todero/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@todero/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@todero/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@todero/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@todero/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Todero app. They can call ordinary Todero HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Todero instance at runtime.
...
- The host ships a small shared React component kit through `@todero/plugin-sdk/ui`. Use it for native Todero controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@todero/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@todero/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Todero information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@todero/plugin-sdk`.
...
metadata. Slow or failing plugins are isolated: Todero logs the failure and
...
await ctx.issues.createComment(issueId, replyText, companyId, {
  actorUserId: verifiedSlackUser.toderoUserId,
});
```
...
```tsx
import { usePluginData, usePluginAction } from "@todero/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@todero/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@todero/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@todero/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@todero/plugin-sdk/ui";
...
Routes Todero-internal plugin links through the host router without a full document reload. Use `linkProps()` for anchors so the browser still gets a real `href` for copy-link, modifier-click, middle-click, and open-in-new-tab behavior.

```tsx
import { useHostNavigation } from "@todero/plugin-sdk/ui";
...
Avoid raw same-origin `href`s or `window.location.assign()` for Todero-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
```tsx
import { usePluginStream } from "@todero/plugin-sdk/ui";
...
The host provides selected shared UI components through `@todero/plugin-sdk/ui`.
...
```tsx
import { copyTextToClipboard } from "@todero/plugin-sdk/ui";
...
used by Todero issue comments and documents:

```tsx
import { MarkdownBlock, MarkdownEditor } from "@todero/plugin-sdk/ui";
...
```tsx
<MarkdownBlock
  content={"See [[wiki/entities/todero|Todero]]."}
  enableWikiLinks
...
```tsx
import { FileTree, type FileTreeNode } from "@todero/plugin-sdk/ui";
...
or configure work against Todero entities. Both are controlled components and
load their options from the host for the provided company.

```tsx
import { AssigneePicker, ProjectPicker } from "@todero/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@todero/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@todero/plugin-sdk/ui";
...
```ts
import { createPluginBundlerPresets } from "@todero/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@todero/plugin-sdk/dev-server";
