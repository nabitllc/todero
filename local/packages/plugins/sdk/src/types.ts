/**
 * Core types for the Todero plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@todero/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @todero/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Todero object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Todero UI. */
  title?: string;
...
export interface PluginLocalFoldersClient {
  ...
  declarations(): import("@todero/shared").PluginLocalFolderDeclaration[];
...
/**
 * `ctx.events` — subscribe to and emit Todero domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Todero domain event or a plugin-namespaced event.
   *
 ...
 * Todero secret provider system at execution time.
 *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed Todero routines.
 *
 ...
 * ```
 *
 * @see usePluginStream in `@todero/plugin-sdk/ui`
 */
