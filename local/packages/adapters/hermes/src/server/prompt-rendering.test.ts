...
test("renders standard assignment wake with task authority and no backlog discovery guidance", () => {
  ...
  expect(prompt).toContain("Todero task context:");
...
test("renders scoped planning wake authority before the Hermes default workflow", () => {
  const prompt = buildPrompt(baseContext(), {
    toderoApiUrl: "http://127.0.0.1:3101/api",
  });
...
test("renders resume deltas instead of full scoped-wake boilerplate when continuing a session", () => {
  ...
  expect(prompt).toContain("## Todero Resume Delta");
  expect(prompt).toContain("You are resuming an existing Todero session.");
...
test("renders safe Todero API examples from environment variables with multiline update preservation", () => {
  const prompt = buildPrompt(baseContext(), {
    toderoApiUrl: "http://todero.local/api",
  });
  ...
  expect(prompt).toContain('-H "X-Todero-Run-Id: $PAPERCLIP_RUN_ID"');
...
test("preserves custom prompt templates while exposing runtime and wake variables", () => {
  const prompt = buildPrompt(baseContext(), {
    ...
    promptTemplate: [
      ...
      "agent={{agent.name}}",
      "api={{toderoApiUrl}}",
      "keyEnv={{toderoApiKeyEnv}}",
      "runEnv={{toderoRunIdEnv}}",
      "wakePrompt={{toderoWakePrompt}}",
      "task={{toderoTaskMarkdown}}",
      "wakeJson={{toderoWakeJson}}",
  ...
  expect(prompt).toContain("api=http://todero.local/api");
  expect(prompt).toContain("keyEnv=PAPERCLIP_API_KEY");
  expect(prompt).toContain("runEnv=PAPERCLIP_RUN_ID");
  expect(prompt).toContain("wakePrompt=## Todero Wake Payload");
  expect(prompt).toContain("task=Todero task context:");
  expect(prompt).toContain("wakeJson={\"reason\":\"issue_assigned\"");
  expect(prompt).toContain('"reason":"issue_assigned"');
  expect(prompt).toContain("## Todero Wake Payload");
  expect(prompt).toContain("Issue description:\n```text\nUse the wake payload as runtime authority.\n```");
  expect(prompt).not.toContain("Todero runtime identity:");
});
