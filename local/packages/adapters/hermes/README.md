# Todero Adapters for Hermes Agent

A [Todero](https://todero.vercel.app) adapter package that lets you run [Hermes Agent](https://github.com/NousResearch/hermes-agent) as a managed employee in a Todero company.
...
Choose `hermes_local` when Todero and Hermes run on the same trusted host
and Todero should start `hermes chat` for each heartbeat. Choose
...
- **Skills integration** — Scans both Todero-managed and Hermes-native skills (`~/.hermes/skills/`), with sync/list/resolve APIs
- **Structured transcript parsing** — Raw Hermes stdout is parsed into typed `TranscriptEntry` objects so Todero renders proper tool cards with status icons and expand/collapse
...
This package ships with Todero core as the built-in `hermes_local` and
...
manager, or add an entry to `~/.todero/adapter-plugins.json` and restart
Todero. The external package can override either built-in Hermes adapter
while it is enabled:

```json
[
  {
    "packageName": "@todero/hermes-paperclip-adapter",
    "localPath": "/absolute/path/to/todero/packages/adapters/hermes",
    "type": "hermes_local",
...
Todero core imports these same package entrypoints for built-in adapter
registration.

### 2. Create a local Hermes agent in Todero

In the Todero UI or via API, create an agent with adapter type `hermes_local`:
...
This mode shells out to the local `hermes` CLI. Todero injects runtime
environment variables and captures stdout/stderr from the child process.

### 3. Create a Hermes gateway agent in Todero
...
`http://127.0.0.1:9119/chat`, Todero maps it to
`http://127.0.0.1:9119/api` before calling Hermes API routes. `/chat` and the
dashboard root are browser UI routes; Todero tests `/api/health` and starts
...
`@todero/adapter-hermes-gateway` remains as a deprecated compatibility shim
for one release. It re-exports the gateway entrypoints from
`@todero/hermes-paperclip-adapter/gateway` and preserves the legacy exports
for existing plugin installs. New installs and built-in Todero registrations
should use `@todero/hermes-paperclip-adapter`; the adapter type remains
`hermes_gateway`.

### Runtime API guidance

Hermes receives Todero runtime identity through environment variables:
...
`Authorization: Bearer $PAPERCLIP_API_KEY` on Todero API requests and
`X-Todero-Run-Id: $PAPERCLIP_RUN_ID` on mutating issue requests. For
multiline comments or status updates, preserve newlines with a heredoc plus
`jq --arg`.

### Hermes-originated Todero tasks

The package includes a Hermes skill/helper for the reverse direction: a user
starts in Hermes and asks Hermes to create or update Todero work. This is not
the same as Todero waking Hermes through `hermes_local` or `hermes_gateway`.

Configure Todero access in Hermes env/profile secrets, not prompt text:
...
Create issues in Todero and assign them to your Hermes agent. On each heartbeat, Hermes will:

1. Receive the task instructions
2. Use its full tool suite to complete the work
3. Report results back to Todero
...
| `paperclipApiUrl` | string | `http://127.0.0.1:3100/api` | Todero API base URL |
...
| `{{paperclipApiUrl}}` | Todero API base URL |
...
6. **Reports** results back to Todero with cost, usage, and session state
...
- **Todero-managed skills** — bundled with the adapter, togglable from the UI
...
Todero UI can display both managed and native skills in one view.

## Development

```bash
git clone https://github.com/nabitllc/todero
cd todero/packages/adapters/hermes
pnpm install
pnpm build
```
...
- [Todero](https://github.com/nabitllc/todero) — The orchestration platform
- [Nous Research](https://nousresearch.com) — The team behind Hermes
- [Todero Docs](https://github.com/nabitllc/todero) — Todero documentation
