...
import { asString, parseObject } from "@todero/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Todero server host.",
        });
      }
    } catch (err) {
      checks.push({
