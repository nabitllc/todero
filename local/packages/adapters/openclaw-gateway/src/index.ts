...
Use when:
- You want Todero to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Todero server.
...
- toderoApiUrl (string, optional): absolute Todero base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/todero-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default todero)

Wake payload notes:
- Todero wake context is embedded into the generated message text
- No top-level todero field is sent; the gateway agent schema rejects unknown root params
