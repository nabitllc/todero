...
This document describes how `@todero/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Todero logs/transcript parsing
...
Like every Todero adapter, this one must treat the local execution-workspace
...
the OpenClaw side, use the round-trip helpers in `@todero/adapter-utils`
