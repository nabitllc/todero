import type { AdapterModelProfileDefinition } from "@todero/adapter-utils";
...
- networkScope (string, optional): "deny" blocks all network egress; "allowlist" permits only networkAllowlist targets through Todero's HTTP(S) proxy. Off by default.
...
- If instructionsFilePath is configured, Todero prepends that file's contents to the stdin prompt on every run.
- Codex exec automatically applies repo-scoped AGENTS.md instructions from the active workspace. Todero cannot suppress that discovery in exec mode, so repo AGENTS.md files may still apply even when you only configured an explicit instructionsFilePath.
- Todero injects desired local skills into the effective CODEX_HOME/skills/ directory at execution time so Codex can discover "$todero" and related skills without polluting the project working directory. For new and updated agents, Todero assigns an isolated managed home at ~/.todero/instances/<id>/companies/<companyId>/agents/<agentId>/codex-home/skills/; when CODEX_HOME is explicitly overridden in adapter config, that override is used instead.
...
- Fast mode is supported on GPT-5.6 (sol/terra/luna), GPT-5.5, GPT-5.4 and manual model IDs. When enabled for those models, Todero applies \`service_tier="fast"\` and \`features.fast_mode=true\`.
- When Todero realizes a workspace/runtime for a run, it injects PAPERCLIP_WORKSPACE_* and PAPERCLIP_RUNTIME_* env vars for agent-side tooling.
