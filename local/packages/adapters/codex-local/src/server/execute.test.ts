...
import type { SandboxManagedRuntimeAsset } from "@todero/adapter-utils/sandbox-managed-runtime";
...
const REMOTE_RUNTIME_ROOT = "/remote/workspace/.todero-runtime/codex";
...
describe("codex execute — outbound auth copy-back restore contribution", () => {
  ...
  async function runTeardown(input: {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "todero-codex-copyback-e2e-"));
