import { asBoolean, asString, asStringArray } from "@todero/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; Todero will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
