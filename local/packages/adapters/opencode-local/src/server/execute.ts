...
import { removeMaintainerOnlySkillSymlinks } from "@todero/adapter-utils/server-utils";
...
async function ensureOpenCodeSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[todero] Removed maintainer-only OpenCode skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[todero] ${result === "repaired" ? "Repaired" : "Injected"} OpenCode skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[todero] Failed to inject OpenCode skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildOpenCodeSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "todero-opencode-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const openCodeSkillEntries = await readToderoRuntimeSkillEntries(config, __moduleDir);
  const desiredOpenCodeSkillNames = resolveLegacyToderoDesiredSkillNames(config, openCodeSkillEntries);
  ...
  try {
    ...
    if (executionTarget?.kind === "remote") {
      ...
      await onLog(
        "stdout",
        `[todero] Syncing workspace and OpenCode runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
    ...
    if (executionTargetIsRemote && adapterExecutionTargetUsesToderoBridge(runtimeExecutionTarget)) {
      ...
      if (toderoBridge) {
        Object.assign(preparedRuntimeConfig.env, toderoBridge.env);
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[todero] OpenCode session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
      );
    } else if (runtimeSessionId && !canResumeSession) {
      await onLog(
        "stdout",
        `[todero] OpenCode session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
      );
    }
    ...
    try {
      ...
      if (
        ...
        await onLog(
          "stdout",
          `[todero] OpenCode session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
        );
