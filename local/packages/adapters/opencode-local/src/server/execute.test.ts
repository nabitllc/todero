...
vi.mock("@todero/adapter-utils/execution-target", async (importOriginal) => {
...
import { runAdapterExecutionTargetProcess } from "@todero/adapter-utils/execution-target";
...
describe("OpenCode local skill injection", () => {
  it("injects runtime skills into the configured child HOME", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "todero-opencode-configured-home-"));
    ...
    try {
      ...
      const installedSkill = path.join(configuredHome, ".claude", "skills", "todero");
      ...
      await expect(fs.lstat(path.join(processHome, ".claude", "skills", "todero"))).rejects.toThrow();
    } finally {
