 ...
 * and tests all agree on the dedicated Todero runtime port range and the
...
/**
 * Dedicated, allowlisted port range for managed Todero runtime APP
...
/**
 * Fixed offset between an app port and its Todero Vite HMR companion port.
 * Matches Todero dev mode's `server port + offset` HMR convention and keeps
...
export function deriveViteHmrPort(appPort: number): number {
  ...
  return deriveToderoViteHmrPort(appPort);
}

/**
 * Derive Todero's Vite HMR companion port for any valid application port.
 *
 * Normal Todero instances can use ports outside the dedicated exposure
 ...
 */
export function deriveToderoViteHmrPort(serverPort: number): number {
