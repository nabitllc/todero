/**
 * Scheme/transport safety for OAuth endpoints Todero learned from a remote
 * server (PAP-17099).
 *
 ...
 * `authorization_endpoint` it names is not just fetched server-side — Todero
 ...
 *   reads as the wrong origin to a human and Todero must not help;
...
export interface OAuthEndpointUrlOptions {
   ...
   * in practice just Todero's own deployment origin. A deployment served over
   * plaintext HTTP on a LAN address can still run its own authorization
   * endpoints (the smoke-lab fixture does), and opening Todero from Todero
...
/**
 * Is `value` an OAuth endpoint Todero is willing to use — and, for the
...
export function oauthEndpointUrlRejectionMessage(
  ...
  switch (reason) {
    ...
    case "malformed":
      return `This server's ${label} address is not a valid URL, so Todero stopped.`;
    case "unsupported_scheme":
      return `This server's ${label} address does not use https, so Todero stopped.`;
    case "insecure_transport":
      return `This server's ${label} address is not secure (https), so Todero stopped.`;
    case "embedded_credentials":
      return `This server's ${label} address hides a different site behind a username, so Todero stopped.`;
    case "fragment":
      return `This server's ${label} address is malformed for sign-in, so Todero stopped.`;
  }
}
