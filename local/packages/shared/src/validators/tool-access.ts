...
export const connectionGrantSchema = z.object({
  ...
  if (grant.externalCredential && grant.credentialSecretRefs.length > 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["credentialSecretRefs"], message: "External grants cannot also contain Todero secret references" });
  }
});
 ...
 *
 * `auto` is the default and the only value the simple path sends: Todero
 ...
 * it becomes a Todero secret ref and is never read back.
 */
...
/**
 * Reject `headers.*` credential paths whose header name Todero refuses to
...
export const toolPolicyConditionsSchema = z.object({
  ...
  trustBoundary: z.object({
    ...
    toderoSelfOnly: z.boolean().optional(),
  }).strict().optional(),
