...
usage() {
  cat <<'EOF'
Usage:
  scripts/todero-issue-update.sh [--issue-id ID] [--status STATUS] [--comment TEXT] [--dry-run]
...
Examples:
  scripts/todero-issue-update.sh --issue-id "$PAPERCLIP_TASK_ID" --status in_progress <<'MD'
  ...
  scripts/todero-issue-update.sh --issue-id "$PAPERCLIP_TASK_ID" --status done --dry-run <<'MD'
  Done
