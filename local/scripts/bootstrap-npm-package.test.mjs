...
test("parseArgs recognizes the publish flag", () => {
  assert.deepEqual(parseArgs(["@todero/adapter-kimi-local", "--publish"]), {
    help: false,
    packageName: "@todero/adapter-kimi-local",
    publish: true,
  });
});

test("parseArgs defaults to a dry run", () => {
  assert.deepEqual(parseArgs(["@todero/adapter-kimi-local"]), {
    help: false,
    packageName: "@todero/adapter-kimi-local",
    publish: false,
  });
...
test("parseArgs rejects a second package name", () => {
  assert.throws(() => parseArgs(["@todero/a", "@todero/b"]), /exactly one package name/);
});

test("parseArgs rejects unknown options", () => {
  assert.throws(() => parseArgs(["@todero/a", "--skip-build"]), /unknown option/);
  assert.throws(() => parseArgs(["@todero/a", "--otp", "123456"]), /unknown option/);
});

test("validatePackageName accepts @todero scoped names", () => {
  validatePackageName("@todero/adapter-kimi-local");
  validatePackageName("@todero/plugin-workspace-diff");
});

test("validatePackageName rejects names outside the @todero scope", () => {
  assert.throws(() => validatePackageName("left-pad"), /@todero scope/);
  assert.throws(() => validatePackageName("@evil/adapter-kimi-local"), /@todero scope/);
  assert.throws(() => validatePackageName("@todero/UPPER"), /@todero scope/);
});

test("buildPlaceholderFiles produces a publishable manifest at the placeholder version", () => {
  const files = buildPlaceholderFiles("@todero/adapter-kimi-local");
  const manifest = JSON.parse(files["package.json"]);

  assert.equal(manifest.name, "@todero/adapter-kimi-local");
...
test("buildPlaceholderFiles entry point throws with a pointer to the repo", () => {
  ...
  assert.match(files["index.js"], /github\.com\/todero\/todero/);
...
test("waitForPackageVisible requires consecutive sightings before reporting success", async () => {
  ...
  const visible = await waitForPackageVisible("@todero/x", {
    attempts: 10,
    consecutive: 2,
