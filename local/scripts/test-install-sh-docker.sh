...
RESULTS_DIR="$(mktemp -d "${TMPDIR:-/tmp}/todero-install-sh.XXXXXX")"
...
run_with_node() {
  ...
  shift
  docker run --rm \
    -v "$REPO_ROOT/scripts:/todero-scripts:ro" \
    -v "$RESULTS_DIR:/results" \
    -e "PAPERCLIP_INSTALL_TEST_LOG=/results/$name.args" \
    -e PATH="/todero-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
...
echo "==> existing Node"
run_with_node with-node bash /todero-scripts/install.sh --no-prompt --no-onboard
assert_line "$RESULTS_DIR/with-node.args" "todero@latest"
...
docker run --rm \
  ...
  -e PATH="/todero-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  node:24-bookworm-slim \
  bash /todero-scripts/install.sh --no-prompt --no-onboard
...
echo "==> --ref master"
if run_with_node ref-master bash /todero-scripts/install.sh --ref master --no-onboard; then
...
if run_with_node piped-rejected bash -c 'cat /todero-scripts/install.sh | bash -s -- --no-onboard'; then
  echo "Expected piped install without --no-prompt to fail" >&2
  exit 1
fi

echo "==> piped --no-prompt"
run_with_node piped bash -c 'cat /todero-scripts/install.sh | bash -s -- --no-prompt --no-onboard'
...
if docker run --rm \
  ...
  ubuntu:24.04 \
  bash -c 'cat /todero-scripts/install.sh | bash -s -- --no-prompt --no-onboard' \
  >"$RESULTS_DIR/piped-no-node.out" 2>&1; then
  echo "Expected piped install without Node.js to fail before privileged bootstrap" >&2
  exit 1
fi
assert_line "$RESULTS_DIR/piped-no-node.out" "[todero] error: Node.js bootstrap is disabled for piped installs; download install.sh, review it, and run 'bash install.sh --no-prompt'"

echo "==> dry run"
run_with_node dry-run bash /todero-scripts/install.sh --no-prompt --dry-run --no-onboard
...
docker run --rm \
  ...
  -e PATH="/todero-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  node:24-bookworm-slim \
  bash /todero-scripts/install.sh
assert_line "$RESULTS_DIR/env.args" "todero@2026.722.0"
...
docker run --rm \
  ...
  -e PATH="/todero-scripts/install-sh-fixtures:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  ubuntu:24.04 \
  bash -c 'apt-get update >/dev/null && apt-get install -y ca-certificates curl >/dev/null && bash /todero-scripts/install.sh --no-prompt --no-onboard'
assert_line "$RESULTS_DIR/no-node.args" "todero@latest"
