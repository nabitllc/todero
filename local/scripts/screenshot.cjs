#!/usr/bin/env node
/**
 * Screenshot utility for Todero UI.
 *
 * Reads the board token from ~/.todero/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/todero-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".todero/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.todero/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the Todero origin only
