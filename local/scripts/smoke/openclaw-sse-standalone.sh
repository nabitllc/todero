...
OPENCLAW_USER="${OPENCLAW_USER:-todero-smoke}"
...
Run your Todero heartbeat procedure now.
EOF
...
PAYLOAD="$(jq -nc \
  ...
  '{
    ...
    metadata: {
      ...
      paperclip_session_key: ("todero:run:" + $runId)
    }
  }')"
...
args=(
  ...
  -H "x-openclaw-session-key: todero:run:${PAPERCLIP_RUN_ID}"
  -D "$headers_file"
  -o "$body_file"
  --data "$PAYLOAD"
  "$OPENCLAW_URL"
