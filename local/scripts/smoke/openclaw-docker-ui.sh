...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.todero-openclaw.override.yml"
...
Todero URL for OpenClaw container:
EOF
...
EOF
  ...
  If Todero rejects the host, run on host machine:
    pnpm todero allowed-hostname host.docker.internal
  Then restart Todero and re-run this script.
EOF
  fi
  cat <<EOF
