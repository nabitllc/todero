...
const CHANNEL_ENTRYPOINT_PACKAGE = "todero";
...
// Sections whose @todero workspace deps are rewritten to the calver release
...
// release, and every @todero/* workspace: dep it declares is rewritten to that
  ...
  for (const pkg of packages) {
    ...
    for (const section of RESOLVED_DEP_SECTIONS) {
      ...
      for (const [depName, spec] of Object.entries(deps)) {
        if (!depName.startsWith("@todero/")) continue;
...
function replaceWorkspaceDeps(deps, version) {
  ...
  for (const [name, value] of Object.entries(next)) {
    if (!name.startsWith("@todero/")) continue;
