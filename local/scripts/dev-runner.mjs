...
if (tailscaleAuth) {
  ...
  env.HOST = "0.0.0.0";
  console.log("[todero] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@todero/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
...
async function maybePreflightMigrations(options = {}) {
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[todero] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
