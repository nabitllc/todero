...
test("release package list publishes the installable channel entrypoint last", () => {
  ...
  assert.equal(enabledPackages.at(-1)?.name, "todero");
  assert.ok(enabledPackages.slice(0, -1).some((pkg) => pkg.name === "@todero/server"));
});
...
test("Hermes release surface publishes the unified built-in package and keeps gateway as a shim", () => {
  const packages = buildReleasePackagePlan();
  const hermes = packages.find((pkg) => pkg.name === "@todero/hermes-paperclip-adapter");
  const gatewayShim = packages.find((pkg) => pkg.name === "@todero/adapter-hermes-gateway");
...
test("guard flags a publishFromCi:true package depending on a publishFromCi:false package", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@todero/server", {
      publishFromCi: true,
      dependencies: { "@todero/skills-catalog": "workspace:*" },
    }),
    pkg("@todero/skills-catalog", { publishFromCi: false }),
  ]);

  assert.equal(problems.length, 1);
  assert.match(problems[0], /@todero\/server/);
  assert.match(problems[0], /@todero\/skills-catalog/);
});
...
test("guard inspects optional and peer dependency sections too", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@todero/server", {
      publishFromCi: true,
      optionalDependencies: { "@todero/opt": "workspace:^" },
      peerDependencies: { "@todero/peer": "workspace:*" },
    }),
    pkg("@todero/opt", { publishFromCi: false }),
    pkg("@todero/peer", { publishFromCi: false }),
  ]);

  assert.equal(problems.length, 2);
});

test("guard treats a workspace dep on an unknown @todero package as unpublishable", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@todero/server", {
      publishFromCi: true,
      dependencies: { "@todero/private-internal": "workspace:*" },
    }),
  ]);
...
test("guard allows true->true workspace edges", () => {
  const problems = findUnpublishableWorkspaceEdges([
    pkg("@todero/server", {
      publishFromCi: true,
      dependencies: { "@todero/shared": "workspace:*" },
    }),
    pkg("@todero/shared", { publishFromCi: true }),
  ]);
...
test("guard ignores non-workspace specs, non-internal deps, and edges from off-train packages", () => {
  const problems = findUnpublishableWorkspaceEdges([
    ...
    pkg("@todero/pinned", { publishFromCi: false }),
    pkg("@todero/offtrain", {
      publishFromCi: false,
      dependencies: { "@todero/also-off": "workspace:*" },
    }),
    pkg("@todero/also-off", { publishFromCi: false }),
  ]);
