...
This spec defines how Todero actually runs agents while staying runtime-agnostic.
...
1. Todero is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Todero manages lifecycle, and on exit Todero parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `todero.skill` (shared Todero skill text block)
