...
Todero's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Todero's API, UI, and data model | Yes |
...
modules/
  observability/
    todero.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@todero/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@todero/core";
...
```
1. Scan modules/ directory for todero.module.json manifests
...
```jsonc
// todero.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@todero/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@todero/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:todero/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.todero.vercel.app/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm todero store list                    # browse available modules and templates
pnpm todero store install <module-id>     # install a module
pnpm todero store import <template-id>    # import a company template
pnpm todero store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Todero instance | `server:started` |
...
4. **Module config** — `todero.config.json` with per-module config, env var interpolation
...
10. **`@todero/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
