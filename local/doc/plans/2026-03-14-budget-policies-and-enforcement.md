...
Todero already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Todero should implement next.

## Product Goals

Todero should let operators:
...
For the next implementation pass, Todero should enforce these defaults:
...
Todero should separate subscription-included usage from billed spend:
...
Todero should have two threshold classes:
...
Todero must know that a project is budget-paused and enforce it.
...
Todero must also block execution before new work starts.
