...
- A Todero MCP server package that exposes Todero REST operations outward as MCP tools.
...
1. MVP clients are Todero agents only. External human AI clients are out of scope.
...
4. Todero is the enforcement gateway. Agents connect to Todero, not directly to upstream MCP credentials or ungoverned upstream configs.
5. The gateway governs Todero plugin tools, Todero self-MCP tools, and upstream MCP tools through one policy/audit/invocation model.
...
9. First examples are synthetic fixtures, then Todero self-read, GitHub triage, and approval-gated outbox/social examples.
...
Server-evaluated rule that can allow, block, require approval, redact, rate-limit, or defer because runtime capacity is unavailable. Policies are evaluated from authoritative Todero state, not model-provided context.

`Gateway Session`

A short-lived per-run session minted by Todero. It exposes only allowed tools for that run and prevents upstream credentials from reaching the agent process.
...
At heartbeat invocation time, Todero resolves the authoritative agent, issue, project, routine, workspace, adapter, and run records. It then creates a gateway session tied to that run.

The adapter receives only Todero gateway configuration:

- gateway base URL;
- short-lived session token;
- optional generated MCP config pointing at Todero;
...
- Remote HTTP MCP is preferred. It uses no local process. Todero proxies requests with timeouts, retries where safe, auth, and audit.
...
- Todero self-read: list issues, get issue context, read plan document, list recent runs.
- Todero self-governed write: add comment draft, create child issue draft, update plan draft.
...
- mint/read run-bound gateway session where invoked by Todero runtime;
...
- `packages/mcp-server/README.md` for the current Todero MCP server boundary.
