...
Date: 2026-05-26
Source plan: approved Todero skills CLI and catalog plan

This document freezes the first implementation contract for the `todero skills`
command group and the app-shipped skills catalog. It is intentionally a build
contract, not a full product spec.

## Decisions

- `todero skills` manages Todero company skills. It does not manage
...
- Root `skills/` remains reserved for Todero runtime and operational skills.
...
- Agent desired skills continue to live in
  `adapterConfig.toderoSkillSync.desiredSkills` for the first release. Do not
...
- Catalog skill: an app-shipped skill entry in `@todero/skills-catalog`.
...
| `skills agent sync <agent-ref> --skill <skill-ref>...` | Replaces the agent's non-required desired skill set with the supplied refs and triggers adapter sync. Required Todero skills remain enforced by the server. | `AgentSkillSnapshot` |
...
Catalog commands are for the app-shipped Todero catalog only. External GitHub,
...
Package name: `@todero/skills-catalog`.
...
- `todero skills --help` exposes the Phase B command group.
