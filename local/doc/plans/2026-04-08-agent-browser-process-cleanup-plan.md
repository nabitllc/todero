...
Yes, there is a likely root cause in Todero's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Todero can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Todero already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Todero may think it cancelled the run because the parent process is gone
...
This work is successful when Todero can:
...
- require every skill author to add bespoke browser teardown logic before Todero can clean up correctly
...
- make the leak measurable from Todero's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Todero
...
Treat this as a Todero executor ownership bug, not an `agent-browser` bug.
