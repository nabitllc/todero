...
Add a single storage subsystem for Todero that supports:
...
- API serves attachment bytes through authenticated Todero endpoints.
...
- Default local path is under instance root: `~/.todero/instances/<instanceId>/data/storage`.
...
- `todero onboard` writes a valid `storage` config block by default.
- `todero configure --section storage` can switch between local and s3 modes.
...
- `todero doctor` reports actionable storage status.
