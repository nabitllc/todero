...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Todero.
...
- current Todero plugin and adapter surfaces in this repo
...
## What Todero already has

Todero already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Todero

These ideas from `pi-mono` fit Todero with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Todero should continue exposing run and transcript events to plugins, for example:
...
This matches Todero's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Todero already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Todero already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Todero, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Todero core:

### 1. Arbitrary mutation hooks on control-plane decisions

Todero should not let general plugins rewrite:
...
Todero should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Todero is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Todero's board-and-issues model.

## Recommended Todero direction
...
If the question is "what works for Todero too?":
