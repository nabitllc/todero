...
Agents need API keys to authenticate with Todero. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Todero
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Todero generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Todero CLI
...
```
todero auth login
# Opens browser -> user confirms -> token stored at ~/.todero/credentials
```
...
managed by the Todero server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Todero.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Todero SKILL.md (or a link to it)
   - What information Todero needs from the agent (e.g. webhook URL, adapter
...
5. Todero stores the pending registration.
...
7. On approval, Todero provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Todero issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.todero.vercel.app/skills/todero/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Todero should send you heartbeats",
...
1. Generate a Todero invite link for the company.
...
4. A Todero company member approves the new agent.
5. Todero begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `todero auth login` for developer-managed remote agents.                                      |
