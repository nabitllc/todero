...
`PAP-447` asks how Todero should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Todero instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Todero:

- some operators use Todero to manage Todero and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Todero
- Claude and Codex expose different built-in affordances, so Todero should not overfit to one tool

## Core Product Decision

Todero should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Todero
...
- Todero should not assume that every adapter can see or use a host filesystem path directly
...
If Todero makes worktrees agent-first, it will blur:
...
- Todero core resolves and records execution workspace state
...
That gives Todero room to support:
...
- Todero handles workspace runtime services under the hood using project/workspace policy
...
Once Todero is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Todero cannot infer
...
- when issue moves to done, should Todero auto-commit?
- should it auto-push?
...
Todero already has the right foundation for a project-first model.
...
For server-spawned local adapters, Todero already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Todero should mirror directly

Implication:

- for `codex_local`, Todero should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Todero should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Todero should separate:
...
Todero should be able to express intentions such as:
...
These are not specific to Todero. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".todero/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Todero-specific `devServer` flag.
...
Todero should distinguish between:
...
Todero should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Todero should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Todero
...
For runtime services specifically, Todero should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Todero-only dev-server flags
...
3. Inject resolved execution workspace details into `context.toderoWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Todero instance can be reused through env/config injection when appropriate
...
- Todero can decide whether to reuse or start a fresh service deterministically
...
3. Test against Todero-on-Todero workflows first.
...
7. Workspace runtime services are modeled generically, not as Todero-specific dev-server toggles.
