...
This document defines the rollout plan for adapter-wide skill support in Todero.
...
- Todero stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Todero currently has these adapters:
...
1. Desired skills live in Todero for every adapter.
...
These adapters have a stable local skills directory that Todero can read and manage.
...
These adapters do not have a meaningful Todero-owned persistent install state.
...
- desired skills are stored in Todero
...
- desired skills stored in Todero
...
- Todero reconciles desired skills into `~/.cursor/skills`
...
- Todero reconciles desired skills into `~/.gemini/skills`
...
- Todero reconciles desired skills into `~/.pi/agent/skills`
...
- Todero can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Todero skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Todero may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Todero skills that are clearly marked as Todero-managed
...
- Todero stores desired skills only
...
- avoid destructive wording unless Todero can prove a skill is Todero-managed
