# Agent OS Technical Report for Todero
...
`agent-os` is not a competitor to Todero's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. Todero is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that Todero could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in Todero.

My recommendation is:

1. Do not merge agent-os concepts into the Todero core product model.
...
4. Do not import its workflow/cron/queue abstractions into Todero core until they are reconciled with Todero's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than Todero's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to Todero because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## Todero anchor points

The most relevant current Todero surfaces for any future `agent-os` integration are:
...
## What Todero can learn from it

### 1. A safer local execution substrate

Todero's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits Todero because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

Todero already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
Todero has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that Todero could adopt even without adopting the runtime itself:
...
Todero's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.

Concrete Todero uses:
...
This aligns well with Todero's governance story.

### 5. Better adapter normalization around sessions and capabilities

Todero's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
Todero does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
Todero can adopt that philosophy directly:
...
## What does not fit Todero well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. Todero already has higher-level orchestration concepts:
...
If Todero copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

Todero should keep orchestration authoritative at the control-plane layer.
...
Those are Todero's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. Todero would still need to answer:
...
- keep Todero heartbeat/task/workspace/budget logic unchanged
...
- Todero's Codex adapter is already important and carries repo-specific behavior
...
- Todero still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, Todero should consider standardizing adapter/runtime permissions around a vocabulary like:
...
Todero should evaluate whether some execution workspaces can be backed by:
...
Todero plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into Todero core

Todero should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving Todero orchestration into agent-os workflows
...
`agent-os` is useful to Todero as an execution technology reference, not as a product model.

Todero should treat it the same way it treats sandboxes or agent CLIs:
