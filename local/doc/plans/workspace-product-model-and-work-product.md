...
Todero needs to support two very different but equally valid ways of working:
...
Today, Todero already has the beginnings of this model:
...
- making Todero feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Todero users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Todero deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Todero into a full code review product
...
This object must be recorded explicitly so that Todero can:
...
Todero should treat PRs as a type of work product linked back to:
...
Todero already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Todero must be a first-class deployment mode

Todero cannot assume that it is running on the same machine as the code.

In cloud deployments, Todero may:
...
- `Runtime service`: a process or service Todero owns or tracks for a workspace
...
It also lets cloud-hosted Todero deployments point at codebases and remotes without pretending the Todero host has direct filesystem access.
...
Without an explicit `execution workspace` record, Todero has nowhere stable to attach:
...
Todero needs a single place to show:
...
- Todero-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Todero owning the execution host
...
## 1. Todero host and execution host must be decoupled
...
without the Todero host itself running local git or local preview processes.
...
Todero should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Todero.
...
Todero should keep workspace automation centered on a small set of hooks:
...
Todero should provide the equivalent visibility in the project `Code` surface:
...
Todero should treat execution workspaces as the natural home for:
...
Todero should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Todero should continue treating cleanup policy as part of the core workspace model:
...
Todero should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Todero host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Todero should borrow Worktrunk's good execution patterns while keeping the broader Todero model:
...
That keeps Todero focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Todero deployments remain first-class

That is a better fit for Todero than either extreme:
