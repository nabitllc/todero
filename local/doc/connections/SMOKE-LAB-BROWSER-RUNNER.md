...
**The shared dev worktree service works as-is now.** The Todero-managed dev
service (`pnpm dev`, `PAPERCLIP_DEPLOYMENT_MODE=authenticated` + `NODE_ENV=production`,
e.g. `http://todero-dev:45439`) is private, so it can run the Smoke Lab directly —
...
pnpm todero onboard --yes --run    # serves http://127.0.0.1:3211, own embedded PG
...
(`.todero/browser-runtime/chromium-arm64/bin/chromium-agent-browser`) — see
