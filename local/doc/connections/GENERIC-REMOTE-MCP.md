...
Todero can connect a standards-compliant remote HTTP MCP server without a
Todero code change. A curated `AppDefinition` is a **convenience layer** —
...
convenience layer on top for a vendor Todero should promote.
...
| --- | --- | --- |
| Guided URL | **Apps → Connect an app → Connect your own MCP server** | You have the server's address. Todero probes it and walks you through whatever it needs. |
| Paste a config | **Advanced → Paste a config** | A README gave you an `mcpServers` snippet, or the server needs headers with names Todero could not guess. |
...
on how to obtain each credential. Paste only the JSON block back into Todero;
Todero reads the header names from it and asks you for the values, which it
stores as Todero secrets.

## What the guided URL flow does

After you paste an address and press **Check link**, Todero probes the
...
Every value you enter becomes a Todero secret. Values are write-only: they
...
Todero refuses to send header names it manages or that belong to the
...
You never choose this; Todero resolves it and the wizard shows none of it.
...
2. **Client ID Metadata Document (CIMD).** When the authorization server
   advertises `client_id_metadata_document_supported`, Todero presents the URL
   ...
   methods Todero uses — no company, connection or secret data.
3. **Dynamic client registration (RFC 7591).** When the authorization server
   advertises a `registration_endpoint`. Todero registers a public client
...
- a Todero-minted client (CIMD or DCR) is **re-registered**;
- a client you supplied yourself is **not** — Todero stops and asks you to
...
(`checkOAuthEndpointUrl` in `@todero/shared`) and must be:
...
- **Free of embedded credentials.** `https://accounts.google.com@evil.test/…`
  reads as the wrong site to a human, so Todero refuses it.
...
already-running, browser-reachable HTTPS Todero instance with these bindings
...
Todero login succeed. It disables trace, video, and HAR capture, takes only
