...
Post-read action: classify a new integration request, pick the right Todero
...
- **D2: one credential authority per connection, brokered projections.** The
  default is the Todero instance vault: durable third-party credentials live
  ...
  durable credential authority and Todero stores only the connector reference
...
- **D5: inbound stays thin.** External clients that call Todero use scoped
  Todero tokens and existing profiles/rules. They do not get a separate
  permission model.
...
Todero run/session token; the server or broker resolves the connection,
...
| --- | --- | --- | --- |
| **P1. Sign-in methods** | *Who are you?* | `todero-id` (id.todero.vercel.app → Account) | Minimal-scope provider tokens (`openid email profile`), used once to authenticate, encrypted at rest, never exported |
| **P2. Connections (Apps)** | *What may your agents touch?* | Todero App instances (`tool_connections`), acquired via the **connect broker** for hosted + self-hosted | Rich-scope, long-lived resource tokens in the **instance's** encrypted vault; per-agent grants; risk-tier policy defaults |
| **P3. Login with Todero** | *Who may authenticate against us?* | `todero-id` OIDC provider + DB-backed client registry | Our ES256 ID/access tokens issued *by* us to registered RPs (instances, the broker, future third parties) |
...
> Sign-in tokens are never reused as resource tokens; id.todero.vercel.app never
> stores resource tokens; no connections hub on the ID service.

P2 tokens flow broker → instance vault as pass-through only; the id.todero.vercel.app
...
- **Blast radius.** id.todero.vercel.app holding every customer's Vercel/Slack/GitHub
...
connection. `id.todero.vercel.app` is not involved, and neither sign-in tokens nor
...
| --- | --- | --- |
| Todero App instances | P2 | **"Connections"** |
| id.todero.vercel.app Account | P1 | **"Ways to sign in"** |
| id.todero.vercel.app admin | P3 | **"OIDC clients"** (until the app store productizes it) |
...
- [Connecting any remote MCP server](./GENERIC-REMOTE-MCP.md) is the baseline:
  ...
  Todero code change, and how sign-in resolves a client.
