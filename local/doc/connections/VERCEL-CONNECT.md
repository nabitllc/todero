...
Audience: Todero operators and engineers enabling Vercel Connect for outbound
remote MCP apps.

Vercel Connect is an optional credential authority inside Apps v2. Operators
create, attach, authorize, and inspect connectors in Vercel. Todero records
...
not enter Todero secrets, connection config, agent config, prompts, API
...
> **Preview UI paused:** Todero currently withholds Vercel Connect from Apps
...
Todero pins `@vercel/connect` to `0.6.1`.
...
it outside Todero, and never put it in `company_secrets`, app config, task
...
rollout. Todero prefers workload OIDC whenever both authorities are present,
...
normal SDK cache path rather than persisted by Todero.
...
1. Open [Vercel Connect](https://vercel.com/connect), create the provider
   ...
   Todero. Vercel remains the inventory and management UI.
2. In a future reviewed build, Todero will expose one isolated Vercel Connect
...
3. Choose the reviewed app, open the Vercel handoff if needed, and paste only
   the connector UID (for example `notion/todero`) or connector ID (`scl_…`).
4. Todero calls `getConnectorMetadata` and rejects missing, unattached, or
...
5. For a user OAuth connector, continue through Vercel authorization. The
   ...
   app/API-key connectors, Todero verifies setup with a token request. If an
   installation is required, complete it in Vercel and retry; Todero does not
   use the experimental installation API.
6. Review the discovered catalog and complete the existing Todero access,
   resource-filter, risk, and agent-install steps.

V1 requires one dedicated app-subject connector per Todero connection. Do
not reuse an app-subject connector across Todero companies. Multi-installation
routing and shared app-subject connectors are intentionally deferred.

## Runtime and subject binding

Todero selects the effective connection grant before requesting a token. It
derives user-subject IDs from the Todero instance, company, connection, grant
kind, and responsible user. Organization OAuth uses a company-scoped
pseudonymous user subject; personal OAuth adds the Todero user. App/API-key
methods use Vercel's `app` subject. Neither browsers nor agents may supply these
subject identifiers.

Token resolution uses the SDK cache plus Todero single-flight deduplication.
...
`none` and PKCE `S256`; Todero sends the canonical resource plus reviewed
...
placement. Todero injects the bearer only after the connection grant,
...
Stable Todero reason codes include:

| Code | Operator action |
| --- | --- |
| `vercel_connect_unavailable` | Restore workload OIDC or the BYO access token. |
| `vercel_connect_auth_failed` | Repair or refresh Todero's Vercel authority, and verify that the configured token type is accepted by Connect. |
...
Todero creates the existing authorization interaction. Reconnect UI sends
...
external cleanup. Todero revokes subjects it generated for user-mode
...
synchronization, Todero-hosted relay services, and Vercel's experimental
...
instead of copying rates into Todero documentation.
