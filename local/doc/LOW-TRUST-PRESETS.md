...
Todero ships core trust preset names so containment decisions are enforced in
...
scope for risky access, Todero fails closed.
...
agent can read or mutate through the Todero API and prevents raw untrusted
...
Managed `low_trust_review` runs fail closed unless Todero can enforce the
runtime boundary:
...
local review, but Todero-managed low-trust execution requires a sandboxed
