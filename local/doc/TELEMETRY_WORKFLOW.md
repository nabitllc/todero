...
Todero first-party telemetry is schema-led for stable events and proposal-led for new product instrumentation.
...
A proposed event is a normal `client.track()` call whose event name is not yet in `ToderoEventName`. The runtime client swallows unregistered first-party event names before queueing, state initialization, or network flush, so proposed events do not leave the process until the generated telemetry schema adopts the event name.
...
export function trackYourFeatureActionPerformed(
  ...
  client.track(
    // @ts-expect-error -- proposed-telemetry(https://github.com/nabitllc/todero/issues/123): measure feature action completion
...
`<issue>` should be a public `https://github.com/nabitllc/todero/issues/123` URL. The rationale should be a short product reason for collecting the event. Missing issue or rationale text is tolerated at the call site and flagged by `scripts/extract-proposed-events.mjs`; it is not an OSS CI failure.
...
When a proposed event is approved and registered in the telemetry backend, sync the regenerated telemetry artifact into the OSS repo. The event name is then part of `ToderoEventName`, so the proposal marker should fail with TS2578. Remove the marker and keep the wrapper payload aligned with the registered dimensions in the same change.
