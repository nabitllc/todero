...
Maintainer runbook for shipping Todero across npm, GitHub, and the website-facing changelog surface.
...
Todero uses calendar versions that still fit semver syntax:
...
2. **npm** — `todero` and public workspace packages are published
...
- publishes the user-facing `todero` package last, so `todero@canary` does not advance before the full package set exists
- verifies that `canary` resolves to the just-published version and that published internal dependencies exist on npm
- installs `todero@canary` into a clean temporary prefix as the final npm gate
...
# or
npx todero@canary onboard --data-dir "$(mktemp -d /tmp/todero-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/nabitllc/todero/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Todero. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
TODEROAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
TODEROAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary TODEROAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable TODEROAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
...
- `npx todero@canary onboard` installs
