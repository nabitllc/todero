...
`pnpm test:hermes-gateway-smoke` does not start Docker, Hermes, or Todero. It
...
- Diagnostic files are redacted before they are written, except the join output
  file intentionally contains the claimed Todero agent key and is written
  `chmod 600`.
...
- `PAPERCLIP_API_URL`: Todero URL used by the operator shell.
- `PAPERCLIP_API_URL_FOR_HERMES`: Todero URL used from inside the Hermes
  container or remote Hermes host.
- `HERMES_GATEWAY_API_BASE_URL`: Hermes gateway URL stored on the Todero
  adapter, reachable by the Todero server.
...
## Docker Desktop or Linux host Todero

Use this when Todero runs on the host at `127.0.0.1:3100` and Docker can
...
## Same Docker network as Todero

Use this when Todero is a container on a Docker network and the Hermes smoke
...
PAPERCLIP_API_URL=http://127.0.0.1:3100 \
PAPERCLIP_AUTH_HEADER='Bearer <board-token>' \
HERMES_CONTAINER_NAME=todero-hermes-gateway-smoke \
HERMES_SMOKE_NETWORK=paperclip_default \
HERMES_DOCKER_ADD_HOST=0 \
HERMES_GATEWAY_API_BASE_URL=http://todero-hermes-gateway-smoke:8642 \
HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642 \
PAPERCLIP_API_URL_FOR_HERMES=http://todero:3100 \
HERMES_GATEWAY_ALLOW_INSECURE_HTTP=1 \
pnpm smoke:hermes-gateway-e2e
```

Change `paperclip_default` and `todero` to your Compose network and service
name. The unsafe HTTP flag is required because Todero stores a non-loopback
`http://` gateway URL for private Docker DNS.

## LAN or private-network Todero

Use this when Todero is exposed on a private IP or tailnet address and the
...
Use this when Todero should talk to Hermes through a TLS hostname. The smoke
container still publishes a local port, and your reverse proxy forwards the TLS
hostname to that port.

```sh
PAPERCLIP_API_URL=https://todero.example.com \
PAPERCLIP_AUTH_HEADER='Bearer <board-token>' \
PAPERCLIP_API_URL_FOR_HERMES=https://todero.example.com \
...
`HERMES_GATEWAY_API_BASE_URL` to the URL Todero should store on the adapter.
