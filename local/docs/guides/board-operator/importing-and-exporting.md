...
Todero companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .todero.yaml     # Adapter config, env inputs, routines
```
...
- **.todero.yaml** holds Todero-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
todero company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
todero company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
todero company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.todero.yaml`
...
```sh
# From a local folder
todero company import ./my-export

# From a GitHub URL
todero company import https://github.com/org/repo

# From a GitHub subfolder
todero company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
todero company import org/repo
todero company import org/repo/companies/acme
```
...
If `--target` is not specified, Todero infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
todero company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
todero company import org/company-templates/engineering-team \
  --target new \
...
```sh
todero company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
todero company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
todero company import ./package \
  --target new \
  --yes \
  --json
```
...
Todero supports several GitHub URL formats:
