# ClipHub: Marketplace for Todero Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Todero configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Todero-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Todero company (agents, projects, governance) |
...
A complete Todero company configuration:
...
Single-agent configurations designed to plug into a Todero org:
...
Portable skill files that any Todero agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Todero's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Todero version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Todero company
...
| `POST` | `/api/listings/:id/install` | Install to Todero company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Todero company (or create new)
  → ClipHub API calls Todero API to:
  ...
  → Redirect to Todero dashboard with new team running
```
...
### 5.3 Creator: Export from Todero → Publish

```
Running Todero company → "Export as Blueprint" (CLI or UI)
  → Todero exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Todero brand blue/purple)
...
| Install flow | One-click deploy to Todero company |
...
{
  "targetCompanyId": "uuid",         // Existing Todero company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `todero-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Todero
- **Backend**: Node.js API (or extend Todero server)
- **Database**: Postgres (can share Todero's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Todero auth (or OAuth2)

### 9.2 Integration with Todero

ClipHub can be:
- **Option A**: A separate app that calls Todero's API to install blueprints
- **Option B**: A built-in section of the Todero UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Todero UI and API.
...
- [ ] Install flow: blueprint → Todero company
...
- [ ] "Export from Todero" CLI command
