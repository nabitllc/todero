# Todero External Task Protocol Specification

Status: Draft v1 (provider-agnostic)

Purpose: Define the protocol by which Todero interoperates with external task managers (Linear,
Jira, Asana, Notion, Trello, GitHub Issues, and others) so that external tasks can drive Todero
agent work and Todero work is visible in the external tool, without requiring an external user
account for every Todero agent.
...
- `Provider` — an external task manager product (Linear, Jira, Asana, Notion, Trello, GitHub
  Issues, ...).
- `Connector` — an implementation of this protocol for one provider, hosted in the Todero
  plugin runtime.
- `External task` — a task record that lives in the provider.
- `Todero issue` — the native Todero task record.
- `Host` — the Todero control plane: companies, agents, issues, checkout, heartbeat runs,
  execution workspaces, budgets, approvals, blockers, issue documents, and work products.

## 1. Problem Statement

Teams already run their work in external task managers. Todero runs companies of agents. Today
these worlds only meet through manual copy/paste: a human reads a Linear issue, rewrites it as a
Todero issue, and later rewrites the outcome back into Linear.

This protocol defines the contract that lets an external task manager act as a **task source and
collaboration surface** for a Todero company, while Todero remains the **execution and
...
- It defines how an external task becomes (or links to) exactly one Todero issue, with
  idempotent sync state that survives retries, webhook replays, and restarts.
- It defines how external assignment routes to Todero agents without creating a provider user
  per agent.
...
- Let an external task manager act as a task source: active external tasks create or wake
  Todero issues under configured policy.
...
- Route external assignment intent to Todero agents through deterministic, configurable rules.
- Reuse Todero's existing execution semantics — checkout as claim, heartbeat runs, execution
...
- Replacing the Todero board with a clone of any provider's UI.
- Real-time field-level collaborative editing between Todero and a provider.
...
- Creating a provider user account per Todero agent (supported as an OPTIONAL mapping, never
  required).
- Letting external systems bypass Todero governance: checkout, budgets, approvals, blockers,
...
2. `Task Link Store`
   - Persists the 1:1 association between an external task and a Todero issue.
...
3. `Inbound Sync Engine`
   ...
   - Creates/updates Todero issues and imports comments with provenance.
   - Runs scheduled reconciliation to repair drift.

4. `Routing Engine`
   - Resolves external assignment intent into a Todero assignee using the ordered rule
...
2. `Layer 2 — Task Link` (REQUIRED)
   - Durable external-task ↔ Todero-issue association with sync state.

3. `Layer 3 — Execution Orchestration, Symphony Profile` (RECOMMENDED)
   - Active external states drive Todero issue creation/wake; Todero checkout/run/retry
...
| Claim | Todero issue checkout + execution lock (Section 9.2) |
...
A connector MUST perform all Todero issue/comment/status mutations through the capability-gated
...
Field names use camelCase, matching Todero API conventions. (Symphony uses snake_case; the
...
The durable 1:1 association between one external task and one Todero issue.
...
- `companyId`, `projectId`, `goalId`, `issueId` (strings; `issueId` is the linked Todero issue)
- `handoffHistory` (list of `{issueId, endedAt, cause}`; empty for a link that has never moved)
  - Prior Todero issue associations retained when reopened work is handed to a follow-up.
...
- `originSide` (enum: `todero`, `external`, `manual_link`)
...
- `lastToderoFingerprint` (string or null)
  - Hash of the last Todero-side state this connector projected outbound.
...
- Default policy (RECOMMENDED): `title`/`description`/`priority`/`labels`/`dueAt` owned by
  `external` for links with `originSide=external` (and by `todero` when
  `originSide=todero`); `state` is `shared` and mediated by Section 7.4 and Section 10.3;
...
- `displayName` (string)
- `mappedUserId` (string or null) — Todero user, if mapped.
- `mappedAgentId` (string or null) — Todero agent, if explicitly mapped (OPTIONAL feature).
...
- `import_only` — external → Todero only.
- `export_only` — Todero → external only.
...
- `paperclip_controlled` — Todero governance decides when agents run; external state changes
  inform but do not command execution.
- `symphony_compatible` — active external states create/wake Todero work automatically per
...
- `Idempotency key` for outbound effects — `(linkId, effectKind, paperclipSourceId)` (e.g. the
  Todero comment ID being mirrored). Retried jobs MUST NOT double-post.
...
- Person references in body text SHOULD be converted to plain display names; raw provider mention
  syntax MUST NOT leak into Todero issue bodies where it could be misparsed as Todero
  mentions.
...
1. `Import` (`originSide=external`)
   - An external task is selected (manually, or automatically under `runMode=symphony_compatible`
     state mapping) and a Todero issue is created from its projection.
...
2. `Export` (`originSide=todero`)
   - A Todero issue is projected into the provider via `createTask`.
   - REQUIRED only for connectors declaring write capability.

3. `Manual link` (`originSide=manual_link`)
   - Operator pairs an existing external task with an existing Todero issue.
...
2. Todero backlink already present on the external task (custom field/comment marker).
3. External URL/key reference already present on a Todero issue in the same company.
...
- When an external task is deleted/archived: the connector MUST NOT delete or cancel the Todero
  issue. Default behavior: mark the link `conflict` (`statusDetail: external task deleted`),
  comment on the Todero issue, and let the assignee/operator decide. `import_only` links with
  no Todero-side activity MAY auto-unlink.
- When a Todero issue is cancelled/deleted: outbound policy decides whether to comment and/or
...
## 7. Inbound Synchronization (External → Todero)
...
- Imported comment content is **untrusted input**: it MUST be clearly attributed in the issue
  thread, MUST NOT be interpreted as Todero system/agent instructions, and mention-like syntax
  MUST NOT trigger Todero mention semantics except via the strict command path (Section 8.2).
...
### 7.4 External State Transitions Against Todero Work
...
- `active -> backlog` or `active -> blocked`: the connector SHOULD surface this on the issue; under
  ...
  Todero-native obligations (approvals in flight, blockers) are unaffected.
- `* -> terminal` while the Todero issue is active (checked out, running, in review, or carrying
...
- `terminal -> active` (reopen): if the linked issue is terminal, policy chooses between creating a
  follow-up Todero issue linked to the same external task (RECOMMENDED default) or reopening,
...
External transitions MUST NOT directly set Todero issue status; they translate into host-level
...
External assignment intent resolves to a Todero assignee through an ordered pipeline. The first
...
2. `custom_field` — provider custom field naming an agent (example: `Todero Agent = CodexCoder`).
3. `label` — namespaced label (example: `pc:agent/codexcoder`, `pc:route/qa`).
4. `state_lane` — provider state/lane mapped to a route (example: Trello list `Todero: QA`).
...
- Syntax: line-anchored `/todero <verb> [args]`. Minimum verbs: `assign <agent>`,
...
- The connector MUST reply (externally, if writes allowed; on the Todero issue otherwise) with
...
- Todero agents MUST NOT be required to exist as provider users.
- Outbound writes use one connector service account per connector instance (Section 10.4), with
  agent attribution carried in message content ("CodexCoder via Todero").
- `user_mapping` rules MAY map specific provider users to Todero users (for `assign it back to
...
Symphony's poll-tick loop becomes, in Todero:
...
- Todero issue checkout is the claim. One agent owns an issue at a time; checkout conflicts
...
1. Todero project workflow document (RECOMMENDED MVP form).
...
## 10. Outbound Synchronization and Write Policy (Todero → External)
...
| --- | --- | --- |
| `postBacklink` | on | Todero URL on the external task (field or pinned comment). |
...
| `mirrorAgentComments` | off | Full Todero comment thread mirrored externally. |
...
Todero issue status projects to provider states through the same mapping table as Section 7.2,
inverted, with these rules:

- Projection only moves the external task between states the operator mapped; unmapped Todero
...
- `done` projects to the mapped terminal state only when `fieldPolicy.state` permits
  Todero-side closure; otherwise it posts a completion comment and leaves the transition to
  external users.
...
- Outbound writes use the connector instance's service account. Message bodies MUST carry agent
  attribution and the Todero issue link.
...
- `keep_paperclip` (push Todero value outbound)
...
On the provider side, a linked task SHOULD show: the Todero backlink, current Todero
...
Destructive cascade is prohibited: external deletion/archival never deletes Todero work
(Section 6.4).
...
- Comment-command verb extensions (e.g. `/todero plan`, `/todero qa`)
...
- Containers: board, list. Envelope: list id, checklists, members, power-up metadata, card
  position.
- State mapping: lists are lanes; map lists→categories (`Todero: QA` style lanes also drive
  `state_lane` routing).
