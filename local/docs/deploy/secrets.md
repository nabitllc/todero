...
Todero encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Todero protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Todero does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Todero can no longer guarantee
...
At runtime, Todero resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Todero injects its
...
Todero stores the feature as value-free metadata plus the user's own secret
value record:
...
identity, not from Todero secrets.
...
- Keep Todero-managed AWS values under the Todero managed namespace.
  External refs under that namespace are blocked by guardrails; use the
  Todero-managed flow when Todero should create and rotate the value.

### IAM Caveats

Todero enforces company scoping, responsible-user derivation, declaration
...
For AWS Secrets Manager, the Todero runtime role needs `GetSecretValue` and
any required KMS decrypt permission for every user-scoped value it may resolve.
If you link user-specific external refs outside the Todero managed prefix,
...
refs. Todero can prevent an agent from choosing another credential owner,
but a runtime role with broad external vault read permissions can still read
what IAM allows if another code path is introduced outside Todero.
...
```
~/.todero/instances/default/secrets/master.key
```

This key is auto-created during onboarding. The key never leaves your machine.
Todero best-effort enforces `0600` permissions when it creates or loads the
key file. `todero doctor` and the provider health API warn when the file is
...
Provider-owned secrets can be linked without copying values into Todero by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Todero stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Todero Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Todero company secrets**. Allowed credential sources are
workload identity attached to the Todero server (instance profile, IRSA, ECS
...
Todero `external_reference` secrets. This is a metadata-only link: Todero
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the Todero name and key, then
   submit.
...
cursors. Do not expect Todero to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Todero runtime
...
- Keep managed create/rotate/delete permissions scoped to the Todero
...
dedicated Todero runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Todero
...
- `aws_secrets_manager`: back up Todero's database for vault metadata
  ...
  same Todero company at the same AWS namespace and confirming the runtime
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Todero. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Todero
...
For Todero Cloud, provision the server runtime IAM role/workload identity,
...
Todero secrets. Bootstrap material belongs in infrastructure IAM/workload
...
```sh
npx todero secrets migrate-inline-env --company-id <company-id>
...
Use the CLI command for normal operations because it goes through the Todero
...
the value custodian; Todero stores metadata and provider version references,
