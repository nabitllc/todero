...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Todero.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.todero.yaml`
...
- Todero must not require extra top-level fields for skill validity
- Todero-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
For Todero, the preferred fidelity extension is:
...
- issue/task Todero-only metadata
...
- Todero tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Todero shape:
...
Additional rules for Todero exporters:
...
- omit empty and default-valued Todero fields when possible
...
- Todero exporters should emit `.todero.yaml` as a sidecar by default
...
Todero can map this spec to its runtime model like this:
...
- Todero extension:
  - `.todero.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Todero-specific fidelity

Inline Todero-only metadata that must live inside a shared markdown file should use:

- `metadata.paperclip`

That keeps the base format broader than Todero.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Todero.

## 20. Cutover

Todero should cut over to this markdown-first package model as the primary portability format.

`todero.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Todero, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
