...
- Either a host Codex login with `~/.codex/auth.json`, or a per-agent
  `OPENAI_API_KEY` configured in adapter env (Todero materializes this into
...
The adapter symlinks Todero skills into the global Codex skills directory (`~/.codex/skills`). Existing user skills are not overwritten.

## Fast Mode

When `fastMode` is enabled, Todero adds Codex config overrides equivalent to:

```sh
-c 'service_tier="fast"' -c 'features.fast_mode=true'
```

Todero currently applies that only when the selected model is `gpt-5.4`. On other models, the toggle is preserved in config but ignored at execution time to avoid unsupported runs.

## Managed `CODEX_HOME`

When Todero is running inside a managed worktree instance (`PAPERCLIP_IN_WORKTREE=true`), the adapter instead uses a worktree-isolated `CODEX_HOME` under the Todero instance so Codex skills, sessions, logs, and other runtime state do not leak across checkouts. It seeds that isolated home from the user's main Codex home for shared auth/config continuity.
...
- **Genuine external overrides** (a `CODEX_HOME` outside the Todero-managed company tree) are treated as self-managed and are never seeded or overwritten.
...
3. **External `CODEX_HOME`:** if adapter env points `CODEX_HOME` outside the
   Todero-managed company tree, that home is self-managed. Todero does
   not seed or overwrite it, so its own `auth.json` wins.

For sandbox or SSH execution, Todero uploads the effective managed
...
`$HOME/.codex/auth.json`, and the Todero host is logged in with a ChatGPT
subscription. For a managed `codex_local` agent, Todero symlinks the host
...
If `instructionsFilePath` is configured, Todero reads that file and prepends it to the stdin prompt sent to `codex exec` on every run.

This is separate from any workspace-level instruction discovery that Codex itself performs in the run `cwd`. Todero does not disable Codex-native repo instruction files, so a repo-local `AGENTS.md` may still be loaded by Codex in addition to the Todero-managed agent instructions.
