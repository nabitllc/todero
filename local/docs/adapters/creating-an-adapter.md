...
Build a custom adapter to connect Todero to any agent runtime.
...
|---|---|---|
| Source | Inside `todero-fork` | Separate npm package |
| Distribution | Ships with Todero | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Todero's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@todero/adapter-utils/server-utils`
2. Build environment with `buildToderoEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@todero/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildToderoEnv(agent)` | `@todero/adapter-utils/server-utils` | Inject `PAPERCLIP_*` env vars |
| `renderTemplate(tpl, data)` | `@todero/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@todero/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@todero/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Todero's source:
...
`format-event.ts` — pretty-prints stdout for `todero run --watch` using `picocolors`.
...
With these flags set, the Todero UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Todero source changes required.
...
Make Todero skills discoverable to your agent runtime without writing to the agent's working directory:
