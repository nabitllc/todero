...
function getRuntimeServicesDir() {
  return path.resolve(resolveToderoInstanceRoot(), "runtime-services");
}

function getRuntimeServiceLogsDir() {
  return path.resolve(resolveToderoInstanceRoot(), "runtime-service-logs");
}
 ...
 * as spawn returns without tying the service's stdio lifetime to Todero's.
 */
