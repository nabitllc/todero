 ...
 * Todero went on reporting the leased lane `stopped` and its exposure
 * `removed`.
 *
 ...
 */
import { deriveViteHmrPort, isRuntimeExposureAppPort } from "@todero/shared";
...
export type ExposureReservationSource =
  ...
  | "in_memory_runtime"
  /** The broker reports a live Todero-owned Serve mapping on the port. */
  | "broker_mapping"
...
/** One Todero-owned listener as reported by the broker's `list()`. */
...
export interface ExposureReservationDrift {
  /** The row Todero believes is stopped/removed. */
  runtimeServiceId: string;
  owner: ExposureOwnerIdentity;
  port: number;
