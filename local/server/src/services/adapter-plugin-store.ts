 ...
 * ~/.todero/adapter-plugins.json. This is the source of truth for which
...
import { resolveToderoHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-todero-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const toderoDir = resolveToderoHomeDir();
  return {
    adapterPluginsDir: path.join(toderoDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(toderoDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(toderoDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "todero-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Todero external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
