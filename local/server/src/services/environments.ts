...
const DEFAULT_LOCAL_ENVIRONMENT_DESCRIPTION =
  "Default execution environment for Todero runs on this machine.";
...
export interface KubernetesEnvironmentConfigInput {
   ...
   */
  timeoutMs?: number;
  adapters?: import("@todero/shared").AdapterRegistryEntry[];
  [key: string]: unknown;
}
...
export function environmentService(db: Db) {
  /**
   * Idempotently ensure THE Todero-managed sandbox environment for this
    ...
    const reconciliation = await db.transaction(
      async (tx): Promise<ManagedSandboxEnvironmentReconcileResult> => {
        ...
        let row = sandboxRows.find(
          (candidate) => (candidate.metadata as Record<string, unknown> | null)?.managedByTodero === true,
        ...
        if (stockStatus === "operator_modified") {
          ...
          // an operator edit. If the binding records that Todero archived
  ...
  /**
   * Archive the Todero-managed sandbox row when its provider became
  ...
  const archiveManagedSandboxEnvironment = async (
    ...
    const archived = await db.transaction(async (tx) => {
      const existing = await tx
        ...
        .then(
          (rows) => rows.find(
            (row) => (row.metadata as Record<string, unknown> | null)?.managedByTodero === true,
          ) ?? null,
        );
      ...
      // Archival is a Todero-owned availability transition. Record only
  ...
  return {
     ...
     * stamped `managedByTodero: true` (other metadata preserved) — so the
    ...
    ensureLocalEnvironment: async (_companyId?: string): Promise<Environment> => {
      ...
      if (isCloudManagedInstance() && existingMetadata.managedByTodero !== true) {
        const adopted = await db
          .update(environments)
          .set({
            metadata: { ...existingMetadata, managedByTodero: true },
            updatedAt: new Date(),
          })
     ...
     * `managedByTodero`-marked slot row), if one exists. Read-only
