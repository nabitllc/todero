...
import { claudeConfigDir, parseClaudeStreamJson } from "@todero/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@todero/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@todero/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveToderoInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "todero-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "todero-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "todero-feedback-v1";
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readToderoSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let toderoRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      toderoRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "todero/run.json",
        contentType: "application/json",
        source: "paperclip_run",
        contents: `${JSON.stringify(toderoRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.toderoRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "todero/run-events.json",
        contentType: "application/json",
      ...
      if (logText) {
        files.push(makeBundleFile({
          path: "todero/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "paperclip_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.toderoRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
