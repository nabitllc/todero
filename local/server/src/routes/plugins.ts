...
import type { PluginPerformActionActorContext, ToolRunContext } from "@todero/plugin-sdk";
import { JsonRpcCallError, PLUGIN_RPC_ERROR_CODES } from "@todero/plugin-sdk";
import {
  assertAuthenticated,
  assertBoard,
  assertBoardOrAgent,
...
interface PluginInstallRequest {
  /** npm package name (e.g., @todero/plugin-linear) or local path */
  packageName: string;
...
function manifestSourcePath(packageRoot: string, pkgJson: Record<string, unknown>): string | null {
  ...
  const manifestPath = (toderoPlugin as Record<string, unknown>).manifest;
    ...
    bundledPlugins.push({
      entry: {
        ...
        description: metadata.description
          ?? `Bundled Todero plugin from ${path.relative(REPO_ROOT, packageRoot)}.`,
