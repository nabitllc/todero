...
describe("agent instructions service", () => {
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const toderoHome = await makeTempDir("todero-agent-instructions-home-");
    const externalRoot = await makeTempDir("todero-agent-instructions-external-");
    cleanupDirs.add(toderoHome);
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("todero-agent-instructions-new-external-");
    cleanupDirs.add(toderoHome);
    cleanupDirs.add(externalRoot);
    process.env.PAPERCLIP_HOME = toderoHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("todero-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const toderoHome = await makeTempDir("todero-agent-instructions-recover-");
    cleanupDirs.add(toderoHome);
    process.env.PAPERCLIP_HOME = toderoHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const toderoHome = await makeTempDir("todero-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("todero-agent-instructions-stale-root-");
    cleanupDirs.add(toderoHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const toderoHome = await makeTempDir("todero-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("todero-agent-instructions-heal-write-stale-");
    cleanupDirs.add(toderoHome);
    cleanupDirs.add(staleRoot);
    process.env.PAPERCLIP_HOME = toderoHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const toderoHome = await makeTempDir("todero-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("todero-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(toderoHome);
    cleanupDirs.add(staleRoot);
    process.env.PAPERCLIP_HOME = toderoHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const toderoHome = await makeTempDir("todero-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("todero-agent-instructions-partial-root-");
    cleanupDirs.add(toderoHome);
    cleanupDirs.add(staleRoot);
    process.env.PAPERCLIP_HOME = toderoHome;
    process.env.PAPERCLIP_INSTANCE_ID = "test-instance";
