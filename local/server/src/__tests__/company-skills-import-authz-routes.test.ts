...
describeEmbeddedPostgres("company skill import authorization routes", () => {
  ...
  beforeAll(async () => {
    ...
    toderoHome = await fs.mkdtemp(path.join(os.tmpdir(), "todero-company-skills-import-authz-home-"));
    process.env.PAPERCLIP_HOME = toderoHome;
    process.env.PAPERCLIP_INSTANCE_ID = "default";
    tempDb = await startEmbeddedPostgresTestDatabase("todero-company-skills-import-authz-");
  ...
  async function writeSkillFixture(companyId: string) {
    ...
    const skillDir = await fs.mkdtemp(path.join(managedRoot, "todero-import-authz-skill-"));
