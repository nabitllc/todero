...
import type { CatalogSkill } from "@todero/shared";
...
function catalogSkill(slug: string, name = slug): CatalogSkill {
  return {
    id: `todero:bundled:software-development:${slug}`,
    key: `todero/bundled/software-development/${slug}`,
    kind: "bundled",
...
describe("skills catalog service", () => {
  ...
  beforeEach(() => {
    ...
    mockRequireResolve.mockImplementation((specifier: string) => {
      if (specifier === "@todero/skills-catalog/package.json") {
        return "/published/node_modules/@todero/skills-catalog/package.json";
      }
      if (specifier === "@todero/skills-catalog/catalog.json") {
        return "/published/node_modules/@todero/skills-catalog/generated/catalog.json";
      }
  ...
  it("caches and reloads the generated catalog manifest when it changes", async () => {
    ...
    expect(service.listCatalogSkills().map((skill) => skill.key)).toEqual([
      "todero/bundled/software-development/old-skill",
    ]);
    expect(service.listCatalogSkills().map((skill) => skill.key)).toEqual([
      "todero/bundled/software-development/old-skill",
    ]);
    ...
    expect(service.listCatalogSkills().map((skill) => skill.key)).toEqual([
      "todero/bundled/software-development/new-skill",
    ]);
  ...
  it("resolves the manifest and bundled skill files from the published package layout", async () => {
    ...
    expect(mockReadFileSync).toHaveBeenCalledWith(
      "/published/node_modules/@todero/skills-catalog/generated/catalog.json",
      "utf8",
    );
    expect(mockReadFile).toHaveBeenCalledWith(
      "/published/node_modules/@todero/skills-catalog/catalog/bundled/software-development/published-skill/SKILL.md",
    );
    expect(mockRequireResolve).toHaveBeenCalledWith("@todero/skills-catalog/package.json");
    expect(mockRequireResolve).toHaveBeenCalledWith("@todero/skills-catalog/catalog.json");
  });
