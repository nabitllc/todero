...
// The `.todero.yaml` extension is consumed by the importer's own hand-rolled
...
function renderToderoYaml(value: Record<string, unknown>): string {
...
function buildSyntheticBundle(options: SyntheticBundleOptions): {
  ...
  files[".todero.yaml"] = renderToderoYaml({ schemaVersion: 7, tasks });
...
describeEmbeddedPostgres("company import batches inserts", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("todero-import-batching-");
  ...
  it("preserves bundle timestamps and parent hierarchy through a real import", async () => {
    ...
    files[".todero.yaml"] = renderToderoYaml({
      schemaVersion: 7,
      tasks: {
