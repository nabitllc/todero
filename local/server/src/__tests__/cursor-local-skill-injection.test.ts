...
import { ensureCursorSkillsInjected } from "@todero/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Todero skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("todero-cursor-skills-src-");
    const skillsHome = await makeTempDir("todero-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "todero");
    await createSkillDir(skillsDir, "todero-create-agent");
    ...
    const injectedA = path.join(skillsHome, "todero");
    const injectedB = path.join(skillsHome, "todero-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "todero")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "todero-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "todero"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "todero-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("todero-cursor-preserve-src-");
    const skillsHome = await makeTempDir("todero-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "todero");
    ...
    expect((await fs.lstat(path.join(skillsHome, "todero-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("todero-cursor-fail-src-");
    const skillsHome = await makeTempDir("todero-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
