...
const BLOCKED_HOSTNAME_MESSAGE =
  "This hostname is not allowed for this Todero instance. " +
  "If you want to allow a hostname, run npx todero allowed-hostname <host>.";
...
export function privateHostnameGuard(opts: {
  ...
  return (req, res, next) => {
    ...
    if (!hostname) {
      const error = "Missing Host header. If you want to allow a hostname, run npx todero allowed-hostname <host>.";
      if (wantsJson) {
