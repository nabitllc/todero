...
import type { Agent, AgentDesiredSkillEntry } from "@todero/shared";
...
const MATERIALIZATION_NOTE =
  "Enabled skills are materialized into the stable Todero-managed prompt bundle on the agent's next run.";

/** Company skill key of the Todero core skill that carries beta releases. */
const PAPERCLIP_CORE_SKILL_KEY = "nabitllc/todero/todero";
...
export function AgentSkillsTab({ agent, companyId }: { agent: Agent; companyId?: string }) {
  ...
  const toderoCoreSkill = useMemo(
    () => (companySkills ?? []).find((skill) => skill.key === PAPERCLIP_CORE_SKILL_KEY) ?? null,
    [companySkills],
  );

  // Seeded releases (release_id IS NOT NULL) for the todero core skill. Only
  // fetched when the flag is on and the skill is present in the library.
  const { data: toderoVersions } = useQuery({
    queryKey: queryKeys.companySkills.versions(companyId ?? "", toderoCoreSkill?.id ?? ""),
    queryFn: () => companySkillsApi.versions(companyId!, toderoCoreSkill!.id),
    enabled: Boolean(companyId && betaSkillsEnabled && toderoCoreSkill?.id),
  });
  const toderoReleases = useMemo(
    () => (toderoVersions ?? []).filter((version) => version.releaseId != null),
    [toderoVersions],
  );
  ...
  const unsupportedMessage = useMemo(() => {
    ...
    if (
      ...
      return "Todero cannot manage skills for custom ACP commands yet.";
    }
    if (agent.adapterType === "openclaw_gateway") {
      return "Todero cannot manage OpenClaw skills here. Visit your OpenClaw instance to manage this agent's skills.";
    }
    return "Todero cannot manage skills for this adapter yet. Manage them in the adapter directly.";
  ...
  // The release picker only applies to the enabled todero core skill while the
  // beta-skills flag is on and seeded releases exist.
  const releasePickerActive = betaSkillsEnabled && toderoReleases.length > 0;
  ...
  const renderRow = (row: AgentSkillRowData, variant: "enabled" | "available") => {
    ...
    const pinnedRelease = pinnedVersionId
      ? toderoReleases.find((release) => release.id === pinnedVersionId) ?? null
      : null;
