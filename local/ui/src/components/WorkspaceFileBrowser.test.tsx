...
import type { Project, ProjectWorkspace, WorkspaceFileListDirectoryItem, WorkspaceFileListFileItem, WorkspaceFileListItem, WorkspaceFileListResponse } from "@todero/shared";
...
function createWorkspace(overrides: Partial<ProjectWorkspace> = {}): ProjectWorkspace {
  return {
    ...
    cwd: "/srv/todero/home/nabitllc/todero-content",
    repoUrl: null,
    repoRef: null,
    defaultRef: null,
    visibility: "default",
...
describe("WorkspaceFileBrowser", () => {
  ...
  it("groups nested paths into collapsible folder rows instead of repeating full paths", () => {
    useQueryMock.mockReturnValue(
      ok(availableResponse([
        createItem({
          title: "tweet.md",
          relativePath: "videos/90-days-todero/tweet.md",
          displayPath: "videos/90-days-todero/tweet.md",
        }),
        createItem({
          title: "90-days-todero-1x1.mp4",
          relativePath: "videos/90-days-todero/out/90-days-todero-1x1.mp4",
          displayPath: "videos/90-days-todero/out/90-days-todero-1x1.mp4",
          previewKind: "video",
        }),
      ])),
    );
    ...
    expect(container.textContent).toContain("90-days-todero");
    expect(container.textContent).toContain("tweet.md");
    expect(container.textContent).toContain("90-days-todero-1x1.mp4");
    expect(container.textContent).not.toContain("videos/90-days-todero/tweet.md");
  ...
  it("opens a result from a selected other project workspace", () => {
    const contentItem = createItem({
      ...
      displayPath: "Todero Content / content-os/cases/active/2026-06-06-pap-10199-bundled-skills/README.md",
      workspaceLabel: "Todero Content",
      workspaceKind: "project_workspace",
      workspaceId: "workspace-content",
      projectId: "project-content",
      projectName: "Todero Content",
    });
    useQueryMock.mockImplementation((options: { queryKey: readonly unknown[] }) => {
    ...
    expect(container.textContent).toContain("Todero Content / Todero Content");
    const listCall = useQueryMock.mock.calls.find(([options]) => options.queryKey?.[3] === "list");
    expect(listCall?.[0].queryKey[4]).toMatchObject({
      workspace: "project",
      projectId: "project-content",
  ...
  it("focuses an initial folder in a selected other project workspace", () => {
    ...
    const contentItem = createItem({
      ...
      displayPath: `Todero Content / ${folderPath}README.md`,
      workspaceLabel: "Todero Content",
      workspaceKind: "project_workspace",
      workspaceId: "workspace-content",
      projectId: "project-content",
