...
type ToderoSprite = (typeof TODERO_SPRITES)[number];
...
function spriteSize(sprite: ToderoSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = TODERO_SPRITES[(baseCol + baseRow) % TODERO_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = TODERO_SPRITES[Math.floor(Math.random() * TODERO_SPRITES.length)]!;
