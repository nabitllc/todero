...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Todero will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Todero handles this" mechanics bullets, composed from the
