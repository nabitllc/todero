export const SELECTION_DEBUG_STORAGE_KEY = "toderoDebugSelection";
...
export function initializeSelectionDebug(): SelectionDebugState {
  if (!window.__paperclipSelectionDebug) {
    ...
    console.info(`[todero selection debug] enabled; set localStorage.${SELECTION_DEBUG_STORAGE_KEY} = "0" and reload to disable`);
  }
...
function record(type: string, details: SelectionDebugEvent["details"]): void {
  ...
  console.debug("[todero selection debug]", event);
}
