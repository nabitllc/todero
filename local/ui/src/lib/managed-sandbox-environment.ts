 ...
 * managed-config environment provisioner stamps `metadata.managedByTodero`).
...
export function isPlatformManagedEnvironment(
  ...
  return environment?.metadata?.managedByTodero === true;
}
 ...
 * default Todero environment, and the raw driver key ("sandbox") is
