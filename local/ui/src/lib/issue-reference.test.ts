...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Todero issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/nabitllc/todero/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/nabitllc/todero/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute Todero issue URLs so origin, port, and hash are not lost", () => {
