...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the Todero API key");
    ...
    expect(prompt).toContain("agentDefaultsPayload.toderoApiUrl");
    ...
    expect(prompt).toContain("Todero maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the Todero host");
    expect(prompt).toContain("Hermes-originated Todero API calls");
    expect(prompt).toContain("Do not rotate or invent a Todero key manually");
