...
beforeEach(() => {
  ...
  process.env.PAPERCLIP_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "todero-service-restart-"));
});
...
function managerFixture(active = true) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "todero-service-doctor-"));
  const definitionPath = path.join(root, "todero.service");
  fs.writeFileSync(definitionPath, "unit");
  return {
    platform: "systemd" as const,
    instanceId: "default",
    serviceName: "todero.service",
    definitionPath,
  ...
  it("detects a foreground process on the configured port while the service is inactive", async () => {
    ...
    expect(results).toContainEqual(
      expect.objectContaining({
        ...
        status: "fail",
        message: expect.stringContaining("another Todero process"),
      }),
    );
  });
});
...
describe("service runtime shim awareness", () => {
  function inactiveManager() {
    return {
      ...
      instanceId: "default",
      serviceName: "ing.todero.toderoai",
  ...
  it("diagnoses against the executable recorded in the definition, not the current env", async () => {
    ...
    const runtime = results.find((r) => r.name === "Service runtime");
    expect(shimPresent).toHaveBeenCalledWith("/custom/bin/todero");
    expect(runtime?.message).toContain("/custom/bin/todero");
    expect(runtime?.repairHint).toContain("/custom/bin/todero");
    expect(runtime?.repairHint).toContain("unset PAPERCLIP_SHIM_PATH");
    expect(runtime?.repairHint).toContain("`todero install` followed by `todero service install`");
  });
  ...
  it("attributes a healthy foreign responder instead of reporting Healthy", async () => {
    ...
    expect(healthResult?.message).toContain("but not from ing.todero.toderoai");
    const runtime = results.find((r) => r.name === "Service runtime");
    expect(runtime?.message).toContain("serving another Todero process");
  });
});

describe("definition executable extraction", () => {
  it("round-trips through both renderers", () => {
    const unit = renderSystemdUnit({ instanceId: "default", shimPath: "/custom/bin/todero", homeDir: "/home/x/.todero" });
    expect(extractExecutableFromSystemdUnit(unit)).toBe("/custom/bin/todero");
    const plist = renderLaunchdPlist({ instanceId: "default", shimPath: "/custom/bin/todero", homeDir: "/home/x/.todero", stdoutPath: "/tmp/o.log", stderrPath: "/tmp/e.log" });
    expect(extractExecutableFromLaunchdPlist(plist)).toBe("/custom/bin/todero");
  ...
  it("round-trips paths the renderers escape", () => {
    const hostile = '/tmp/we"ird $pa%th & <x>/todero';
    const unit = renderSystemdUnit({ instanceId: "default", shimPath: hostile, homeDir: "/home/x/.todero" });
    expect(extractExecutableFromSystemdUnit(unit)).toBe(hostile);
    const plist = renderLaunchdPlist({ instanceId: "default", shimPath: hostile, homeDir: "/home/x/.todero", stdoutPath: "/tmp/o.log", stderrPath: "/tmp/e.log" });
