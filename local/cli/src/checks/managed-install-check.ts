...
export function nodeRuntimeCheck(): CheckResult {
  return isSupportedNodeVersion(process.versions.node)
    ...
    : {
        ...
        repairHint: `Install Node.js ${MINIMUM_NODE_VERSION} or newer before installing or running Todero`,
      };
}
    ...
    return [
      {
        ...
        repairHint: "Re-run `todero install` to rebuild the managed install metadata",
      },
    ];
  }
    ...
    return [
      {
        ...
        repairHint: "Re-run `todero install`",
      },
    ];
  }
  ...
  results.push(
    payloadExists && currentMatches
      ...
      : {
          ...
          repairHint: "Re-run `todero install` or roll back to a retained payload",
        },
  );
  ...
  results.push(
    orphaned.length === 0
      ...
      : {
          ...
          repairHint: "A successful `todero update` prunes unretained payloads",
        },
  );
