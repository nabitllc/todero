...
export async function channelsCommand(
  ...
  console.log(pc.bold("Todero release channels"));
  console.log("");
  for (const entry of state) {
    ...
    console.log(`  ${" ".repeat(8)} ${pc.dim(`npx todero@${entry.distTag} onboard`)}`);
    console.log("");
  }
  ...
  console.log(`Docker images use the same names: ghcr.io/nabitllc/todero:{latest,beta,nightly,canary}`);
}
