...
import { resolveToderoInstanceRoot } from "./config/home.js";

export const PAPERCLIP_RUNTIME_INFO_FILENAME = "runtime-info.json";

export type ToderoRuntimeInfo = {
  schemaVersion: 1;
  instanceId: string;
  pid: number;
  host: string;
...
export function resolveRuntimeInfoPath(instanceId?: string): string {
  return path.join(resolveToderoInstanceRoot(instanceId), PAPERCLIP_RUNTIME_INFO_FILENAME);
}

function parseRuntimeInfo(value: unknown): ToderoRuntimeInfo | null {
  ...
  return record as ToderoRuntimeInfo;
}

export function readRuntimeInfo(instanceId?: string, filePath = resolveRuntimeInfoPath(instanceId)): ToderoRuntimeInfo | null {
  try {
