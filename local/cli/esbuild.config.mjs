/**
 * esbuild configuration for building the todero CLI for npm.
 *
 * Bundles all workspace packages (@todero/*) into a single file.
...
// to npm and resolved at runtime (e.g. @todero/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@todero/") && !bundledCliNpmDependencies.has(name)) {
      externals.add(name);
    }
  }
