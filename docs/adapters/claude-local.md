...
What it means: the on-disk Claude Code transcript JSONL for that session contains a malformed (non-`msg_`-prefixed) `previous_message_id`. Anthropic's `/v1/messages` rejects every resume attempt against that transcript with a deterministic 400. Without guards, Todero would re-persist the same poisoned session id and the issue is stranded permanently — see [RED-976](../../../) / [RED-978](../../../).
...
The adapter creates a temporary directory with symlinks to Todero skills and passes it via `--add-dir`. This makes skills discoverable without polluting the agent's working directory.
...
`CLAUDE_CONFIG_DIR` is configured, Todero creates a remote
...
its own Claude Code login. Todero starts a managed remote `claude_local` run,
...
Todero-managed sandbox run uploads a host-owned `CODEX_HOME/auth.json` and
...
This installs Todero skills in `~/.claude/skills`, creates an agent API key, and prints shell exports to run as that agent.
