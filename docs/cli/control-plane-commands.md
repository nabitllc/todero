...
```sh
# List issues
npx todero issue list [--status todo,in_progress] [--assignee-agent-id <id>] [--match text]

# Get issue details
npx todero issue get <issue-id-or-identifier>

# Create issue
npx todero issue create --title "..." [--description "..."] [--status todo] [--priority high]
...
# Checkout task
npx todero issue checkout <issue-id> --agent-id <agent-id>
...
npx todero company export <company-id> --out ./exports/acme --include company,agents

# Preview import (no writes)
npx todero company import \
  <owner>/<repo>/<path> \
  --target existing \
...
npx todero skills browse [--kind bundled|optional] [--category software-development] [--query github]
npx todero skills search "pull request" [--json]
...
npx todero skills install github-pr-workflow --as pr-flow --force --company-id <company-id>

# External sources still use import instead of catalog install
npx todero skills import ./skills/my-skill --company-id <company-id>
npx todero skills import owner/repo/path/to/skill --company-id <company-id>

# Attach desired company skills to an agent after install/import
npx todero skills agent sync <agent-id> --skill github-pr-workflow --mode add --company-id <company-id>
```
...
```sh
# List approvals
npx todero approval list [--status pending]

# Get approval
npx todero approval get <approval-id>

# Create approval
npx todero approval create --type hire_agent --payload '{"name":"..."}' [--issue-ids <id1,id2>]
...
```sh
npx todero activity list [--agent-id <id>] [--entity-type issue] [--entity-id <id>]
```

## Dashboard

```sh
npx todero dashboard get
```
...
```sh
npx todero heartbeat run --agent-id <agent-id> [--api-base http://localhost:3100]
```
