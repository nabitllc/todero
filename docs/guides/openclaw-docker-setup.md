...
How to get OpenClaw running in a Docker container for local development and testing the Todero OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Todero includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-todero-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Todero host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-todero-smoke`)
...
If your Todero deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Todero server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Todero URL (typically `http://host.docker.internal:3100`).
- If Todero rejects the container-visible host with a hostname error, allow it from host:

```bash
npx todero allowed-hostname host.docker.internal
```

Then restart Todero and rerun the smoke script.
