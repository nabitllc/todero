...
Use `hermes_local` when Todero should start the local Hermes CLI. Use
`hermes_gateway` when Hermes is already running as an API server and Todero
should call that server. Both are built-in adapter types from the unified
`@todero/hermes-paperclip-adapter` package.

For `opencode_local`, configure an explicit `adapterConfig.model` (`provider/model`).
Todero validates the selected model against live `opencode models` output.
