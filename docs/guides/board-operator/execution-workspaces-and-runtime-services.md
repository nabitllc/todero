...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Todero.

Todero now presents this as a workspace-command model:
...
- Todero does not automatically restart workspace services on server boot — services only come back up when the next run (or a manual start) brings them up.
...
2. Todero realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Todero persists execution-workspace metadata such as paths, refs, and provisioning settings.
...
A managed service that runs Todero itself needs one canonical origin for Better Auth and tool OAuth callbacks. Todero resolves that origin in this order:
