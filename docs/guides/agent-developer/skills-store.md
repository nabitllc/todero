...
The **Skills Store** is Todero's library of reusable skills. A skill is a markdown
...
|---|---|---|
| **The catalog** | A curated, read-only set of skills that ships with Todero | The `@todero/skills-catalog` package |
...
- **`bundled`** — first-party Todero skills (e.g. `issue-triage`, `task-planning`,
  `qa-acceptance`, `wireframe`, `github-pr-workflow`, `doc-maintenance`). These carry the
  reserved `nabitllc/todero/...` key namespace.
...
- **`category`** — grouping such as `software-development`, `quality`, `product`,
  `research`, `content`, `browser`, `todero-operations`, `docs`.
...
|---|---|---|
| `catalog` | Todero / catalog | Installed from the bundled catalog |
...
`ramp` skill is the model: Todero ships the stable governance wrapper, source
...
a vendored snapshot would go stale, and when Todero can keep the safety boundary in
the wrapper. For financial, legal, or account-control domains, the wrapper must require
Todero approvals before spend, incorporation, account authorization, card issuance,
...
Paste a source and Todero fetches and imports it. Accepted forms include:
...
The catalog is its own publishable package, `@todero/skills-catalog`:
