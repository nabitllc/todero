...
Add first-class **documents** to Todero as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Todero workflow/docs.
...
Update the Todero agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/todero/SKILL.md`
