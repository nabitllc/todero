...
Audience: Operators setting up and running agents in Todero

## 1. What this system does

Agents in Todero do not run continuously.  
...
- `hermes_local`: starts your local `hermes` CLI through `@todero/hermes-paperclip-adapter`
- `hermes_gateway`: calls an already-running Hermes API server through `@todero/hermes-paperclip-adapter/gateway`
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-todero-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Todero assumes the CLI is already installed and authenticated on the host machine. For `hermes_gateway`, Todero assumes the Hermes API server is already running, reachable from the Todero server, and configured with an API key. The older `@todero/adapter-hermes-gateway` npm package is only a deprecated compatibility shim; the adapter type remains `hermes_gateway`.
...
Todero stores session IDs for resumable adapters.
...
Todero pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Todero surfaces this as a warning in environment tests, not a hard error.
