...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Todero-Signature` + `X-Todero-Timestamp` header pair matching the trigger's signing mode.
