...
Todero `external_reference` secrets without copying plaintext into
Todero.
...
`ListSecrets` only and import stores a Todero external reference plus
...
remote metadata, or copies secret plaintext into Todero.
...
- `conflict`: the suggested Todero `name` or `key` is already in use.
...
Todero-managed AWS secret through a broad runtime role.
...
Each row may include an optional Todero `description` entered during review;
...
the Todero runtime role can list the AWS secret but lacks
