...
def log_recovery(issue, age_h, prior_notes):
    """Append recovery entry; pattern-detect if 3+ same agent/type → escalate."""
    ...
    if not RECOVERY_LOG.exists():
        RECOVERY_LOG.write_text(
            ...
            "Auto-generated by `scripts/monitor-stale.py`. "
            "Records every issue auto-rolled back from `in_progress` after >2h of inactivity.\n\n"
        )
...
def pattern_detect(agent, itype, task_key):
    """If same agent or issue-type recovered 3+ times, append escalation note to memory.md."""
    if not RECOVERY_LOG.exists():
        return
    text = RECOVERY_LOG.read_text()
    ...
    try:
        existing = HOT_MEMORY.read_text() if HOT_MEMORY.exists() else ""
        # Avoid duplicate stamp for same day
        if f"Stale-Recovery Pattern (auto-detected {today})" in existing:
            return
        with HOT_MEMORY.open("a") as f:
            f.write(note)
        print(f"[pattern] escalated to memory.md: {triggers}")
    except Exception as e:
        print(f"[pattern] failed to write memory.md: {e}")
