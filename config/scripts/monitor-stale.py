...
RECOVERY_LOG = Path.home() / "todero/config" / "self-improving" / "stale-recoveries.md"
HOT_MEMORY = Path.home() / "todero/config" / "self-improving" / "memory.md"
...
def memory_api_append(agent_id: str, memory_type: str, new_text: str) -> None:
    """GET existing content from MC API, append new_text, POST back."""
    base = "http://localhost:3000"
    try:
        with urllib.request.urlopen(
            f"{base}/api/agent-memory?agent_id={agent_id}&type={memory_type}", timeout=10
        ) as r:
    ...
    payload = json.dumps({
        ...
        "date_key": None,
        "content": existing + new_text,
    }).encode()
    req = urllib.request.Request(
        f"{base}/api/agent-memory", data=payload,
    ...
    try:
        ...
        print(f"[memory_api] failed to write {agent_id}/{memory_type}: {e}")
        # Fallback: write locally so data isn't lost
        fallback = RECOVERY_LOG if memory_type == "corrections" else HOT_MEMORY
...
def log_recovery(issue, age_h, prior_notes):
    """Append recovery entry to DB corrections memory; pattern-detect if 3+ same agent/type."""
    ...
    memory_api_append(agent, "corrections", entry)
    pattern_detect(agent, itype, task_key)


def pattern_detect(agent, itype, task_key):
    """If same agent or issue-type recovered 3+ times, append escalation note to long_term memory."""
    # Read corrections from DB to count patterns
    try:
        with urllib.request.urlopen(
            f"http://localhost:3000/api/agent-memory?agent_id={agent}&type=corrections", timeout=10
        ) as r:
            rows = json.loads(r.read()).get("memory", [])
            text = rows[0].get("content", "") if rows else ""
    except Exception:
        return
    ...
    if agent_count >= 3:
        triggers.append(
            f"Agent `{agent}` has been auto-recovered {agent_count} times — investigate why it stalls"
        )
    if type_count >= 3 and itype not in ("?", "task"):
        triggers.append(
            f"Issue type `{itype}` has been auto-recovered {type_count} times — review templates/workflow"
        )
    ...
    for t in triggers:
        note_lines.append(f"- {t} (latest: {task_key})")
    note = "\n".join(note_lines) + "\n"

    memory_api_append(agent, "long_term", note)
    print(f"[pattern] escalated to long_term memory: {triggers}")
