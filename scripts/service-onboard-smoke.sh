...
DATA_DIR="${DATA_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/todero-service-smoke.XXXXXX")}"
...
SERVICE_NAME="todero.service"
SHIM_PATH="${PAPERCLIP_SHIM_PATH:-$HOME/.local/bin/todero}"
...
echo "==> Onboarding todero@$TODEROAI_VERSION with --install-service"
echo "    Data dir: $DATA_DIR"
if ! timeout "$ONBOARD_TIMEOUT_SECONDS" \
  npx --yes "todero@${TODEROAI_VERSION}" onboard --yes --install-service --data-dir "$DATA_DIR"; then
  diagnostics
