...
publish_package_to_npm() {
  ...
  publish_log="$(mktemp "${TMPDIR:-/tmp}/todero-npm-publish.XXXXXX")"
...
verify_npm_installable() {
  ...
  install_dir="$(mktemp -d "${TMPDIR:-/tmp}/todero-release-install.XXXXXX")"
  ...
  installed_version="$(node -e "console.log(require(process.argv[1]).version)" "$install_dir/node_modules/todero/package.json")"
  rm -rf "$install_dir"
