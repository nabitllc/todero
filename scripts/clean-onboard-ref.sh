...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Todero data dir to use
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/todero-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/todero-data}"
...
env \
  ...
  pnpm todero onboard --yes --data-dir "$PC_DATA"
