...
const DEFAULT_QUERY = "\"Co-Authored-By: Todero <noreply@todero.vercel.app>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/todero-commit-metrics-cache.json");
...
const PAPERCLIP_EMAIL = "noreply@todero.vercel.app";
const PAPERCLIP_NAME = "todero";
...
function printHelp() {
  console.log(`Usage: tsx scripts/todero-commit-metrics.ts [options]
...
function printSummary(summary: Summary) {
  console.log("Todero commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "todero-commit-metrics",
