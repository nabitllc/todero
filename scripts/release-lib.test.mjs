...
function runPublishHelper({
  ...
  const fixtureDir = mkdtempSync(join(tmpdir(), "todero-release-lib-"));
...
${
  waitForRegistry
    ? `publish_package_to_npm_and_wait ${distTag} @todero/example 1.2.3 ${publishTool} 1 0`
    : `publish_package_to_npm ${distTag} @todero/example 1.2.3 ${publishTool}`
}
`;
...
test("publish_package_to_npm retries bundled directory tlog failures without provenance", () => {
  ...
  assert.equal(result.status, 0);
  assert.match(result.calls, /^npm view @todero\/example@1\.2\.3 version$/m);
  assert.match(
    result.calls,
