...
description: >
  Handle confidential GitHub Security Advisory response for Todero. Use when
...
```
gh api repos/nabitllc/todero/security-advisories/{{ghsaId}}
...
```
gh api --method POST \
  repos/nabitllc/todero/security-advisories/{{ghsaId}}/forks
...
```
# Clone the private fork somewhere outside ~/todero
...
**Do not edit `~/todero`** — the dev server is running off the `~/todero` master branch and we don't want to touch it. All work happens in the private fork clone.
...
```
gh api --method POST \
  repos/nabitllc/todero/security-advisories/{{ghsaId}}/cve
...
```
gh api repos/nabitllc/todero/security-advisories/{{ghsaId}} --jq '.credits'
...
```
gh api --method PATCH \
  repos/nabitllc/todero/security-advisories/{{ghsaId}} \
  --input - << 'EOF'
{
  "credits": [
    {
...
### Advisory
https://github.com/nabitllc/todero/security/advisories/{{ghsaId}}
...
gh release view v{{patchedVersion}} --repo nabitllc/todero
...
* The published advisory URL: `https://github.com/nabitllc/todero/security/advisories/{{ghsaId}}`
* The release URL
