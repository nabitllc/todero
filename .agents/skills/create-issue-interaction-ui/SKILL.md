...
description: >
  Add a new Todero issue-thread interaction kind end-to-end. Use when repo
...
Developer/maintainer skill. Do NOT install this on production Todero agents.

This skill walks a Todero contributor through introducing a new issue-thread
...
human or coding agent making code changes inside `nabitllc/todero`, not
the operational agents that run inside a deployed Todero company.

Do NOT install this on production Todero agents. This guide is for repository contributors changing Todero itself.
...
- Changing how Todero agents *call* interactions. Update `skills/todero`
...
| Helpers/Docs | CLI command, MCP tool, plugin SDK type+host+testing path, `skills/todero` guidance.            |
...
your own plan document if you are running this work through Todero itself.
...
- `skills/todero/SKILL.md` — add a row to the interaction-kinds table:
  *when to use*, *when not to use*, plus a copyable payload example.
- `skills/todero/references/api-reference.md` — full payload and result
...
- [ ] CLI, MCP, and plugin SDK helpers all accept the new payload shape and
      have test coverage.
- [ ] `skills/todero/SKILL.md` and `references/api-reference.md` updated.
