---
name: todero-dev-workspace-run-verify-fix
description: >
  Run, verify, reseed, and repair Todero isolated dev workspace services. Use
...
This skill is for Todero-specific development workspaces whose service is
started through project execution workspace runtime services, typically a
worktree service such as `todero-dev`.
...
- The master checkout `/srv/todero/home/nabitllc/todero` (the primary
  ...
  under `<master>/.todero/worktrees/`, and never edit its `.env` while
  repairing a workspace. Managed workspaces run in their own folders, for
  example `~/.todero-worktrees/instances/<slug>/…`.
- Never trust a `running` / `healthy` runtime row or a passing `/api/health`
  alone. The `service:todero-dev` port is pinned in service config, so a
...
- Read `doc/DEVELOPING.md` before running Todero CLI, dev server, worktree,
  database, build, or test commands.
- Use the Todero CLI and API as the source of truth for worktree and
...
- runtime workspace command id, usually `service:todero-dev`
- expected service URL, for example `http://todero-dev:40631`
...
`X-Todero-Run-Id` so the mutation is associated with the current heartbeat.
...
curl -sS -X POST \
  ...
  --data-binary '{"workspaceCommandId":"service:todero-dev"}'
```
...
`service:todero-dev` pins an explicit port in service config, so every
...
```sh
npx todero worktree reseed --from-instance default --seed-mode full --yes
```
...
Use this only to identify and remove a stale matching Todero dev-runner
...
- matching workspace command id, usually `service:todero-dev`
- `status: "running"`
...
The cloned Todero app must also know about the service. Query the same
...
Symptom: the workspace URL loads a working Todero app, but it is a sibling
...
Todero dev-runner process group for the target port and terminate only that
...
Symptom: the main app sees `todero-dev` running, but the cloned app copied a
...
the cloned app state itself must be repaired, use normal Todero issue/run
...
- Worktree instance logs live at
  `~/.todero-worktrees/instances/<slug>/logs`. Read them before guessing at
  login or startup failures.
- Probing the service through a LAN or tailscale IP can return `403` while
  `127.0.0.1` (or the `todero-dev` hostname) works. Probe loopback first
