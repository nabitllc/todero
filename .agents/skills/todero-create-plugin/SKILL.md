---
name: todero-create-plugin
description: >
  Create and develop external Todero plugins with the CLI-first workflow. Use
  when scaffolding a plugin, iterating on a local plugin, installing it into
  Todero, or updating plugin authoring docs.
---

# Create and develop a Todero plugin

Use this skill when the task is to create, scaffold, or iterate on a Todero plugin against a local Todero instance.

## 1. Default: build the plugin OUTSIDE Todero core
...
- Scaffold the plugin into a directory outside the Todero checkout (e.g. `~/dev/todero-plugins/<name>`).
- Install it into the running Todero instance by local absolute path.
- Edit code in the external package; let Todero pick up rebuilt output.

Only edit Todero core itself when the user asks to surface a plugin as a bundled example (`server/src/routes/plugins.ts`, in-repo example lists, docs).
...
Use `todero plugin init`. Do not invoke the scaffold package node entrypoint by hand unless the CLI command is unavailable in the environment.

```bash
todero plugin init @acme/my-plugin --output ~/dev/todero-plugins
```
...
- `--sdk-path <path>` — snapshot the local SDK from a Todero checkout into `.todero-sdk/` (useful when developing against an unreleased SDK).

On success the command prints the exact next commands (`cd`, `pnpm install`, `pnpm dev`, `todero plugin install <abs-path>`). Run them in order.

If `todero` is not on PATH in your environment, fall back to:

```bash
pnpm --filter @todero/create-todero-plugin build
node packages/plugins/create-todero-plugin/dist/index.js @acme/my-plugin \
...
todero plugin install /absolute/path/to/my-plugin
```

Notes:

- `todero plugin install` auto-detects local paths (absolute, `./`, `../`, `~`, or an existing relative folder) and forwards `isLocalPath: true` to the server. Pass `--local` to force local mode if the heuristic is ambiguous.
...
```bash
todero plugin list
todero plugin inspect <plugin-key>
```
...
- `package.json` — `toderoPlugin` block points at `dist/manifest.js`, `dist/worker.js`, `dist/ui/`
...
If you changed Todero SDK/host/plugin runtime code in addition to the plugin, also run the relevant Todero workspace checks.
...
- **Commands run** — the exact `todero plugin init`, `pnpm install`, `pnpm dev`, `todero plugin install <path>` invocations (and any verification commands).
- **Install status** — output of `todero plugin list` / `plugin inspect` (plugin key, version, status). Note if `status` is anything other than `ready` and include `lastError`.
...
## 8. When NOT to edit Todero core
