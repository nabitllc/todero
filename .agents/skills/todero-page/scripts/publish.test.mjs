...
function createSite(name = "todero-page-test") {
  const siteDir = mkdtempSync(join(tmpdir(), `${name}-`));
  tempDirs.add(siteDir);
  writeFileSync(join(siteDir, "index.html"), "<!doctype html><title>Todero</title>\n");
  return siteDir;
}
...
function writeState(siteDir, state) {
  mkdirSync(join(siteDir, ".todero-page"), { recursive: true });
  writeFileSync(join(siteDir, ".todero-page", "state.json"), `${JSON.stringify(state)}\n`);
}
  ...
  try {
    return {
      output: execFileSync("bash", [scriptPath, ...args], {
        ...
        env: {
          ...process.env,
          PAPERCLIP_PAGE_BUCKET: "todero-pages-test",
...
test("dry run validates and prints the planned target without requiring AWS", () => {
  ...
  assert.match(result.output, /^todero-page dry run$/m);
  assert.match(result.output, /^mode: publish$/m);
  assert.match(result.output, /^bucket: todero-pages-test$/m);
...
test("namespaced page keys conflict with PAPERCLIP_PAGE_AWS_PROFILE", () => {
  const result = runPublish(
    [createSite(), "--slug", "demo-page", "--dry-run"],
    {
      ...
      PAPERCLIP_PAGE_AWS_PROFILE: "todero-page-uploader",
    },
  );
...
test("namespaced page keys are scoped to the helper's aws calls", () => {
  const siteDir = createSite();
  const binDir = mkdtempSync(join(tmpdir(), "todero-page-bin-"));
  tempDirs.add(binDir);
...
`,
  ...
  const seen = readFileSync(envDump, "utf8");
  assert.match(seen, /^PROFILE_ARG=todero-page-uploader$/m);
...
`,
  ...
  assert.equal(existsSync(join(siteDir, ".todero-page", "state.json")), true);

  const state = JSON.parse(readFileSync(join(siteDir, ".todero-page", "state.json"), "utf8"));
  assert.equal(state.bucket, "todero-pages-test");
