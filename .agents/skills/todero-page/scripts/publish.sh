...
Publishes a static directory with a root index.html to the configured Todero
...
assert_safe_source_tree() {
  ...
  found="$(
    cd "$source_dir"
    find . -mindepth 1 \
      \( -path './.todero-page' -o -path './.todero-page/*' \) -prune -o \
...
assert_update_ownership() {
  ...
  local bucket="$2"
  local prefix="$3"
  local state_file="$source_dir/.todero-page/state.json"
  local state_bucket
  local state_prefix
...
compute_source_hash() {
  ...
  (
    cd "$source_dir"
    find . -type f ! -path './.todero-page/*' -print0 |
      LC_ALL=C sort -z |
...
write_state() {
  ...
  local base_url="$6"
  local source_hash="$7"
  local state_dir="$source_dir/.todero-page"
...
cat <<EOF
todero-page published
mode: $mode
url: $url
bucket: $bucket
prefix: $prefix
state: $source_dir/.todero-page/state.json
EOF
