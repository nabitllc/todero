...
name: todero-page
description: >
  Publish static HTML pages and asset folders to the Todero S3/CloudFront page
...
Use this skill to publish a static directory to the configured Todero pages
host, for example `https://pages.todero.ing/<slug>/`.
...
- Environment variables are configured:
  ...
  - `PAPERCLIP_PAGE_AWS_ACCESS_KEY_ID` and `PAPERCLIP_PAGE_AWS_SECRET_ACCESS_KEY`
    with the page-uploader credentials from Todero Secrets
...
```bash
.agents/skills/todero-page/scripts/publish.sh ./site --slug my-page
```
...
```bash
.agents/skills/todero-page/scripts/publish.sh ./site --slug my-page --update
```

When the target prefix already exists, `--update` requires local ownership proof
from `./site/.todero-page/state.json` generated by an earlier publish from
...
- `Slug already exists`: choose a different slug or use `--update` from the
  original source directory containing `.todero-page/state.json`.
...
- `AccessDenied`: confirm the uploader IAM policy allows `ListBucket`,
  ...
  agent received the Todero Secrets.
