...
`todero-page` publishes static page directories to a Todero-controlled S3
bucket served through CloudFront. It is the durable Todero-owned replacement
...
- Public content only.
- Dedicated uploader IAM identity, separate from Todero attachment storage.
...
```bash
.agents/skills/todero-page/scripts/publish.sh ./site --slug demo --dry-run
```

Publish:

```bash
.agents/skills/todero-page/scripts/publish.sh ./site --slug demo
```

Update an existing page from the same source directory:

```bash
.agents/skills/todero-page/scripts/publish.sh ./site --slug demo --update
```
...
- `.todero-page/state.json` is allowed and excluded from uploads.
...
export PAPERCLIP_PAGE_BUCKET=todero-pages-prod
export PAPERCLIP_PAGE_BASE_URL=https://pages.todero.ing
...
export PAPERCLIP_PAGE_AWS_PROFILE=todero-page-uploader
...
Recommended Todero secret names:

- `todero-page-aws-access-key-id`
- `todero-page-aws-secret-access-key`
...
use, other buckets). Do not reuse Todero's internal S3 attachment/object
storage credentials.
...
```bash
export AWS_PROFILE=todero-admin
export AWS_REGION=us-east-1
export BUCKET=todero-pages-prod
export DOMAIN=pages.todero.ing
export UPLOADER_USER=todero-page-uploader
export CLOUDFRONT_COMMENT="Todero pages"
...
```bash
mkdir -p /tmp/todero-pages-bootstrap
printf '<!doctype html><title>Not found</title><h1>Not found</h1>\n' \
  > /tmp/todero-pages-bootstrap/404.html

aws s3 cp /tmp/todero-pages-bootstrap/404.html "s3://$BUCKET/404.html" \
  --profile "$AWS_PROFILE" \
...
aws acm request-certificate \
  ...
  --idempotency-token toderopages \
  > /tmp/todero-pages-acm.json

export CERT_ARN="$(jq -r '.CertificateArn' /tmp/todero-pages-acm.json)"
...
aws cloudfront create-origin-access-control \
  ...
  --origin-access-control-config "{
    \"Name\":\"todero-pages-oac\",
    \"Description\":\"OAC for $BUCKET\",
    \"SigningProtocol\":\"sigv4\",
    \"SigningBehavior\":\"always\",
    \"OriginAccessControlOriginType\":\"s3\"
  }" \
  > /tmp/todero-pages-oac.json

export OAC_ID="$(jq -r '.OriginAccessControl.Id' /tmp/todero-pages-oac.json)"
```
...
```bash
cat > todero-pages-index-router.js <<'EOF'
...
aws cloudfront create-function \
  --profile "$AWS_PROFILE" \
  --name todero-pages-index-router \
  --function-config 'Comment=Rewrite clean page URLs to index.html,Runtime=cloudfront-js-2.0' \
  --function-code fileb://todero-pages-index-router.js \
  > /tmp/todero-pages-function.json

export FUNCTION_ETAG="$(jq -r '.ETag' /tmp/todero-pages-function.json)"

aws cloudfront publish-function \
  --profile "$AWS_PROFILE" \
  --name todero-pages-index-router \
  --if-match "$FUNCTION_ETAG" \
  > /tmp/todero-pages-function-live.json

export FUNCTION_ARN="$(jq -r '.FunctionSummary.FunctionMetadata.FunctionARN' /tmp/todero-pages-function-live.json)"
```

Create `cloudfront-config.json`:

```bash
export CALLER_REFERENCE="todero-pages-$(date +%s)"
...
aws cloudfront create-distribution \
  ...
  > /tmp/todero-pages-cloudfront.json

export DISTRIBUTION_ID="$(jq -r '.Distribution.Id' /tmp/todero-pages-cloudfront.json)"
export DISTRIBUTION_DOMAIN="$(jq -r '.Distribution.DomainName' /tmp/todero-pages-cloudfront.json)"
```
...
Create `todero-page-uploader-policy.json`. This policy supports collision
...
jq -n \
  ...
  }' > todero-page-uploader-policy.json
```
...
aws iam put-user-policy \
  --profile "$AWS_PROFILE" \
  --user-name "$UPLOADER_USER" \
  --policy-name ToderoPagePublisher \
  --policy-document file://todero-page-uploader-policy.json
```
...
aws iam create-access-key \
  --profile "$AWS_PROFILE" \
  --user-name "$UPLOADER_USER" \
  > /tmp/todero-page-uploader-key.json
chmod 600 /tmp/todero-page-uploader-key.json
```
...
```bash
export CF_ZONE_ID=<todero.ing-zone-id>
...
```bash
export PAPERCLIP_PAGE_AWS_ACCESS_KEY_ID="$(jq -r '.AccessKey.AccessKeyId' /tmp/todero-page-uploader-key.json)"
export PAPERCLIP_PAGE_AWS_SECRET_ACCESS_KEY="$(jq -r '.AccessKey.SecretAccessKey' /tmp/todero-page-uploader-key.json)"

npx todero secrets create \
  --company-id <company-id> \
  --name todero-page-aws-access-key-id \
  --value-env PAPERCLIP_PAGE_AWS_ACCESS_KEY_ID

npx todero secrets create \
  --company-id <company-id> \
  --name todero-page-aws-secret-access-key \
...
```json
{
  ...
  "PAPERCLIP_PAGE_BUCKET": { "type": "plain", "value": "todero-pages-prod" },
  "PAPERCLIP_PAGE_BASE_URL": { "type": "plain", "value": "https://pages.todero.ing" },
...
```bash
npx todero skills create \
  --company-id <company-id> \
  --name "Todero Page" \
  --slug todero-page \
  --description "Publish static pages to the Todero pages host" \
  --body-file .agents/skills/todero-page/SKILL.md
```

Attach it to an agent:

```bash
npx todero skills agent sync <agent-id-or-shortname> \
  --company-id <company-id> \
  --skill todero-page
```
...
aws iam create-access-key \
  --profile "$AWS_PROFILE" \
  --user-name "$UPLOADER_USER" \
  > /tmp/todero-page-uploader-key-rotation.json
chmod 600 /tmp/todero-page-uploader-key-rotation.json
```

2. Store new secret versions in Todero Secrets.
