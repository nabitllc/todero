...
test("extracts only pull requests from the requested repository", () => {
  assert.equal(extractPullRequestNumber("https://github.com/nabitllc/todero/pull/9507", "nabitllc/todero"), 9507);
  assert.equal(extractPullRequestNumber("github.com/nabitllc/todero/pull/9507", "nabitllc/todero"), 9507);
  assert.equal(extractPullRequestNumber("https://github.com/other/repo/pull/9507", "nabitllc/todero"), null);
});
...
test("origin selection prioritizes work products then comment mentions", () => {
  const issues = [
    ...
    },
    {
      ...
      workProducts: [{ type: "pull_request", url: "http://github.com/nabitllc/todero/pull/9507?source=todero#review" }],
    },
  ];
  assert.equal(chooseOriginatingIssue(issues, "https://github.com/nabitllc/todero/pull/9507").issueId, "origin");
});

function discoveryFixture() {
  let extractPath = "";
  const toderoGet = async (path) => {
        ...
        results: [
          {
            ...
            matches: [
              { value: "https://github.com/nabitllc/todero/pull/1", field: "comment", label: "Comment", source: { type: "comment", commentId: "c1" } },
              { value: "https://github.com/nabitllc/todero/pull/1", field: "document_body", label: "Document", source: { type: "document", documentId: "d1", documentKey: "plan" } },
              { value: "https://github.com/nabitllc/todero/pull/2", field: "description", label: "Description", source: { type: "issue", issueId: "issue-1" } },
              { value: "https://github.com/nabitllc/todero/pull/3", field: "description", label: "Description", source: { type: "issue", issueId: "issue-1" } },
              { value: "https://github.com/nabitllc/todero/pull/4", field: "comment", label: "Comment", source: { type: "comment", commentId: "c2" } },
            ],
          },
        ],
      };
    }
    return [{ type: "pull_request", url: "https://github.com/nabitllc/todero/pull/1/" }];
  };
  const ghJson = (args) => {
    ...
    return {
      number,
      url: `https://github.com/nabitllc/todero/pull/${number}`,
      title: `PR ${number}`,
  ...
  return { toderoGet, ghJson, extractPath: () => extractPath };
}
...
test("candidate discovery deduplicates mentions, drops closed PRs, and excludes community authors by default", async () => {
  ...
  const result = await findCandidates({
    ...
    api_url: "http://todero.test",
    api_key: "test-key",
    company_id: "company-1",
    now: "2026-07-20T00:00:00Z",
    todero_get: fixture.toderoGet,
  ...
  const query = new URL(`http://todero.test${fixture.extractPath()}`).searchParams;
...
test("capped match sets are recorded instead of aborting discovery", async () => {
  const fixture = discoveryFixture();
  const toderoGet = async (path) => {
    const page = await fixture.toderoGet(path);
...
test("summarizes PR bodies into a one-line purpose", () => {
  ...
  assert.equal(
    summarizePullRequestBody("> - Todero is the control plane.\n> - Blocker edges gate work."),
    "Todero is the control plane. Blocker edges gate work.",
  );
...
test("renders scope, purpose, confidence groups, and immutable guardrail", () => {
  const entry = {
    number: 1,
    url: "https://github.com/nabitllc/todero/pull/1",
    title: "Example",
    author: "cryppadotta",
  ...
  assert.match(report, /Scope: PRs authored by `cryppadotta` \(this Todero instance\) referenced by issues active in the last 14 day\(s\)/);
...
test("escapes contributor-controlled Markdown in report titles and purposes", () => {
  const report = renderReport({
    ...
    pullRequests: [
      {
        number: 2,
        url: "https://github.com/nabitllc/todero/pull/2",
