...
export function resolveAuthorAllowlist(options, getGhJson) {
  ...
  // Default to the authenticated gh identity: every PR this Todero instance
...
export async function toderoGet(path, { apiUrl, apiKey }) {
  const response = await fetch(`${apiUrl.replace(/\/$/, "")}/api${path}`, {
    headers: { Authorization: `Bearer ${apiKey}` },
  });
  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Todero GET ${path} failed (${response.status}): ${body}`);
  }
