...
export async function findCandidates(options) {
  const getTodero = options.todero_get ?? toderoGet;
  ...
  while (true) {
    ...
    const page = await getTodero(`/companies/${companyId}/search/extract?${query}`, { apiUrl, apiKey });
  ...
  const workers = Array.from({ length: Math.min(8, issueIds.length) }, async (_, workerIndex) => {
    for (let index = workerIndex; index < issueIds.length; index += 8) {
      ...
      const workProducts = await getTodero(`/issues/${issueId}/work-products`, { apiUrl, apiKey });
