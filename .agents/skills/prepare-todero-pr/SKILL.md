---
name: prepare-todero-pr
description: Prepare a Todero branch for PR with commits, template body, and checks.
---
# Prepare Todero PR

The standard Todero procedure for turning branch work into a reviewed,
green pull request against `nabitllc/todero` master. Apply it once per
...
* Do all PR work in a **git worktree** on a dedicated branch. The main
  `~/todero` checkout typically runs the live Todero server — never
...
* Confirm which remote/ref you are targeting (normally `master` on the
  `nabitllc/todero` repo; the task may name a specific remote such as
  `origin` or `public-gh`).
...
* Commit messages must end with exactly:
  `Co-Authored-By: Todero <noreply@todero.ing>`
...
* Follow `CONTRIBUTING.md` (repo root,
  https://github.com/nabitllc/todero/blob/master/CONTRIBUTING.md) for
