...
key: todero/optional/content/simplified-english
recommendedForRoles:
  - engineer
  - product
  - writer
...
- `pnpm --filter @todero/skills-catalog build:manifest` → regenerates
  `generated/catalog.json` with the new entry.
- Add `"todero/optional/content/simplified-english"` to
  `EXPECTED_OPTIONAL_KEYS` in `src/shipped-catalog.test.ts` (alphabetical).
- `pnpm --filter @todero/skills-catalog test` → green.
