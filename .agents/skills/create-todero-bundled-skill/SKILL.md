---
name: create-todero-bundled-skill
description: >
  Turn an idea, tweet, or task into a skill in the Todero skills catalog
...
# Create a Todero Bundled Skill

Take source material — a tweet, a task description, a blog post, "make a skill
that does X" — and land it as a skill in the Todero skills catalog
(`packages/skills-catalog/`), delivered as a reviewed PR. The catalog is the
shelf every Todero company browses and installs from, so the bar is: correct
...
- A human sends a tweet/link/idea and asks for it to become a Todero skill.
...
- You only need a repo-internal agent skill for working on Todero itself —
...
**Tweets / X links.** Use the `xc` CLI (X API client). Todero engineering
...
2. **Already in this repo?** Check `.agents/skills/`, `skills/`, and issue
   history (`gh search issues` / Todero board) for prior work on the topic.
...
- **kind**: default to `optional`. Use `bundled` only when the skill should
  ship to every Todero company by default — that needs explicit human/board
  direction, not your judgment call.
- **category**: reuse an existing directory when one fits (`browser`,
  `content`, `docs`, `finance`, `todero-operations`, `product`, `quality`,
...
key is `todero/<kind>/<category>/<slug>`.
...
```sh
pnpm --filter @todero/skills-catalog build:manifest   # regenerates generated/catalog.json
pnpm --filter @todero/skills-catalog validate         # must report no errors
```
...
```sh
pnpm --filter @todero/skills-catalog test
```
...
Follow the `prepare-todero-pr` skill (`.agents/skills/prepare-todero-pr/`)
against `nabitllc/todero` master. The diff should contain exactly:
