...
name: release
description: >
  Coordinate a full Todero release across engineering verification, npm,
...
Run the full Todero maintainer release workflow, not just an npm publish.
...
7. If running through Todero, you have issue context for status updates and follow-up task creation.
...
Todero now uses a commit-driven release model:
...
npm view todero@canary version
```
...
- release summary in Todero issue context
...
`PAPERCLIP_API_KEY`, and `PAPERCLIP_RUN_ID`. Include `X-Todero-Run-Id` on
...
POST /api/companies/:companyId/cases
{
  ...
  "summary": "Stable release content package for Todero vYYYY.MDD.P.",
  "status": "in_progress",
  "fields": {
    "schema_version": 1,
    "version": "vYYYY.MDD.P",
...
{
  "title": "Todero vYYYY.MDD.P release body",
  "format": "markdown",
  "body": "# Todero vYYYY.MDD.P\n\nRelease summary and links...",
...
- `blog_post`, key `todero-release:vYYYY.MDD.P:blog-post`, status
  `in_progress`, body document key `body`
- `tweet_storm`, key `todero-release:vYYYY.MDD.P:tweet-storm`, status
  `in_progress`, body document key `body`
