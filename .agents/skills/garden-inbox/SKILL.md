---
name: garden-inbox
description: Scan a Todero user's Mine inbox, classify reversible archive candidates, request checkbox confirmation, and archive only accepted selections. Use when asked to garden, clean up, prune, or tidy a Todero inbox without changing issues, branches, or workspaces.
---
...
The script posts sequential cards when a scan has more than 200 candidates. Re-running `confirm` with the same scan file is idempotent. Leave the driving issue in the waiting posture required by the surrounding Todero heartbeat workflow.
