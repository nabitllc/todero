...
description: >
  Generate the stable Todero release changelog at releases/vYYYY.MDD.P.md by
  reading commits, changesets, and merged PR context since the last stable tag.
---

# Release Changelog Skill

Generate the user-facing changelog for the **stable** Todero release.

## Versioning Model

Todero uses **calendar versioning (calver)**:
...
- The release **source** is the commit the newest `beta/v<beta-version>`
  ...
  npm view todero dist-tags   # the beta dist-tag names the version
...
```
- **Feature name** — Description. ([#123](https://github.com/nabitllc/todero/pull/123), @contributor1, @contributor2)
```
...
The opening line of the changelog must be an H1 of the format `# Todero {version}`
(no braces), e.g. `# Todero v2026.618.0`. Always include the `Todero ` prefix and
...
case when the run has Todero API context. Use `skills/todero/references/cases.md`
...
{
  "caseType": "release",
  "key": "todero-release:vYYYY.MDD.P",
  "title": "Todero vYYYY.MDD.P release",
  "summary": "Stable Todero release notes for vYYYY.MDD.P.",
  "status": "in_progress",
  "fields": {
    "schema_version": 1,
    "version": "vYYYY.MDD.P",
...
{
  "title": "Todero vYYYY.MDD.P changelog",
  "format": "markdown",
...
1. confirm the H1 heading is `# Todero {version}` (e.g. `# Todero v2026.618.0`) with the stable version only
