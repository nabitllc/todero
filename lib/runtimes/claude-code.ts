...
nohup bash -c 'CHILD='"$CHILD"'; LOG='"${JSON.stringify(opts.logFile).replace(/'/g, "'\\''")}"'; TASK_ID='"${JSON.stringify(opts.taskId ?? '').replace(/'/g, "'\\''")}"'; for i in $(seq 1 1080); do if ! kill -0 $CHILD 2>/dev/null; then echo "[spawn-exit] $(date -u +%FT%TZ) pid=$CHILD watcher_detected=true" >> "$LOG"; if [ -n "$TASK_ID" ]; then curl -s -X PATCH http://localhost:3000/api/issues -H "Content-Type: application/json" -d "{\"id\":\"$TASK_ID\",\"status\":\"open\",\"started_at\":null,\"heartbeat_at\":null,\"worked_by\":null,\"transitioned_by\":\"spawn-watcher\"}" -o /dev/null 2>/dev/null; echo "[spawn-exit] reset to open for $TASK_ID" >> "$LOG"; fi; exit 0; fi; sleep 5; done' >/dev/null 2>&1 </dev/null &
disown || true
`
