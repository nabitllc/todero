...
nohup bash -c 'CHILD='"$CHILD"'; LOG='"${JSON.stringify(opts.logFile).replace(/'/g, "'\\''")}"'; TASK_ID='"${JSON.stringify(opts.taskId ?? '').replace(/'/g, "'\\''")}"'; SUPA_URL=https://twthgapiouiqhavrcnry.supabase.co; SUPA_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR3dGhnYXBpb3VpcWhhdnJjbnJ5Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NDUzMTY3NiwiZXhwIjoyMDkwMTA3Njc2fQ.EyNdtvECdcHx3RuaizdfLGNRY4OJotzjE2QeOQ9Yf4Q; for i in $(seq 1 1080); do if ! kill -0 $CHILD 2>/dev/null; then echo "[spawn-exit] $(date -u +%FT%TZ) pid=$CHILD watcher_detected=true" >> "$LOG"; if [ -n "$TASK_ID" ]; then CURRENT_STATUS=$(curl -sf "$SUPA_URL/rest/v1/issues?id=eq.$TASK_ID&select=status" -H "apikey: $SUPA_KEY" -H "Authorization: Bearer $SUPA_KEY" 2>/dev/null | python3 -c "import json,sys; d=json.loads(sys.stdin.read()); print(d[0]['"'"'status'"'"'] if d else '"'"'unknown'"'"')" 2>/dev/null || echo unknown); if [ "$CURRENT_STATUS" = "in_progress" ]; then curl -s -X PATCH http://localhost:3000/api/issues -H "Content-Type: application/json" -d "{\"id\":\"$TASK_ID\",\"status\":\"open\",\"started_at\":null,\"heartbeat_at\":null,\"worked_by\":null,\"transitioned_by\":\"spawn-watcher\"}" -o /dev/null 2>/dev/null; echo "[spawn-exit] reset to open (was in_progress) for $TASK_ID" >> "$LOG"; else echo "[spawn-exit] skipping reset — status is $CURRENT_STATUS (agent completed) for $TASK_ID" >> "$LOG"; fi; fi; exit 0; fi; sleep 5; done' >/dev/null 2>&1 </dev/null &
disown || true
`
