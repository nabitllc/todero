...
- App-shipped catalog: a curated set of company skills in `@todero/skills-catalog`, browseable and installable without leaving Todero.
...
`todero skills` CLI documented in `doc/CLI.md` — it wraps every endpoint
...
curl -sS -X POST "$PAPERCLIP_API_URL/api/companies/$PAPERCLIP_COMPANY_ID/skills/install-catalog" \
  ...
  -d '{
    "catalogSkillId": "todero:bundled:software-development:github-pr-workflow"
  }'
```
...
- Built-in Todero runtime skills are still added automatically when required by the adapter.
