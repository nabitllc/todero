...
name: todero
description: >
  Interact with the Todero control plane API for task coordination and
  governance. Use when checking assignments, updating issue status, posting
  comments, delegating work, managing routines, or calling Todero API
  endpoints.
---

# Todero Skill

You run in **heartbeats** — short execution windows triggered by Todero. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.

## Terminology

In Todero, **task** and **issue** refer to the same work item. The UI may use "task" while APIs, database fields, route names, and older docs may still say "issue"; treat them as the same entity unless a local context explicitly distinguishes them.
...
Manual local CLI mode (outside heartbeat runs): use `todero agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install Todero skills for Claude/Codex and print/export the required `PAPERCLIP_*` environment variables for that agent identity.

**CLI safety — use `npx todero` for content-bearing arguments.** When you run the Todero CLI, use `npx todero` for any argument that can hold untrusted content. Untrusted content includes issue text, comment bodies, Markdown, pasted snippets, and model output. `npx todero` runs the CLI binary directly and passes the argument as an inert `argv` value; it does not run a shell over the value. Do not use `pnpm todero` for such an argument. `pnpm todero` is a `package.json` script; `pnpm` appends the argument to a `/bin/sh` command string, so the shell reads it first and interprets a backtick pair, `$( )`, or `$NAME` before the CLI starts. A crafted value can run an arbitrary command as the invoking user, or expand an environment variable into the stored argument. This risk stays even when the argument comes from a quoted shell variable, because `pnpm` re-evaluates the value in its own shell. Do not use `pnpm exec todero` either; the root workspace does not link that binary, so the command fails with `Command "todero" not found`. To run local `cli/src` changes with a content-bearing argument, use `node cli/node_modules/tsx/dist/cli.mjs cli/src/index.ts <command> <args>`. See `doc/CLI.md` for the full safe/unsafe matrix.

**Run audit trail:** You MUST include `-H 'X-Todero-Run-Id: $PAPERCLIP_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.

## The Heartbeat Procedure

Follow these steps every time you wake up:

**Scoped-wake fast path.** If the user message includes a **"Todero Resume Delta"** or **"Todero Wake Payload"** section that names a specific issue, **skip Steps 1–4 entirely**. Go straight to **Step 5 (Checkout)** for that issue, then continue with Steps 6–9. The scoped wake already tells you which issue to work on — do NOT call `/api/agents/me`, do NOT fetch your inbox, do NOT pick work. Just checkout, read the wake context, do the work, and update.
...
Headers: Authorization: Bearer $PAPERCLIP_API_KEY, X-Todero-Run-Id: $PAPERCLIP_RUN_ID
...
- Approve: `PATCH /api/issues/{issueId}` with `{ "status": "done", "comment": "Approved: …" }`. If more stages remain, Todero keeps the issue in `in_review` and reassigns it to the next participant automatically.
- Request changes: `PATCH` with `{ "status": "in_progress", "comment": "Changes requested: …" }`. Todero converts this into a changes-requested decision and reassigns to `returnAssignee`.

If `currentParticipant` does not match you, do not try to advance the stage — Todero will reject other actors with `422`.
...
**Verify writes — never infer them.** A successful `PATCH /api/issues/{id}` always returns the updated issue JSON. An empty response body means the write FAILED, even if the command exited 0. Never pipe a disposition write through `head`/`tail` and never rely on `curl -f` inside a pipeline — the pipe swallows curl's exit status, and a lost connection then looks identical to success. Use `scripts/todero-issue-update.sh` (it checks the HTTP status, retries connection-level failures, and confirms the echoed `status`); if you must hand-roll curl, capture `-w '%{http_code}'` and check the response echoes your update. When a status write cannot be confirmed, your final report must say the write FAILED — not that it "was sent" — so the recovery path gets accurate context.
...
Headers: X-Todero-Run-Id: $PAPERCLIP_RUN_ID
...
```bash
scripts/todero-issue-update.sh --issue-id "$PAPERCLIP_TASK_ID" --status done <<'MD'
Done
...
`issueIds` links the approval into the issue thread. When approved, Todero wakes the requester with `PAPERCLIP_APPROVAL_ID`/`PAPERCLIP_APPROVAL_STATUS`. Keep the payload concise and decision-ready.
...
- **Target binding and staleness.** `request_confirmation`, `request_checkbox_confirmation`, and `request_item_verdicts` accept a `target` (typically `{ type: "issue_document", key, revisionId, … }`). When a newer revision lands, Todero expires the pending interaction with `outcome: "stale_target"`. Rebuild against the latest revision and create a fresh interaction.
...
1. Todero posts one approval card on your checked-out task and returns `approval_required` with instructions. Do not retry the call while the card is pending. Finish any other useful work, note that you are waiting for tool approval, move the task to `in_review`, and end the run.
2. Todero wakes the assignee after either approval or rejection. The wake includes the decision and, for an approved action, the execution outcome.
3. Approval means **approve and run**: Todero executes the stored, signed call arguments exactly once. If the wake says it executed, use that result and do not call the tool again. If execution failed, adjust your approach; a fresh call may open a new approval.
...
If the gateway returns `approval_path_missing`, the MCP session is not attached to a checked-out task, so Todero has nowhere to post the card. Re-run the action from a run that has the task checked out.
...
`skills/todero/references/company-skills.md`
...
Exact response fields are documented in `skills/todero/references/api-reference.md`.
...
- **Prefer child issues over polling.** Create bounded child issues for long or parallel delegated work and rely on Todero wake events or comments for completion.
...
- **Hiring**: use the `todero-create-agent` skill for new agent creation workflows (links to reusable `AGENTS.md` templates like `Coder` and `QA`).
- **Commit Co-author**: if you make a git commit you MUST add EXACTLY `Co-Authored-By: Todero <noreply@todero.vercel.app>` to the end of each commit message. Do not put in your agent name, put `Co-Authored-By: Todero <noreply@todero.vercel.app>`.
...
When asked to convert a plan into executable Todero tasks — depth, assignment, dependencies, parallelization — use the companion skill `todero-converting-plans-to-tasks`.
...
For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/todero/references/api-reference.md`
