...
// These are pure canvas drawing functions with no React dependencies.

import {
  MAP_COLS, MAP_ROWS, STANCHION_R, ORCHESTRATOR_ID,
  ALL_AGENTS, ACTIVE_IDS, BENCH_POS, DESK_POS, DEPENDENCIES,
  CONF_TX, CONF_TY, CONF_TW, CONF_TH, COL_X, ROW_Y
...
export type { AgentRunInfo as AgentRunInfoType };
...
export function applyCamera(ctx: CanvasRenderingContext2D, cam: any) {
...
export function drawFloor(
  ...
  for (let r = 0; r < MAP_ROWS; r++) for (let c = 0; c < MAP_COLS; c++) {
    ...
    ctx.fillStyle = hold
      ? ((c + r) % 2 === 0 ? thm.floorHold : thm.floorHoldB)
      : ((c + r) % 2 === 0 ? thm.floorA : thm.floorB);
  ...
  for (let c = 0; c < MAP_COLS; c += 4) {
    ...
    ctx.fillRect(px - T * 0.045, sy - T * 0.32, T * 0.09, T * 0.64);
    ctx.beginPath(); ctx.arc(px, sy - T * 0.32, T * 0.07, 0, Math.PI * 2); ctx.fill();
  }
...
export function drawFurniture(
  ...
  thm: typeof THEMES.A,
  liveRuns: Record<string, AgentRunInfo> = {}
) {
  ctx.save(); applyCamera(ctx, cam);
  ...
  if (showDepGraph) {
    Object.entries(DEPENDENCIES).forEach(([srcId, dstIds]) => {
      const src = agents.find((a: any) => a.id === srcId); if (!src || !src.active) return;
      (dstIds as string[]).forEach(dstId => {
        const dst = agents.find((a: any) => a.id === dstId); if (!dst || !dst.active) return;
  ...
  critPairs.forEach(([srcId, dstId]: any) => {
    const src = agents.find((a: any) => a.id === srcId), dst = agents.find((a: any) => a.id === dstId);
    if (!src || !dst) return;
  ...
  // ── Orchestrator desk ──
  const orchAg = agents.find((a: any) => a.id === ORCHESTRATOR_ID);
  const othersWorking2 = agents.filter((a: any) => a.id !== ORCHESTRATOR_ID && a.active && a.state === "working");
  if (orchAg && orchAg.state === "working" && othersWorking2.length >= 1) {
    const cx = CONF_TX * T + T * 1.1, cy = CONF_TY * T + T * 1.0;
  ...
  {
    const x = CONF_TX * T, y = CONF_TY * T, dw = T * 2.2, dh = T * 2.0;
  ...
  ACTIVE_IDS.filter(id => id !== ORCHESTRATOR_ID).forEach(id => {
    ...
    const ag = agents.find((a: any) => a.id === id); if (!ag) return;
    ...
    if (working) {
      ...
      if (run && run.status !== 'never') {
        ...
        if (run.todayErrors > 0) { ctx.fillStyle = "#ff4444"; ctx.fillText(`${run.todayErrors} err`, mx2 + dw * 0.04, my2 + dh * 0.55); }
      } else {
  ...
  const chairs = [
    ...topBottom.map(fx => ({ fx, fy: -0.14 })),
    ...topBottom.map(fx => ({ fx, fy: 1.10 })),
    ...leftRight.map(fy => ({ fx: -0.12, fy })),
    ...leftRight.map(fy => ({ fx: 1.08, fy })),
  ];
  ...
  if (activeMeeting && topic) {
    ...
    const meetingAgents = agents.filter((a: any) => a.state === "meeting" || a.state === "moving_to_meeting");
    const pNames = meetingAgents.map((a: any) => a.name).join(", ");
    if (pNames) {
    ...
    ctx.fillStyle = "#ff4444"; ctx.fillText("🚨 INCIDENT", tcx + tw / 2, tcy + th + T * 0.3);
  } else {
...
export function drawAgent(
  ...
  darkAlpha: number,
  incidentActive: boolean,
  boardTasksMap: Record<string, string> = {},
  ...
  if (state === "idle" && active && ag.lastStateChange) {
    ...
    if (idleMins >= 1) {
      ...
      ctx.fillStyle = "#4a4a6a"; ctx.fillText(`idle ${idleText}`, px, py + hs + T * 0.35 + dy2 + oy);
    }
  }
  if (agentCost > 0 && active) {
    ...
    ctx.fillStyle = "#4a6a5a"; ctx.fillText(`💰 ${costText}`, px, py + hs + T * 0.48 + dy2 + oy);
  }
  if (ag.id === ORCHESTRATOR_ID && subagentCount > 0) {
    ...
    const bh = bPx * 1.6;
    ctx.fillStyle = '#0d1a0d'; ctx.strokeStyle = '#00ff8899'; ctx.lineWidth = T * 0.015;
    ctx.beginPath(); ctx.roundRect(badgeX - bw / 2, badgeY - bh / 2, bw, bh, T * 0.03); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#00ff88'; ctx.textAlign = 'center'; ctx.fillText(label, badgeX, badgeY + bh * 0.28);
  }
  ctx.globalAlpha = 1; ctx.restore();
}
...
export function drawMinimap(
  ...
  agents.forEach((a: any) => {
    const ax = mmX + a.px * sx, ay = mmY + a.py * sy, r = 3;
  ...
  if (showLegend) {
    const items: [string, string][] = [["#00ff88", "Working"], ["#FDCB6E", "Meeting"], ["#4a5568", "Idle"]];
