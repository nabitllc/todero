...
For `claude_local` and `codex_local`, Todero assumes the CLI is already installed and authenticated on the host machine.
...
Todero stores resumable session state per `(agent, taskKey, adapterType)`.
