...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Todero plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Todero.
...
- package: `@todero/plugin-kitchen-sink-example`
- plugin id: `todero.kitchen-sink-example` or `todero-kitchen-sink-example`

Recommendation: use `todero-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Todero event such as `issue.created`
...
Recommendation: default to a Todero-local endpoint or a stable public echo endpoint to avoid flaky docs.
