...
Turn the `agent-os` research into a low-risk Todero execution plan that preserves Todero's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Todero should not absorb `agent-os` as a product model or orchestration layer.

Todero should evaluate `agent-os` in three narrow areas:
...
This work is successful when Todero has:
...
- a proposed runtime capability model that fits current Todero adapters
...
- replace Todero heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Todero core

## Existing Todero integration points
...
- prove that Todero can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Todero's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Todero-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Todero workloads
...
- identify where Todero should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Todero
...
- runtime experimentation could blur the boundary between Todero as control plane and Todero as execution platform
