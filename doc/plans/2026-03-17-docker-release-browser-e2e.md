...
Today release smoke testing for published Todero packages is manual and shell-driven:
...
- runs `todero@${TODEROAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@todero.local`
- password: `todero-smoke-password`
...
- the suite passes locally against both:
  - `TODEROAI_VERSION=canary`
  - `TODEROAI_VERSION=latest`
...
1. A published `todero@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `todero@latest` Docker install can be smoke-tested by Playwright in CI.
