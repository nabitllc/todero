...
This document defines the next-stage plan for Todero company import/export.

The core shift is:

- move from a Todero-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Todero.
...
Todero already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Todero
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `todero.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.todero.yaml` sidecar for high-fidelity Todero-specific details
...
11. adapter-aware skill sync surfaces so Todero can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Todero at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Todero sidecar.
...
Todero must not redefine `SKILL.md`.
...
- Todero-specific fidelity lives in `.todero.yaml`
- Todero may resolve and install `SKILL.md` packages, but it must not require a Todero-only skill format
...
### 5.4 Base Package Vs Todero Extension
...
- base package:
  ...
  - no Todero-only runtime fields by default
- Todero extension:
  - `.todero.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Todero tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`todero.manifest.json` is not part of the future package direction.
...
Todero import/export should support these entity kinds:
...
- Todero can treat “this set of agents came from team package X” as the imported-team model
...
Todero should support source references in package metadata with:
...
Todero should define a new adapter surface area around skills:
...
Planned Todero behavior:

- if an adapter supports read, Todero should show current skills in the UI
- if an adapter supports write, Todero should let the user enable/disable imported skills
- if an adapter supports sync, Todero should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.todero.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.todero.yaml` alongside the base package
...
## 10. Storage Model Inside Todero
...
Todero should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `todero company export <company-id> --out <path>`
- `todero company import <path-or-url> --dry-run`
- `todero company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Todero storage, or only referenced at import time?
...
3. Should Todero support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Todero aligned with:
