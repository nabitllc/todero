...
This document defines the product and UI plan for skill management in Todero.
...
- Todero company import/export can include skills as package content
...
3. The UI must always tell the truth about what Todero knows:
   - desired state in Todero
...
Todero should treat skills at two scopes:
...
- created locally inside Todero later
...
7. “Understand what Todero knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Todero-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Todero did not install.

These should be clearly marked:

- external
- not managed by Todero
...
- `desired_only`: Todero wants it, adapter does not show it yet
- `external`: adapter has it but Todero does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Todero only tracks desired state
...
- Todero can still track desired skills
...
- `.todero.yaml` may add Todero-specific fidelity, but should not replace the base shortname association model
...
- `.todero.yaml` may add Todero-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Todero should have a company-scoped skill model or managed package model representing:
...
Todero should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Todero-specific.
...
That gives Todero one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
