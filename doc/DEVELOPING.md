...
`pnpm dev` and `pnpm dev:once` are now idempotent for the current repo and instance: if the matching Todero dev runner is already alive, Todero reports the existing process instead of starting a duplicate.
...
```sh
pnpm dev --data-dir ./tmp/todero-dev
```
...
```sh
pnpm dev:list --data-dir ./tmp/todero-dev
pnpm dev:stop --data-dir ./tmp/todero-dev
```
...
These run the `@todero/ui` Storybook on port `6006` and build the static output to `ui/storybook-static/`.
...
or inject external font CSS to make Todero text render correctly.

Font assets live in Vite's public directory so `pnpm --filter @todero/ui build`
...
Primary-instance rebuilds that restart `todero.service` can request one-shot live-run adoption instead of using the normal graceful shutdown drain. Before restarting the service, write the marker from the newly staged app with the current service PID:

```sh
old_main_pid="$(systemctl show todero.service -p MainPID --value)"
pnpm --filter @todero/server exec tsx ../scripts/request-hot-restart.ts --server-pid "$old_main_pid"
systemctl restart todero.service
```
...
the process now using its PID. Todero refuses to create a new request without
...
When Todero manages embedded PostgreSQL, it suppresses that dependency's eager
`SIGINT`/`SIGTERM` cleanup hooks. Todero owns signal ordering so the heartbeat
...
```sh
old_main_pid="$(systemctl show todero.service -p MainPID --value)"
pnpm --filter @todero/server exec tsx ../scripts/request-hot-restart.ts \
...
>
> The first-run experience often starts with `npx todero onboard --yes` (before you have a repo checkout). If your global `~/.npmrc` sets `registry` to a private registry (for example GitHub Packages), `npx` may try to resolve `todero` from that private registry and fail with `E404`.
>
> Diagnostic:
>
> ```sh
...
2. `todero doctor` with repair enabled
3. starts the server when checks pass

## Docker Quickstart (No local Node install)

Build and run Todero in Docker:
...
```text
~/.todero/instances/default/                  # instance root
...
`PAPERCLIP_HOME` and `PAPERCLIP_INSTANCE_ID` override the home root and instance id respectively. `todero onboard` echoes the resolved values in its banner (`Local home: <home> | instance: <id> | config: <path>`) so you can confirm where state will land before continuing.
...
- `~/.todero/instances/default/data/storage`
...
Use the helper bundled with the Todero skill from the repo root:

```sh
skills/todero/scripts/todero-upload-artifact.sh dist/demo.mp4 \
...
```sh
skills/todero/scripts/todero-upload-artifact.sh out/walkthrough.webm \
...
When a local agent run has no resolved project/session workspace, Todero falls back to an agent home workspace under the instance root:

- `~/.todero/instances/default/workspaces/<agent-id>`

This path honors `PAPERCLIP_HOME` and `PAPERCLIP_INSTANCE_ID` in non-default setups.

For `codex_local`, Todero assigns new and updated agents an isolated Codex home under the instance root and blocks shared host/company Codex homes:

- `~/.todero/instances/default/companies/<company-id>/agents/<agent-id>/codex-home`

Todero also persists an empty `OPENAI_API_KEY` override for those agents so a host-level `OPENAI_API_KEY` cannot leak into Codex runs through process inheritance. If an operator explicitly configures `adapterConfig.env.CODEX_HOME`, it must not point at the shared company `codex-home`, `$CODEX_HOME`, or `~/.codex`.

If the `codex` CLI is not installed or not on `PATH`, `codex_local` agent runs fail at execution time with a clear adapter error. Quota polling uses a short-lived `codex app-server` subprocess: when `codex` cannot be spawned, that provider reports `ok: false` in aggregated quota results and the API server keeps running (it must not exit on a missing binary).

Local adapters require their corresponding CLI/session setup on the machine running Todero. External adapters are installed through the adapter/plugin flow and should not require hardcoded imports in `server/` or `ui/`.
...
When developing from multiple git worktrees, do not point two Todero servers at the same embedded PostgreSQL data directory.

Instead, create a repo-local Todero config plus an isolated instance for the worktree:

```sh
todero worktree init
# or create the git worktree and initialize it in one step:
npx todero worktree:make todero-pr-432
```

This command:

- writes repo-local files at `.todero/config.json` and `.todero/.env`
- creates an isolated instance under `~/.todero-worktrees/instances/<worktree-id>/`
- when run inside a linked git worktree, mirrors the effective git hooks into that worktree's private git dir
- picks a free app port and embedded PostgreSQL port
- disables automatic database backups for the isolated instance
- by default seeds the isolated DB in `minimal` mode from the current effective Todero instance/config (repo-local worktree config when present, otherwise the default instance) via a logical SQL snapshot
...
Seeded worktree instances quarantine copied live execution by default for both `minimal` and `full` seeds. During restore, Todero disables copied agent timer heartbeats, resets copied `running` agents to `idle`, blocks and unassigns copied agent-owned `in_progress` issues, and unassigns copied agent-owned `todo`/`in_review` issues. This keeps a freshly booted worktree from starting agents for work already owned by the source instance. Pass `--preserve-live-work` only when you intentionally want the isolated worktree to resume copied assignments.

The same quarantine stops copied project/execution-workspace runtime desired states and clears copied runtime process claims. Without this reset, booting the cloned Todero database could restart a source workspace's dev service from the isolated instance, creating duplicate runners, port reassignment, and stale public URLs.

After `worktree init`, both the server and the CLI auto-load the repo-local `.todero/.env` when run inside that worktree, so normal commands like `pnpm dev`, `todero doctor`, and `todero db:backup` stay scoped to the worktree instance.

`pnpm dev` now fails fast in a linked git worktree when `.todero/.env` is missing, instead of silently booting against the default instance/port. If that happens, run `todero worktree init` in the worktree first.
...
- `pnpm todero worktree ensure-seeded` performs the deferred seed **exactly once**. It is lock-guarded and idempotent: only a complete `verified` manifest short-circuits it, so it is safe to call repeatedly and from concurrent processes. Managed workspaces derive the source from the control-plane-provided base project workspace when it carries its own `.todero/config.json`, and otherwise from the control plane's own registered instance config; either way the workspace's manifest never selects it. Manual worktrees must pass `--from-config`.
- `todero run` calls `ensureWorktreeSeeded` automatically before doctor/boot. Managed runs transparently seed a lean worktree from their registered base workspace; an unmanaged lean worktree must first run `worktree ensure-seeded --from-config <source-config>`.
- Managed Todero git worktrees default to the repository's `scripts/provision-worktree.sh` when the strategy omits `provisionCommand`, so the isolated config and pending manifest cannot be silently skipped. Runtime startup also runs `scripts/provision-worktree-runtime.sh` automatically when no explicit runtime provision command is configured and the manifest is not verified. Explicitly configured provision commands still take precedence.
- The built-in deferred seed is recorded as its own terminal `workspace_seed` operation. A zero exit code is not enough for success: the operation succeeds only when `.todero/seed-manifest.json` contains complete verified evidence; failed, missing, or malformed manifests produce a failed operation with the seed phase in metadata.
- Worktrees created before lazy seeding shipped may have neither marker. Todero adopts them only after their configured database proves a compatible migration journal and the core Todero schema; otherwise managed startup creates a pending manifest and performs the normal verified seed. Manual markerless worktrees must provide `--from-config` so the source remains explicit.

Both `minimal` and `full` modes use the same terminal data-validation contract. Source validation accepts a migration journal that is a prefix of the checkout's journal and records the source revision in seed diagnostics; it rejects a source that is ahead of the checkout because that would require a downgrade. After restore, Todero applies pending migrations and requires the target journal to be current. Both validations also read an auth user with an instance administrator role, active company membership, and representative cloned company/issue pair. Authenticated instances additionally require that administrator to have a non-empty credential account. `local_trusted` instances accept the implicit local Board user without an account row because that mode intentionally has no human login flow. Restore, migrations, execution quarantine, routine pausing, workspace rebinding, and post-restore validation all run under the seed lock. An interruption leaves the exact active phase in terminal `failed` state; it cannot produce readiness evidence.
...
When Todero closes a server-managed git worktree, it also reclaims the isolated instance referenced by that worktree's repo-local `.todero/.env`. New server-managed worktrees use a collision-resistant instance id derived from the resolved absolute worktree path, and Todero persists the resulting instance root as execution-workspace ownership metadata. Cleanup requires the env pointer to match that persisted root, stops a running embedded PostgreSQL process, and then removes the instance directory. The deletion guard only accepts canonical instance paths below `PAPERCLIP_WORKTREES_DIR/instances/`; legacy or mismatched ownership, pointers to the default/live Todero home, and all other locations are logged and left untouched.
...
**`npx todero worktree init [options]`** — Create repo-local config/env and an isolated instance for the current worktree.

| Option | Description |
|---|---|
| `--name <name>` | Display name used to derive the instance id |
| `--instance <id>` | Explicit isolated instance id |
| `--home <path>` | Home root for worktree instances (default: `~/.todero-worktrees`) |
...
```sh
todero worktree init --no-seed
todero worktree init --seed-mode full
todero worktree init --from-instance default
todero worktree init --from-data-dir ~/.todero
todero worktree init --force
```
...
```sh
cd /path/to/todero/.todero/worktrees/PAP-884-ai-commits-component
npx todero worktree init --force --seed-mode minimal \
  --name PAP-884-ai-commits-component \
  --from-config ~/.todero/instances/default/config.json
```

That rewrites the worktree-local `.todero/config.json` + `.todero/.env`, recreates the isolated instance under `~/.todero-worktrees/instances/<worktree-id>/`, and preserves the git worktree contents themselves.
...
**`npx todero worktree repair [options]`** — Repair the current linked worktree by default, or create/repair a named linked worktree under `.todero/worktrees/` when `--branch` is provided. The command never targets the primary checkout unless you explicitly pass `--branch`.

| Option | Description |
|---|---|
| `--branch <name>` | Existing branch/worktree selector to repair, or a branch name to create under `.todero/worktrees` |
| `--home <path>` | Home root for worktree instances (default: `~/.todero-worktrees`) |
| `--from-config <path>` | Source config.json to seed from |
| `--from-data-dir <path>` | Source `PAPERCLIP_HOME` used when deriving the source config |
...
```sh
# From inside a linked worktree, rebuild missing .todero metadata and reseed it from the default instance.
cd /path/to/todero/.todero/worktrees/PAP-1132-assistant-ui-pap-1131-make-issues-comments-be-like-a-chat
pnpm todero worktree repair

# From the primary checkout, create or repair a linked worktree for a branch under .todero/worktrees/.
...
For an already-created worktree where you want to keep the existing repo-local config/env and only overwrite the isolated database, use `worktree reseed` instead. Stop the target worktree's Todero server first so the command can replace the DB safely.

**`npx todero worktree reseed [options]`** — Re-seed an existing worktree-local instance from another Todero instance or worktree while preserving the target worktree's current config, ports, and instance identity.
...
**`npx todero worktree:make <name> [options]`** — Create `~/NAME` as a git worktree, then initialize an isolated Todero instance inside it. This combines `git worktree add` with `worktree init` in a single step.
...
```sh
npx todero worktree:make todero-pr-432
npx todero worktree:make my-feature --start-point origin/main
npx todero worktree:make experiment --no-seed
```

**`npx todero worktree env [options]`** — Print shell exports for the current worktree-local Todero instance.
...
Heavier setup that is only needed by a managed runtime service can use `workspaceStrategy.runtimeProvisionCommand`. Todero runs this command lazily before spawning the first service in a start batch, serializes concurrent provisioning for the same workspace, and records the attempt as `workspace_runtime_provision`. The command receives the same workspace environment as `provisionCommand` and should be idempotent because later service-start batches invoke it again.
...
In Vite middleware mode, Todero gives HMR a dedicated HTTP server bound to the managed runtime's loopback host. The browser still derives the HMR hostname from the public HTTPS page, so listener containment does not break remote hot reload.

When a workspace service runs Todero for browser OAuth QA, configure its `expose.urlTemplate` with the canonical URL the browser can reach. Todero preserves explicit `PAPERCLIP_PUBLIC_URL` or `BETTER_AUTH_URL` settings; otherwise it uses a valid exposed HTTPS origin (or loopback HTTP) as the managed runtime fallback for Better Auth and `/api/tools/oauth/callback`. Internal service names such as `http://todero-dev:<port>` are rejected unless that hostname is genuinely the browser route. Use a unique origin per isolated worktree. See [Execution Workspaces And Runtime Services](../docs/guides/board-operator/execution-workspaces-and-runtime-services.md#browser-reachable-origins-for-oauth-qa) for configuration and verification.

## App-Shipped Skills Catalog

The Todero app ships a curated catalog of company skills out of the box. The
...
paths at request time. Root `skills/` remains reserved for Todero runtime
skills and is not part of the catalog.

Skill-capable legacy local adapters always select the bundled
`nabitllc/todero/todero` operational skill when it is present in the
...
```sh
pnpm --filter @todero/skills-catalog validate
```
...
```sh
pnpm --filter @todero/skills-catalog build:manifest
```

The package's `build` script runs `build:manifest` and then `tsc`; tests live
under `pnpm --filter @todero/skills-catalog test`. Validation fails when:
...
CI runs `pnpm --filter @todero/skills-catalog validate` and the package's
...
```sh
pnpm --filter @todero/teams-catalog validate
```

Regenerate `generated/catalog.json` after editing catalog team files:

```sh
pnpm --filter @todero/teams-catalog build:manifest
```
...
Todero can run automatic logical database backups on a timer. These backups cover
...
- every 60 minutes
- retain 30 days
- backup dir: `~/.todero/instances/default/data/backups`

Automatic backups are disabled for isolated worktree instances created with
`todero worktree init` or `todero worktree:make`. Existing worktree
...
```sh
pnpm todero configure --section database
```

Run a one-off backup manually:

```sh
pnpm todero db:backup
# or:
pnpm db:backup
```
...
- Default local key path: `~/.todero/instances/default/secrets/master.key`
...
- `pnpm todero onboard` writes a default `secrets` config section (`local_encrypted`, strict mode off, key file path set) and creates a local key file when needed.
- `pnpm todero configure --section secrets` lets you update provider/strict mode/key path and creates the local key file when needed.
- `pnpm todero doctor` validates secrets adapter configuration, can create a missing local key file with `--repair`, and reports missing AWS Secrets Manager bootstrap env when that provider is selected.
...
Todero CLI now includes client-side control-plane commands in addition to setup commands.

Quick examples:

```sh
npx todero issue list --company-id <company-id>
npx todero issue create --company-id <company-id> --title "Investigate checkout conflict"
npx todero issue update <issue-id> --status in_progress --comment "Started triage"
```
...
```sh
pnpm todero issue list
pnpm todero dashboard get
```
...
- `GET /api/skills/todero` returns the Todero heartbeat skill markdown.
...
The PostHog smoke targets an already-running authenticated Todero instance.
...
pnpm smoke:posthog-live https://todero.example.test
```

The command fails before browser launch unless all three integration bindings
are present, and never prints their values. A Todero heartbeat derives the
...
screenshots only after OAuth returns to Todero, enables read actions only,
...
Run the focused harness checks without contacting Todero or PostHog:
...
- defaults to isolated config dir `~/.openclaw-todero-smoke`
- resets smoke agent state each run by default (`OPENCLAW_RESET_STATE=1`) to avoid stale provider/auth drift

Networking behavior for this smoke script:

- auto-detects and prints a Todero host URL reachable from inside OpenClaw Docker
- default container-side host alias is `host.docker.internal` (override with `PAPERCLIP_HOST_FROM_CONTAINER` / `PAPERCLIP_HOST_PORT`)
- if Todero rejects container hostnames in authenticated/private mode, allow `host.docker.internal` via `npx todero allowed-hostname host.docker.internal` and restart Todero
