...
This document covers the GitHub and npm setup required for the current Todero release model:
...
Do this for every public package that Todero publishes.
...
3. add a trusted publisher for the GitHub repository `nabitllc/todero`
...
2. open that package on npm and add the `nabitllc/todero` trusted publisher for `.github/workflows/release.yml`
...
2. revoke any personal automation token that used to publish Todero
...
```bash
npm install --prefix "$(mktemp -d)" todero@canary --no-audit --no-fund
```

The release script runs this clean-prefix install after publishing every workspace
package dependency-first and publishing `todero` last. A package that is not
