...
must be attached to the Todero issue before the agent chooses a final
...
```sh
skills/todero/scripts/todero-upload-artifact.sh path/to/output.webm \
  --title "Walkthrough render" \
  --summary "Rendered walkthrough for review"
```

The helper uses the authenticated Todero API from the current heartbeat
environment:
...
`artifact` and `provider` to `todero`, with metadata canonicalized from the
...
Workspace file links resolve only inside registered Todero workspaces. The
...
when it carries both that `projectId` and `workspaceId`. Todero does not
...
2. Upload it with `skills/todero/scripts/todero-upload-artifact.sh`.
...
```sh
skills/todero/scripts/todero-upload-artifact.sh render.bin \
  --title "Demo video render" \
...
Use `type: "artifact"`, `provider: "todero"`, and metadata containing the
