...
| Subject | The app/workspace or Todero user on whose behalf a credential is requested. | The calling agent. |
...
| Trigger | Provider-origin event definition that starts governed Todero work. | An unauthenticated webhook handler. |
| Connector service | Todero-operated relay for managed OAuth callbacks, credential custody, and webhook intake at `connect.todero.vercel.app`. | Todero ID or the per-company broker. |
...
| Skill | Agent instruction package that calls governed connections through Todero. |
...
- Inbound clients use scoped Todero auth; outbound vendor calls use the Apps
