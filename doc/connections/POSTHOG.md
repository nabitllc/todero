...
Todero connects to PostHog's hosted MCP service at
...
- a PostHog personal API key stored as a Todero secret and sent as an
...
PostHog OAuth or an API key managed directly by Todero.

Todero does not silently fall back from OAuth to an API key. The selected
...
through browser sign-in — with no Todero-specific code involved. See
[Connecting any remote MCP server](./GENERIC-REMOTE-MCP.md).

## Service involvement

PostHog hosts both the MCP resource and OAuth authorization service. Todero
...
`/api/tools/oauth/callback`. No Todero-operated vendor relay is involved.

```mermaid
sequenceDiagram
    actor A as Administrator
    participant P as Todero
...
| Todero callback | `/api/tools/oauth/callback` |

Redirect-URI constraints and token lifetimes remain provider-controlled and
must be rechecked during credentialed QA; Todero does not encode guessed
values for either.
...
5. The default setup requests all feature groups and tools. Todero fixes
...
6. For OAuth, continue through browser consent. For API-key setup, create a
   ...
   Todero. Never put the key in connection configuration or a URL.
...
When configured, Todero sends the optional project pin as the
...
capability. Todero sends configured `readonly`, `features`, `tools`, and
