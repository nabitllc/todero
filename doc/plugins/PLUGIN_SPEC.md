# Todero Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Todero's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Todero app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Todero HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@todero/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
Todero plugin design is based on the following assumptions:

1. Todero is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Todero business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Todero core.
...
2. Let plugins add major capabilities without editing Todero core.
...
4. Depend on project-local plugin folders such as `.todero/plugins`.
...
The single Todero deployment an operator installs and controls.

### 5.2 Company

A first-class Todero business object inside the instance.
...
A trusted in-process extension loaded directly by Todero core.
...
An installable instance-wide extension package loaded through the Todero plugin runtime.
...
Todero has two extension classes.
...
Todero already has a concrete workspace model:
...
Plugins live under the Todero instance directory.

Suggested layout:

- `~/.todero/instances/default/plugins/package.json`
- `~/.todero/instances/default/plugins/node_modules/`
- `~/.todero/instances/default/plugins/.cache/`
- `~/.todero/instances/default/data/plugins/<plugin-id>/`
...
Todero should add CLI commands:

- `pnpm todero plugin list`
- `npx todero plugin install <package[@version]>`
- `npx todero plugin uninstall <plugin-id>`
- `npx todero plugin upgrade <plugin-id> [version]`
- `npx todero plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@todero/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
Plugins may contribute tools that Todero agents can use during runs.
...
- Todero server starts one worker process per installed plugin
...
Receives one typed Todero domain event.
...
Trusted orchestration plugins can create and update Todero issues through `ctx.issues` instead of importing server internals. The public issue contract includes parent/project/goal links, board or agent assignees, blocker IDs, labels, billing code, request depth, execution workspace inheritance, and plugin origin metadata.

Plugins that perform durable work should declare managed Todero resources rather than using private plugin state:
...
export function definePlugin(definition: PluginDefinition): ToderoPlugin;
...
6. For recurring business workflows that should create visible Todero work, prefer managed routines and managed resources over jobs. Jobs remain useful for private plugin-runtime maintenance tasks.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@todero/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@todero/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@todero/plugin-sdk/ui`)
...
Todero-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Todero navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@todero/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@todero/plugin-sdk/ui`
...
The `@todero/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that stores the shared `{ type: "secret_ref", secretId, version? }` object shape and resolves through the Todero secret provider system rather than a plain text input
...
1. Core Todero data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Todero objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Todero product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Todero later decides git state is core product data, it should become a first-party table too
...
3. Secret refs resolve through the existing Todero secret provider system using explicit `companyId`.
...
6. The operator may force-purge immediately via CLI: `npx todero plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Todero server. This is a normative requirement, not optional.
...
### 27.1 `@todero/plugin-test-harness`
...
```ts
import { createTestHarness } from "@todero/plugin-test-harness";
...
For developing a plugin against a running Todero instance:

- The operator installs the plugin from a local path: `npx todero plugin install ./path/to/plugin`
...
- `@todero/plugin-workspace-files`
- `@todero/plugin-terminal`
- `@todero/plugin-git`
- `@todero/plugin-linear`
- `@todero/plugin-github-issues`
- `@todero/plugin-grafana`
- `@todero/plugin-runtime-processes`
- `@todero/plugin-stripe`
...
- `@todero/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@todero/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@todero/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@todero/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@todero/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@todero/plugin-sdk/ui`
...
- `@todero/plugin-test-harness`
- `create-todero-plugin` starter template
...
Todero should not implement a generic in-process hook bag modeled directly after local coding tools.

Todero should implement:
...
That is the complete target design for the Todero plugin system.
