...
This is the short happy-path guide for developing a Todero plugin from a folder on your machine. You will scaffold a plugin, run it in watch mode, install it into a running Todero instance from an absolute local path, and edit code with the plugin worker reloading after each rebuild.
...
- A local Todero checkout you can run from source. Local plugin installs read source from disk, so the running server must be able to see the path you give it.

## The five steps

```bash
# 1. Start Todero locally
pnpm todero run

# 2. Scaffold a plugin outside the Todero repo
todero plugin init @acme/hello-plugin --output ~/dev/todero-plugins

# 3. Install dependencies and start the watch build
cd ~/dev/todero-plugins/hello-plugin
pnpm install
pnpm dev

# 4. In another terminal, install the plugin from its absolute path
todero plugin install ~/dev/todero-plugins/hello-plugin

# 5. Confirm it loaded
todero plugin list
todero plugin inspect acme.hello-plugin
```
...
Todero listens on `http://127.0.0.1:3100` by default. The CLI talks to that server, so leave it running.
...
This creates `~/dev/todero-plugins/hello-plugin/` with `src/manifest.ts`, `src/worker.ts`, `src/ui/index.tsx`, an esbuild watch config, a Vitest config, and a snapshot of `@todero/plugin-sdk` from your local Todero checkout. You can run the package and tests without publishing anything to npm.
...
If your plugin has UI and you want a browser-side dev server with hot module replacement during local UI iteration, run `pnpm dev:ui` in a second terminal. It serves `dist/ui/` on `http://127.0.0.1:4177`. This is optional; Todero can load the built UI directly from `dist/ui/` without it.
...
Before it installs, the CLI probes `GET /api/health` on the instance it is configured to talk to and prints the **target diagnostics** so you can confirm *which* Todero you are installing into. You will see a confirmation like:

```
Target Todero: http://127.0.0.1:3100
  health: status=ok  version=0.1.0  mode=local_trusted  exposure=private
Installing plugin from local path: /Users/you/dev/todero-plugins/hello-plugin
✓ Installed acme.hello-plugin v0.1.0 (ready)
Local plugin installs run trusted local code from your machine.
Keep `pnpm dev` running in /Users/you/dev/todero-plugins/hello-plugin;
Todero watches rebuilt dist output and reloads the plugin worker.
```

Read that first line. If the API URL, version, or mode is not the instance you expect, stop and re-point the CLI (see [Targeting a branch / issue-workspace runtime](#targeting-a-branch--issue-workspace-runtime)) before trusting the result. Pass `--no-verify-target` to skip the probe, or run `todero plugin target` to see the same diagnostics without installing anything.

Relative paths are resolved against the current working directory, so `todero plugin install .` from inside the plugin folder works too.
...
The five-step loop above assumes one Todero on `http://127.0.0.1:3100`. That breaks down the moment your plugin depends on **server code that only exists on a branch**. Examples:
...
So the API URL is explicit and overridable — the gap was never that you *couldn't* point at a branch server, it was that nothing told you which server you ended up on. `todero plugin target` and the pre-install probe close that gap.
...
PAPERCLIP_SERVER_PORT=3120 pnpm dev          # or: pnpm todero run

# 2. Confirm the CLI will talk to that exact branch service before installing.
todero plugin target --api-base http://127.0.0.1:3120
# Target Todero: http://127.0.0.1:3120
...
- **Worker code:** save a `.ts` file → esbuild rewrites `dist/worker.js` → Todero debounces ~500ms and restarts the plugin worker. The next worker call uses the new code. There is no in-process hot module replacement for worker code; it is a worker restart.
- **Manifest:** save `src/manifest.ts` → `dist/manifest.js` rewrites → the worker restarts and the host re-reads the manifest.
- **Plugin UI:** save a `.tsx` file → esbuild rewrites `dist/ui/` → Todero reloads the UI bundle on its next mount. To get HMR during UI iteration, run `pnpm dev:ui` and point at the dev server with `devUiUrl` in your manifest while developing.
- **Without `pnpm dev`:** the watcher only fires on `dist/*` changes. If you stop the watch build, source edits do not reach Todero. Restart `pnpm dev` (or run `pnpm build` once) before expecting changes.
- **`node_modules`, `.git`, `.todero-sdk`, and other dotfolders are ignored.** Adding a dependency requires the new code to actually be imported and rebuilt before the worker sees it.

The package's own build scripts still own compilation. Todero does not compile arbitrary local-path plugins for you. The exceptions are bundled plugins inside the Todero repo under `packages/plugins/`: workspace packages auto-build once with `pnpm --filter <package> build`, and standalone sandbox-provider packages under `packages/plugins/sandbox-providers/` first bootstrap package-local dependencies with `pnpm install --ignore-workspace ...` and then run `pnpm build` in place. Set `PAPERCLIP_DISABLE_PLUGIN_AUTOBUILD=1` in the server environment to disable those fallbacks.

## Local path plugins vs npm packages

Both go through the same install endpoint, but they mean different things:

- **Local path plugins are trusted local code.** Todero executes worker code from disk under the same trust boundary as the rest of the running instance. This is meant for developing or operating a plugin against a checkout you control. There is no signature check, no sandboxing of worker code, and no provenance metadata beyond the path. Do not install local-path plugins you did not write.
- **npm packages are the deployable artifact.** `todero plugin install @acme/plugin-foo` (optionally `--version 1.2.3`) installs from your configured npm registry, version-pins, and produces an install record that other operators can reproduce. Ship plugins this way.
...
- **Restart cleanly:** `todero plugin disable <key>` pauses the plugin without removing it. `todero plugin enable <key>` brings it back. `todero plugin uninstall <key>` removes the install record; add `--force` to also purge plugin state and settings.
- **Browse examples:** `todero plugin examples` lists the bundled example plugins that ship with the repo, each with a ready-to-run `todero plugin install <path>` line.
- **Go deeper:** [`PLUGIN_AUTHORING_GUIDE.md`](./PLUGIN_AUTHORING_GUIDE.md) covers worker capabilities, managed agents/projects/routines/skills, plugin database namespaces, scoped API routes, and the shared UI components in `@todero/plugin-sdk/ui`. [`PLUGIN_SPEC.md`](./PLUGIN_SPEC.md) is the longer-form specification, including future ideas that are not yet implemented.
