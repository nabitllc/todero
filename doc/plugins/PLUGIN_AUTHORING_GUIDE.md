...
This guide describes the current, implemented way to create a Todero plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Todero app.
...
- The host provides a small shared React component kit through
  `@todero/plugin-sdk/ui`; use it for common Todero controls before
...
Todero owns inline markdown rendering; plugins must not return React, HTML,
...
```bash
todero plugin init @yourscope/plugin-name --output /absolute/path/to/plugin-repos
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@todero/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@todero/plugin-sdk` from the local Todero checkout into a `.todero-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/todero/packages/plugins/sdk` if you have more than one Todero checkout.

## Local development workflow

See the short [Local Plugin Development guide](./LOCAL_PLUGIN_DEVELOPMENT.md) for the full happy path (`pnpm dev` → `todero plugin install <absolute-path>` → `todero plugin list`) and reload semantics.
...
## Managed Todero resources

Plugins that provide durable Todero business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Todero agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Todero issues. Prefer managed routines over
...
- Managed skills: declare top-level `skills[]` and require `skills.managed`.
  ...
  synced into Todero managed agents.
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the Todero heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Todero issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Todero records that the
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@todero/plugin-sdk/ui`
...
Use shared components from `@todero/plugin-sdk/ui` when the plugin needs a
Todero-native control. The host owns the implementation, so plugins inherit
the board's current styling, ordering, recent selections, and dark-mode behavior
without importing `ui/src` internals.

Prefer shared components for common Todero UX patterns to reduce drift and
...
Use `FileTree` from `@todero/plugin-sdk/ui` whenever the plugin only needs
