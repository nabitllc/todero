...
Todero V1 must provide a full control-plane loop for autonomous agents:
...
- `runtime_config` jsonb not null default `{}`; may include Todero runtime policy such as `modelProfiles.cheap.adapterConfig` for an optional low-cost model lane that does not change the primary adapter config
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Todero runtime-owned keys are injected

Routine execution issues add a routine-scoped env overlay after project env and before Todero runtime-owned keys. Routine env uses the same secret-aware binding format, is stored on `routines.env`, is snapshotted in routine revisions, and resolves secret refs against the routine binding target so routine-owned secrets do not require direct bindings on the executing agent.
...
- Attachment-backed artifact work products use `type: "artifact"`, `provider: "todero"`, and metadata with `attachmentId`, `contentType`, `byteSize`, `contentPath`, `openPath`, `downloadPath`, and optional `originalFilename`.
...
- heartbeat finalization evaluates liveness from persisted Todero state; an issue cannot remain healthy `in_progress` solely because the exiting heartbeat started a local/background watcher
...
- when Todero cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
Todero V1 keeps a company-scoped visibility model as the default because centralized authorization and scoped work-object controls are not yet a core V1 control surface.
...
| Company skills | Open to authenticated company agents; core enforces invariants and any stored restriction policy | Todero EE policy editor, protected-skill controls, presets, simulation, and policy audit UX |
...
Core Skill Studio and Todero EE MUST treat those codes as hard platform failures, not as prompts to loosen policy.
...
- Export MUST include explicit skill policy configuration and retained legacy grants in `.todero.yaml`, never secret values or environment-specific paths. An unconfigured company exports no synthetic restriction.
- If Todero EE is unavailable or removed, core continues to enforce stored policies and expose the policy API. Normal skill work remains available under the open default; explicit denials use core remediation text rather than a broken EE-only link.
...
On a Todero Cloud-managed instance, `POST /companies` returns `403` with
...
- if unset, auto-use embedded PostgreSQL under `~/.todero/instances/default/db`
...
- `.todero.yaml` sidecar for Todero-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.todero.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Todero routine fidelity in `.todero.yaml`
- Todero imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.todero.yaml`
- export never includes secret values; env inputs are reported as portable declarations instead
- export preserves explicit company skill policy and retained legacy skill grants in `.todero.yaml`; absence of policy remains the open default
