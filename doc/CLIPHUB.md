...
ClipHub is the public registry where people share, discover, and download Todero company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Todero what a package registry is to a programming language. Todero already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Todero — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Todero instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Todero spec. It contains:
...
**Install (fresh start):**
```
todero install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Todero instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Todero, export your company as a template, then publish:

```
todero export --template my-company
todero publish cliphub my-company
```
...
ClipHub is a **separate service** from Todero itself. Todero is self-hosted; ClipHub is a hosted registry that Todero instances talk to.
...
| **Todero CLI** | `todero install`, `todero publish`, `todero cliphub sync` — built into Todero |
| **Todero UI** | "Browse ClipHub" panel in the Todero web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Todero) |
| Backend | TypeScript + Hono (consistent with Todero) |
| Database | PostgreSQL |
...
4. Run `todero install cliphub:acme/lean-saas-shop`
5. Todero creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Todero until it works well
2. Export: `todero export --template my-agency`
3. Publish: `todero publish cliphub my-agency`
...
3. Install just that agent: `todero install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Todero

ClipHub is **not required** to use Todero. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Todero what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `todero install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Todero web UI
- [ ] `todero cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Todero's own philosophy)
