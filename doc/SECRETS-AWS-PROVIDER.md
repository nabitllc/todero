...
Operational contract for the hosted `aws_secrets_manager` secret provider used by Todero Cloud.

## Scope

- Hosted provider for Todero-managed company and user-scoped secrets when Todero Cloud runs on AWS.
- Source of truth for secret values is AWS Secrets Manager, not Postgres.
- Todero stores only metadata needed for ownership, bindings, version selection, audit, and runtime resolution.
- AWS provider bootstrap credentials are deployment/runtime credentials, not Todero-managed company secrets.
- Remote import for existing AWS secrets is metadata-only. Preview/import uses
  AWS inventory metadata and creates Todero external references; it does not
  copy plaintext into Todero.
...
The AWS provider has a chicken-and-egg boundary: Todero cannot use
`company_secrets` to unlock the AWS provider that stores those secrets. The
initial AWS trust must exist before the Todero server starts.

Allowed bootstrap locations:

- Infrastructure IAM or workload identity attached to the Todero server
  runtime.
- Process environment or orchestrator secret store used to start the Todero
  server.
...
keys into the Todero board UI. Do not store those bootstrap keys in
`company_secrets`.

## Todero Cloud Bootstrap

Todero Cloud must provision the AWS backing resources before any board user
can create AWS-backed company secrets:

1. Create or select the deployment KMS key.
2. Create the Todero server runtime role for the deployment.
...
4. Configure the server runtime with the non-secret provider environment
   variables below.
5. Run `todero doctor` or the provider health endpoint from the deployed
...
Once this is in place, the board UI can create Todero-managed AWS secrets and
Todero will write them under the deployment/company namespace.
...
```sh
aws sso login --profile todero-dev
AWS_PROFILE=todero-dev \
...
should not be written to Todero config, committed to `.env` files, stored in
`company_secrets`, or used as the default Todero Cloud bootstrap path.
...
```sh
PAPERCLIP_SECRETS_AWS_PREFIX=todero
PAPERCLIP_SECRETS_AWS_ENVIRONMENT=production
PAPERCLIP_SECRETS_AWS_PROVIDER_OWNER=todero
PAPERCLIP_SECRETS_AWS_ENDPOINT=
PAPERCLIP_SECRETS_AWS_DELETE_RECOVERY_DAYS=30
```

Naming convention for Todero-managed secrets:

```text
todero/{deploymentId}/{companyId}/{secretKey}
```

Tag set for Todero-managed secrets:

- `todero:managed-by=todero`
- `todero:provider-owner=<owner tag>`
- `todero:deployment-id=<deployment id>`
- `todero:company-id=<company id>`
- `todero:secret-key=<secret key>`
- `todero:environment=<environment tag>`

When the user-secret service creates Todero-managed AWS values, keep them
...
```text
todero-ext/<environment>/<company-id>/user-secrets/<definition-key>/<opaque-owner-id>
```

Todero stores the external ref and provider version as metadata. Treat those
...
- One Todero app role per deployment.
- One deployment-scoped KMS key per deployment at launch.
- Future per-company KMS keys remain compatible because Todero stores provider refs and version metadata separately from values.
...
```text
arn:aws:secretsmanager:<region>:<account-id>:secret:todero/<deployment-id>/*
```
...
- Prefer workload identity / role-based auth. Do not store AWS credentials inline in Todero config.
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:<region>:<account-id>:secret:todero/<deployment-id>/*"
    },
    {
      "Sid": "ToderoDeploymentKms",
      "Effect": "Allow",
      "Action": [
        "kms:Encrypt",
        "kms:Decrypt",
...
- Todero-managed secrets may be deleted only by Todero or an operator with equivalent break-glass access.
- External references may resolve through Todero runtime, but Todero should not delete the external secret resource.
...
```json
{
  "Sid": "ToderoRemoteSecretInventory",
  "Effect": "Allow",
...
operator-approved external prefixes that Todero is allowed to consume:

```json
{
  "Sid": "ToderoResolveImportedExternalReferences",
  "Effect": "Allow",
...
permissions scoped to `todero/<deployment-id>/*`; do not broaden them for
remote import.

The same rule applies to user-scoped external refs. Todero derives the
...
role can read a broad external prefix, that access is broader than Todero's
...
- Prefer one Todero runtime role per environment/account.
...
Todero also blocks importing refs under its own managed namespace as
external references. Use the Todero-managed flow for
`todero/{deploymentId}/{companyId}/{secretKey}` resources.

## Existing AWS Secrets

V1 keeps existing AWS Secrets Manager entries as **linked external references**, not adopted
Todero-managed resources.

Use the Todero-managed flow when Todero should create and rotate the value. The AWS
...
```text
/todero-bench/anthropic_api_key
```

In that mode Todero stores only the path or ARN, resolves it at runtime, and records
redacted access events. Operators rotate the actual value in AWS. Update the Todero
reference only when the AWS path, ARN, or pinned provider version changes.

Todero does not currently offer an "adopt existing AWS secret" flow that takes over future
`PutSecretValue` writes for an arbitrary existing secret. Adding that later requires explicit
confirmation UX, scope validation, expected Todero tags, and security/cloud-ops review.

## Data Custody

- Todero stores `externalRef`, `providerVersionRef`, provider id, fingerprint hash, status, and binding metadata.
- Todero does not store AWS secret plaintext in `company_secret_versions.material`.
- Runtime resolution fetches the value from AWS only when a bound consumer needs it.

## Rotation Runbook

Manual Todero-managed rotation:

1. Write the new value through the Todero secret rotate flow.
2. Todero creates a new AWS secret version with `PutSecretValue`.
3. Todero records the new `providerVersionRef` in `company_secret_versions`.
4. Re-run or restart affected workloads that consume `latest`, or pin consumers to a specific Todero version before rollout when you need staged release safety.

Guidance:

- Prefer pinned Todero secret versions for risky rollouts.
...
- Todero database metadata for secret ownership, bindings, status, and provider version refs.
...
1. Restore Todero database metadata.
...
4. Confirm the Todero runtime role can call `GetSecretValue` on the restored
...
1. Stop or pause affected Todero runs.
2. Audit recent Todero secret access events for impacted secret ids and consumers.
...
4. Rotate impacted secrets in AWS through Todero-managed versioning.
...
1. Create a test secret through the Todero board or API under a throwaway company.
2. Confirm the resulting AWS secret name matches `todero/{deploymentId}/{companyId}/{secretKey}`.
3. Rotate the secret once and confirm a new `providerVersionRef` appears in Todero metadata.
