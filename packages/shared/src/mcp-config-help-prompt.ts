 ...
 * whatever chat surface they like, so anything folded in here leaves Todero.
 ...
 * operator has to paste, because that is the only part Todero's config
 * preview parser reads. Everything else is prose for the human.
 */
export const MCP_CONFIG_HELP_PROMPT = `I want to connect an MCP (Model Context Protocol) server to Todero, and I need a paste-ready config.
...
export const MCP_CONFIG_HELP_INSTRUCTIONS = [
  ...
  "Todero reads the header names from that JSON and asks you for the values, then stores them as Todero secrets — so the config you paste should contain placeholders, not live credentials.",
] as const;
