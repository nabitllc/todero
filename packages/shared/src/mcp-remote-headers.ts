 ...
 * uses, and its value is stored as a Todero secret like every other one.
 */
...
/**
 * Headers Todero refuses to project from a user-supplied config.
 *
 ...
 * ambient browser-style credentials that Todero cannot scope or rotate.
...
/**
 * Is `name` a header Todero is willing to send on a user-configured remote
...
export function mcpRemoteHeaderRejectionMessage(
  ...
  switch (reason) {
    ...
    case "forbidden":
      return `Todero manages the "${headerName}" header and cannot send a custom value for it.`;
    case "value_too_long":
