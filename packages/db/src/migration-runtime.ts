...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://todero:todero@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "todero");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://todero:todero@127.0.0.1:${preferredPort}/todero`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://todero:todero@127.0.0.1:${port}/todero`,
  ...
  const adminConnectionString = `postgres://todero:todero@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "todero");

  return {
    connectionString: `postgres://todero:todero@127.0.0.1:${selectedPort}/todero`,
