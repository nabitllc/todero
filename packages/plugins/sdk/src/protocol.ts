 ...
 * channel.
 *
 * The Todero plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@todero/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Todero messages.
 */
...
> {
   ...
   */
  readonly toderoInvocation?: PluginInvocationContext;
  /** Opaque top-level invocation id echoed by worker→host requests. */
  readonly toderoInvocationId?: string;
}
...
> {
   ...
   * Worker→host notifications echo only `toderoInvocationId`.
   */
  readonly toderoInvocation?: PluginInvocationContext;
...
/**
 * Todero plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Todero instance. */
    instanceId: string;
    /** Semver version of the running Todero host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
...
export interface PluginPerformActionActorContext {
  /** Authenticated principal type resolved by the Todero host. */
...
export interface PluginLoginPtyOpenParams {
   ...
   * `/tmp/todero-adapter-login/<uuid>`. The worker revalidates the shape
...
export interface WorkerToHostNotifications {
   ...
   * the envelope field `toderoInvocationId`, which the worker RPC host stamps
