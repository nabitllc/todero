 ...
 *
 * These components are exported from `@todero/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@todero/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function ToderoSdkUiComponent(props: TProps) {
...
/**
 * Renders Todero's shared Markdown editor.
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components
 */
...
/**
 * Renders Todero's native issue list component for company-scoped plugin
...
/**
 * Renders Todero's native managed routines list for plugin settings pages.
 */
