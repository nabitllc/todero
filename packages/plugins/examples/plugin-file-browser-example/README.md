...
Example Todero plugin that demonstrates:
...
```bash
pnpm --filter @todero/plugin-file-browser-example build
npx todero plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
npx todero plugin uninstall todero-file-browser-example --force
```
