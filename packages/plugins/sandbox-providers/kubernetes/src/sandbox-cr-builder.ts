 ...
 * which todero-server can exec arbitrary commands. This solves the
 ...
 *
 * NOTE: todero-server runs OUTSIDE the cluster, so we cannot set ownerReferences
...
export function buildSandboxCrManifest(
  ...
  return {
    ...
    metadata: {
      ...
      labels: { ...input.labels },
      // No ownerReferences: todero-server is out-of-cluster. Release is
      // explicit delete.
    },
    spec: {
      podTemplate: {
          ...
          containers: [
            {
              ...
              // sleep infinity keeps the pod running; todero-server execs
