# Manual smoke test — `@todero/plugin-kubernetes`

Manual sanity check that the plugin works end-to-end against a real
todero-server instance and a real Kubernetes cluster (kind for local
...
- A running kind cluster:
  ...
  ```
- `kubectl --context kind-todero get nodes` returns a node in `Ready` state.
...
### 2. Start todero-server in dev mode

In a separate terminal:

```bash
cd /path/to/todero
export PAPERCLIP_HOME=/tmp/todero-smoke
export PAPERCLIP_INSTANCE_ID=smoke
export PAPERCLIP_DEPLOYMENT_MODE=local_trusted
pnpm --filter @todero/server dev
```
...
npx todero plugin install \
  --local /path/to/todero/packages/plugins/sandbox-providers/kubernetes \
  --api-base http://127.0.0.1:3100
```

Expected: `✓ Installed todero.kubernetes-sandbox-provider v0.1.0 (ready)`.
...
curl -s -X POST -H "Content-Type: application/json" \
  -d "{
    ...
    \"config\": {
      ...
      \"imageAllowList\": [\"ghcr.io/todero/agent-runtime-claude:v1\"]
    }
  }" \
...
(`todero-smoke`). On first probe the namespace may not yet exist —
...
1. `ensureTenant` — provision the `todero-smoke` namespace, SA, Role,
...
```bash
kubectl --context kind-todero get namespace todero-smoke
kubectl --context kind-todero get all,networkpolicy,resourcequota,limitrange,sa,role,rolebinding -n todero-smoke
```

Expected:

- Namespace `todero-smoke` exists with PSS labels
  (`pod-security.kubernetes.io/enforce=restricted`)
- ServiceAccount `todero-tenant-sa`
- Role `todero-tenant-role`, RoleBinding `todero-tenant-rb`
- ResourceQuota `todero-quota`, LimitRange `todero-limits`
- NetworkPolicies `todero-deny-all` + `todero-egress-allow`
...
```bash
kubectl --context kind-todero delete namespace todero-smoke
kill %1  # todero-server
```
...
- Output of `kubectl get all -n todero-smoke` after step 6
