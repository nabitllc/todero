# `@todero/plugin-exe-dev`

Published exe.dev sandbox provider plugin for Todero.

This package lives in the Todero monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. That lets operators install it from the Plugins page by package name without introducing root lockfile churn.
...
- When you save an environment, Todero stores pasted API keys and pasted SSH private keys as company secrets.
...
- SSH access from the Todero host to the resulting `*.exe.xyz` VMs.
