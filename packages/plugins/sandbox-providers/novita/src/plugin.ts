...
export function parseNovitaDriverConfig(raw: Record<string, unknown>): NovitaDriverConfig {
  return {
    ...
    requestedCwd: parseOptionalString(raw.requestedCwd) ?? "/home/user/todero-workspace",
...
function buildStdinPath(): string {
  return `/tmp/.todero-stdin-${randomUUID()}`;
}
...
async function createSandbox(params: PluginEnvironmentAcquireLeaseParams | PluginEnvironmentProbeParams, config: NovitaDriverConfig) {
  const metadata = {
    "todero-provider": "novita",
    "todero-company-id": params.companyId,
    "todero-environment-id": params.environmentId,
    ...(params.issueId ? { "todero-issue-id": params.issueId } : {}),
    ...("runId" in params ? { "todero-run-id": params.runId } : {}),
  };
  const opts = {
    ...sandboxOpts(config),
    metadata,
