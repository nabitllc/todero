...
must meet this contract. The Todero runtime does not build the environment at
...
Todero runs agent work inside a sandbox. Todero protects the host from
the code in that sandbox. This section states the rule for that protection. It
does not state the implementation.

A sandbox is an untrusted execution environment. Todero assumes that a
sandbox process can read or change all accessible data.

Todero does not protect sandbox files, processes, credentials, or code from
...
Todero protects two authorities across the boundary:

- The authority to write host files
- The authority to call the Todero API

Sandbox code holds each authority through one surface only:

1. **Outbound workspace synchronization.** Todero copies sandbox files to
   host paths that the Todero orchestrator selects.
2. **The Todero HTTP bridge.** Sandbox code calls the Todero API through
   the bridge.
...
This repository does not enforce these provider rules today, and Todero
...
Todero does not enforce this policy inside the sandbox.
...
- The change lets sandbox code write host files outside workspace
  synchronization.
- The change lets sandbox code call the Todero API outside the HTTP bridge.
...
Todero or the provider must enforce each boundary control outside the
...
### Surface 2: Todero HTTP bridge

Sandbox code must call the Todero API only through the HTTP bridge.
...
Todero sends commands from the host to the sandbox. Command execution can
return output to the host. This output does not give sandbox code authority to
write host files or call the Todero API.
...
Todero probes each CLI before launch. Todero uses the same detection
...
Todero probes each CLI with `command -v <cmd>`. Todero fails loudly when
...
- The Todero runtime never modifies the login profile. The runtime never
  writes a profile file. The runtime never writes an rc file.
- The Todero runtime never sources `nvm` on the exec path.
