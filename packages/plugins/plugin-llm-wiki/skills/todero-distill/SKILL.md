---
name: todero-distill
description: Use when an operation issue is a Todero cursor-window, distill, or backfill. Turn source-bundled Todero activity into wiki-insightful project standups, durable project pages, decisions, and history without asset dereferencing.
---

# Todero Distill

Distill Todero project, issue, comment, and document activity into durable wiki pages. The success criterion is **wiki-insightful, not procedural**: a reader who has never seen Todero should learn what the project is, what was decided, what is at risk, and what the current state is — without scanning a list of `## [YYYY-MM-DD]` headers.

## When this skill is needed

- Cursor-window distillation: the routine fed you a bounded source bundle of recent Todero activity for one project or root issue.
...
- Manual `distill-todero-now` request from the UI.
...
In Phase 1, every Todero distill, backfill, and cursor-window operation writes into the
...
write Todero-derived pages into a non-default space.

This rule is destination-only. The Todero source scope (which projects, root issues,
comments, documents are read) is set elsewhere in the operation issue and is independent of
the destination.

## Inputs

- A Todero source bundle (issue list, comment refs, document refs, source hash, cursor window).
...
Do not treat Todero assets/attachments or issue work products as source text for this skill.

- Allowed Todero body text: issue descriptions, comment bodies, document bodies.
...
1. **Datestamp-as-section-header.** Lines like `## [2026-04-15] todero-distill | proposed` belong in `wiki/log.md`, not in the project page. The project page is durable knowledge; the log is the audit trail.
2. **Procedural status lists.** `Issue mix: 3 todo, 5 in_progress, 2 done` tells the reader nothing they could not read off Todero directly. State *what is happening and why it matters*, then cite the issues that constitute the evidence.
...
4. **Write `wiki/projects/<slug>/standup.md` first.** Every Todero project represented in the wiki must have this file. It is the executive standup: where the project stands today, what changed recently, what is blocked or risky, and what happens next. Use stable sections, in this order:
   ...
   - **Next Actions** — concrete next actions and owners inferred from Todero issues, not vague aspirations.
   - **Links** — durable wiki project page and relevant Todero project/issues/documents.
   Rewrite the standup to today's state. Do not append endless dated sections; the audit trail belongs in `wiki/log.md` and Todero comments.
5. **Write `wiki/projects/<slug>/index.md`** with these stable sections, in this order:
   ...
   - **References** — readable links to the current standup and supporting Todero tasks/documents. Keep hashes and cursor ids out of the narrative.
...
9. **Append `wiki/log.md`** entry — this is where the datestamp belongs:
   ```
   ## [YYYY-MM-DD] todero-distill | <project name>
...
- Cite Todero source refs inline using their issue identifier (e.g. `PAP-3179`), not opaque UUIDs.
...
- Append a `todero-distill | low-signal skip` log entry naming the cursor window.
...
- [ ] The project page reads as wiki content, not as a Todero status report. A reader new to the company should understand what the project is.
...
`wiki_search`, `wiki_read_page`, `wiki_write_page`, `wiki_list_sources`, `wiki_read_source`. Always include the operation issue's `wikiId` and `spaceSlug`. The Todero source bundle arrives as part of the operation context — you do not need to assemble it.
