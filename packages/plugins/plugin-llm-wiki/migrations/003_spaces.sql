...
WITH wiki_pairs AS (
  ...
  UNION
  SELECT company_id, wiki_id FROM plugin_llm_wiki_8f50da974f.todero_distillation_cursors
  UNION
  SELECT company_id, wiki_id FROM plugin_llm_wiki_8f50da974f.todero_distillation_work_items
  UNION
  SELECT company_id, wiki_id FROM plugin_llm_wiki_8f50da974f.todero_distillation_runs
  UNION
  SELECT company_id, wiki_id FROM plugin_llm_wiki_8f50da974f.todero_source_snapshots
  UNION
  SELECT company_id, wiki_id FROM plugin_llm_wiki_8f50da974f.todero_page_bindings
)
...
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_cursors ADD COLUMN IF NOT EXISTS space_id uuid;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_work_items ADD COLUMN IF NOT EXISTS space_id uuid;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_runs ADD COLUMN IF NOT EXISTS space_id uuid;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_source_snapshots ADD COLUMN IF NOT EXISTS space_id uuid;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_page_bindings ADD COLUMN IF NOT EXISTS space_id uuid;
...
UPDATE plugin_llm_wiki_8f50da974f.todero_distillation_cursors t
SET space_id = s.id
FROM plugin_llm_wiki_8f50da974f.wiki_spaces s
WHERE t.company_id = s.company_id AND t.wiki_id = s.wiki_id AND s.slug = 'default' AND t.space_id IS NULL;

UPDATE plugin_llm_wiki_8f50da974f.todero_distillation_work_items t
SET space_id = s.id
FROM plugin_llm_wiki_8f50da974f.wiki_spaces s
WHERE t.company_id = s.company_id AND t.wiki_id = s.wiki_id AND s.slug = 'default' AND t.space_id IS NULL;

UPDATE plugin_llm_wiki_8f50da974f.todero_distillation_runs t
SET space_id = s.id
FROM plugin_llm_wiki_8f50da974f.wiki_spaces s
WHERE t.company_id = s.company_id AND t.wiki_id = s.wiki_id AND s.slug = 'default' AND t.space_id IS NULL;

UPDATE plugin_llm_wiki_8f50da974f.todero_source_snapshots t
SET space_id = s.id
FROM plugin_llm_wiki_8f50da974f.wiki_spaces s
WHERE t.company_id = s.company_id AND t.wiki_id = s.wiki_id AND s.slug = 'default' AND t.space_id IS NULL;

UPDATE plugin_llm_wiki_8f50da974f.todero_page_bindings t
SET space_id = s.id
FROM plugin_llm_wiki_8f50da974f.wiki_spaces s
WHERE t.company_id = s.company_id AND t.wiki_id = s.wiki_id AND s.slug = 'default' AND t.space_id IS NULL;
...
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_cursors ALTER COLUMN space_id SET NOT NULL;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_work_items ALTER COLUMN space_id SET NOT NULL;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_runs ALTER COLUMN space_id SET NOT NULL;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_source_snapshots ALTER COLUMN space_id SET NOT NULL;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_page_bindings ALTER COLUMN space_id SET NOT NULL;

ALTER TABLE plugin_llm_wiki_8f50da974f.wiki_pages
  DROP CONSTRAINT IF EXISTS wiki_pages_company_id_wiki_id_path_key;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_cursors
  DROP CONSTRAINT IF EXISTS paperclip_distillation_cursor_company_id_wiki_id_source_sco_key;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_work_items
  DROP CONSTRAINT IF EXISTS paperclip_distillation_work_i_company_id_wiki_id_idempotenc_key;
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_page_bindings
...
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_distillation_runs
...
ALTER TABLE plugin_llm_wiki_8f50da974f.todero_source_snapshots
...
CREATE INDEX IF NOT EXISTS distillation_runs_space_idx ON plugin_llm_wiki_8f50da974f.todero_distillation_runs (company_id, wiki_id, space_id, created_at DESC);
