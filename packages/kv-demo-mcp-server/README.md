...
mutate — so you can call a tool from a Todero agent and watch the value
...
the right fixture for showing what Todero stores versus what the demo
package stores — the package stores the values, Todero stores the
...
- Four MCP tools (over the Streamable HTTP transport):
  ...
  - `kv_delete` (destructive) — delete a key. Carries `destructiveHint: true`
    so Todero's catalog quarantines it on first sight.
...
```sh
pnpm --filter @todero/kv-demo-mcp-server build
...
MCP tools share state, and listens on a fixed loopback port. Todero's
...
4. If you launched the server with `KV_DEMO_TOKEN`, paste the token in the
   **App key** field. The wizard stores it as an `Authorization: Bearer …`
   header secret.
5. Pick the profile defaults (read/write/destructive) and finish. Todero
...
- **`remote_http` (recommended for this demo)** — required if you want the
  ...
  from that process. Todero's `remote_http` gateway forwards every call to
...
- **`local_stdio` (not used here)** — runs MCP servers as supervised child
  processes inside Todero's runtime slots. Reserved for *trusted local
  ...
  ship in the Todero build when you need stdio; do not try to shoehorn
...
| Concern | Stored in this package | Stored in Todero |
| --- | --- | --- |
| Key/value entries (`kv_*` data) | In-memory `Map`, lost on restart. | Not stored. Todero never sees the values directly; the gateway only sees the MCP request/response envelope and the redacted-by-policy view the audit log keeps. |
...
Resetting the demo state usually means resetting this process; Todero's
...
- **Free the port** — if startup logs `EADDRINUSE`, another `kv-demo`
  ...
  kill <pid>
  ```
- **Quiesce the Todero side** — disable the connection so the gateway
