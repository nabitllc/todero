...
## Scope
This plan is now **gateway-only**. Todero supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Todero skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Todero task.
...
```bash
pnpm --filter @todero/server test:run -- openclaw-gateway
pnpm --filter @todero/server typecheck
pnpm --filter @todero/ui typecheck
pnpm --filter todero typecheck
```
