...
Use when:
- You want Todero to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across Todero heartbeats
- You want Todero to keep task state while Cursor handles remote code execution
...
Notes:
- Todero reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each Todero heartbeat maps to a Cursor run on that durable agent.
- Todero injects PAPERCLIP_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- Todero remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
