...
Use when:
- You want Todero to run Cursor Agent CLI locally as the agent runtime
...
- Todero auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$todero" and related skills on local runs.
- Todero auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
