...
/**
 * Parse Hermes Agent stdout into TranscriptEntry objects for the Todero UI.
 *
 ...
 *   System:     "[hermes] ..."
 *
 * We emit structured tool_call/tool_result pairs so Todero renders proper
 * tool cards (with status icons, expand/collapse) instead of raw stdout blocks.
 */

import type { TranscriptEntry } from "@todero/adapter-utils";
 ...
 * Todero uses this to match them in normalizeTranscript.
 */
 ...
 * Todero renders proper tool cards with status icons and expand/collapse.
 *
...
export function parseHermesStdoutLine(
  ...
  if (trimmed.startsWith("[hermes]") || trimmed.startsWith("[todero]")) {
  ...
  // Emit as stderr so Todero groups them into the amber accordion.
