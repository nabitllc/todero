 ...
 *
 * Translates Todero's CreateConfigValues into the adapterConfig
...
/**
 * Build a Hermes Agent adapter config from the Todero UI form values.
 */
export function buildHermesConfig(
  ...
  // NOTE: Provider is NOT set here because the Todero UI form
  ...
  // Execution limits — let the user configure these from the Todero UI.
  ...
  // Heartbeat config is handled by Todero itself
