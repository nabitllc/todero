/**
 * Hermes Agent adapter for Todero.
 *
 * Runs Hermes Agent (https://github.com/NousResearch/hermes-agent)
 * as a managed employee in a Todero company. Hermes Agent is a
 ...
 *
 * Hermes supports any model via any provider. The Todero UI should
...
/**
 * Documentation shown in the Todero UI when configuring a Hermes agent.
 */
...
This adapter package also ships a Hermes-facing Todero task bridge skill:
\`paperclip-task-bridge\`. Use it when a user starts in Hermes and asks Hermes
to create, comment on, update, or list Todero tasks.
...
- \`PAPERCLIP_BRIDGE_API_KEY\` - Todero agent API key created with \`scope.kind = "task_bridge"\`
...
- \`hermes_local\` means Todero shells out to local \`hermes chat\`.
- \`hermes_gateway\` means Todero wakes remote Hermes through Hermes's API server.
- \`paperclip-task-bridge\` means Hermes calls Todero's REST API to manage tasks.

Create task bridge keys with a parent issue or project boundary. Do not expose
normal claimed Todero agent API keys to internet-facing Hermes chat/webhook
...
/**
 * External adapter plugin entrypoint expected by Todero's adapter manager.
 */
