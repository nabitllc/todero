...
When you wake up, follow the Todero skill — it contains the full heartbeat procedure.
...
4. If a report does not exist, use the `todero-create-agent` skill to hire one before delegating.
...
- Use child issues for delegated work and rely on Todero wake events or comments rather than polling agents, sessions, or processes.
