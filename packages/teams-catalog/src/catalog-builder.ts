...
const CATALOG_PACKAGE_NAME = "@todero/teams-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @todero/teams-catalog build:manifest.");
    }
  }
...
async function loadCatalogSkills(packageDir: string, errors: string[]): Promise<CatalogSkillSummary[]> {
  try {
    const catalogPackageName = "@todero/skills-catalog";
    ...
    try {
      ...
      errors.push(`Could not load @todero/skills-catalog for skill requirement validation: ${errorMessage(error)}`);
      return [];
    }
  }
}
...
async function buildCatalogTeam(
  ...
  const id = `todero:${candidate.kind}:${candidate.category}:${candidate.slug}`;
  const key = `todero/${candidate.kind}/${candidate.category}/${candidate.slug}`;
...
function resolveSkillRequirement(
  ...
  errors.push(`${prefix} skill reference "${ref}" does not resolve to a local team skill or @todero/skills-catalog skill.`);
  return {
    type: "catalog",
    ref,
    agentSlugs: agentSlugs.sort(),
...
function classifyCatalogFile(relativePath: string): CatalogTeamFileKind {
  ...
  if (relativePath === ".todero.yaml") return "extension";
...
function collectCandidateUniquenessErrors(candidates: TeamCandidate[], errors: string[]) {
  const projected = candidates.map((candidate) => ({
    id: `todero:${candidate.kind}:${candidate.category}:${candidate.slug}`,
    key: `todero/${candidate.kind}/${candidate.category}/${candidate.slug}`,
    slug: candidate.slug,
