...
usage() {
  cat <<'EOF'
Usage:
  todero-upload-artifact.sh FILE [options]

Uploads a generated file from the current workspace to the current Todero
...
Examples:
  scripts/todero-upload-artifact.sh dist/demo.mp4 \
    --title "Demo video render" \
    --summary "MP4 render for board review"

  scripts/todero-upload-artifact.sh out/walkthrough.webm \
