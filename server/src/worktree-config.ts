...
function emptyWorktreeEnvFileContents(): string {
  return [
    ...
    "# Generated by Todero worktree repair",
    "",
  ].join("\n");
}
...
function resolveWorktreeRuntimeContext(
  ...
  const configPath = resolveToderoConfigPath(overrideConfigPath);
  ...
  // a target as a worktree when its config sits in a `<root>/.todero/`
  ...
  if (path.basename(path.dirname(configPath)) !== ".todero") return null;
...
function writeConfigFile(configPath: string, config: ToderoConfig): boolean {
  fs.mkdirSync(path.dirname(configPath), { recursive: true });
  const update = toderoConfigSchema.parse(config);
  const source = fs.existsSync(configPath)
    ? toderoConfigSchema.parse(JSON.parse(fs.readFileSync(configPath, "utf8")))
    : null;
  const nextConfig = source
    ? toderoConfigSchema.parse(mergeToderoConfig(source, update))
    : update;
...
function collectSiblingWorktreePorts(
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".todero", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as ToderoConfig;
...
): { config: ToderoConfig; changed: boolean } {
  let changed = false;
  ...
  if (fs.existsSync(context.configPath)) {
    try {
      const runtimeConfig = withWorktreePortRegistryLockSync(context.homeDir, () => {
        const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ToderoConfig;
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: ToderoConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ToderoConfig;
  } catch {
    return;
  }
