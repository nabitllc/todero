...
function summarizeRunFailureForIssueComment(run: LatestIssueRun) {
  ...
  if (readNonEmptyString(run.error) || readNonEmptyString(run.errorCode)) {
    return " Latest retry failure details were withheld from the task thread; inspect the linked turn for evidence.";
  }
  return null;
}
  ...
  async function resolveContinuationWaitingOnReview(issue: typeof issues.$inferSelect) {
    ...
    await issuesSvc.addComment(
      ...
      `This task is waiting on ${waitingOn} to finish. ` +
        ...
        "Todero turned that into a normal dependency wait instead of flagging it as stuck.)",
      {},
      {
        authorType: "system",
      ...
      if (issue.status === "in_review") {
        ...
        if (participantAdapterFailureClassification?.kind === "configuration_incomplete") {
          const updated = await escalateStrandedAssignedIssue({
            ...
            recoveryCause: "configuration_incomplete",
            comment:
              "Todero classified the active review participant's latest adapter failure as " +
      ...
      if (issue.status === "todo") {
        ...
        if (didAutomaticRecoveryFail(latestRun, "assignment_recovery")) {
          const updated = await escalateStrandedAssignedIssue({
            ...
            notice: {
              body:
                "Todero automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
        ...
        if (isRepeatedProductiveContinuationRecovery(successfulRun)) {
          ...
          if (!exempted) {
            const updated = await escalateStrandedAssignedIssue({
              ...
              comment:
                "Todero automatically retried continuation for this assigned `in_progress` issue and the retry " +
      ...
      if (isUnsuccessfulTerminalIssueRun(latestRun)) {
        ...
        if (classification.kind === "non_retryable") {
          const updated = await escalateStrandedAssignedIssue({
            ...
            notice: {
              body:
                "Todero detected a non-retryable failure on this issue's continuation run " +
        ...
        if (didAutomaticRecoveryFail(latestRun, "issue_continuation_needed")) {
          ...
          if (consecutive >= classification.maxAttempts) {
            ...
            const updated = await escalateStrandedAssignedIssue({
              ...
              notice: {
                body:
                  "Todero automatically retried continuation for this assigned `in_progress` issue after its live " +
