...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole } from "@todero/shared";
import {
  forbidden,
  conflict,
  notFound,
...
/** Resolve the Todero repo skills directory (built-in / managed skills). */
function resolveToderoSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  const claudeSkillsDir = resolveClaudeSkillsDir();
  const toderoSkillsDir = resolveToderoSkillsDir();

  // Build set of Todero-managed skill names
  const toderoSkillNames = new Set<string>();
  if (toderoSkillsDir) {
    try {
      for (const entry of fs.readdirSync(toderoSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) toderoSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isToderoManaged: toderoSkillNames.has(entry.name),
      });
    }
  ...
  if (toderoSkillsDir) {
    ...
    try {
      for (const entry of fs.readdirSync(toderoSkillsDir, { withFileTypes: true })) {
        ...
        const skillMdPath = path.join(toderoSkillsDir, entry.name, "SKILL.md");
        let description = "";
        try {
          const md = fs.readFileSync(skillMdPath, "utf8");
          description = parseSkillFrontmatter(md).description;
        ...
        skills.push({
          name: entry.name,
          description,
          isToderoManaged: true,
        });
      }
    } catch { /* skip Todero skills directory */ }
  }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.toderoApiUrl)) {
    const legacyToderoApiUrl = nonEmptyTrimmedString(input.toderoApiUrl);
    if (legacyToderoApiUrl) merged.toderoApiUrl = legacyToderoApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    toderoApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.toderoApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
        ...
        if (apiBaseUrl.protocol !== "http:" && apiBaseUrl.protocol !== "https:") {
          ...
          if (mappedDefaultDashboardRoot) {
            diagnostics.push({
              ...
              hint: "Hermes dashboard and /chat routes are browser UI routes. Todero gateway calls use /api/health and /api/v1/runs.",
            });
          }
  ...
  const rawToderoApiUrl =
    typeof defaults.toderoApiUrl === "string"
      ? defaults.toderoApiUrl.trim()
      : "";
  if (rawToderoApiUrl) {
    try {
      const parsedToderoApiUrl = new URL(rawToderoApiUrl);
      if (
        parsedToderoApiUrl.protocol !== "http:" &&
        parsedToderoApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_paperclip_api_url_protocol",
          level: "warn",
          message: `toderoApiUrl must use http:// or https:// (got ${parsedToderoApiUrl.protocol}).`
        });
      } else {
        normalized.toderoApiUrl = parsedToderoApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_paperclip_api_url_configured",
          level: "info",
          message: `toderoApiUrl set to ${parsedToderoApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        ...
        level: "warn",
        message: `Invalid toderoApiUrl: ${rawToderoApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Todero host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Todero is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run npx todero allowed-hostname <host>`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const skillPath = `/api/invites/${token}/skills/todero`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an external Todero agent, save your one-time claim secret, wait for board approval, then claim your Todero API key through the standard claim endpoint. Use requestType='agent', include your agentName and capabilities, and set adapterType plus agentDefaultsPayload for your runtime when applicable. Hermes Gateway agents must use adapterType='hermes_gateway', start a clean Hermes install with API_SERVER_ENABLED=true and a fresh API_SERVER_KEY, then run `hermes gateway run --replace --accept-hooks`. Put the Hermes gateway URL in agentDefaultsPayload.apiBaseUrl, put the exact API_SERVER_KEY value in agentDefaultsPayload.apiKey, and put the reachable Todero base URL in agentDefaultsPayload.toderoApiUrl. If you use the default Hermes dashboard root or /chat URL on port 9119, Todero maps it to /api automatically. OpenClaw Gateway agents must use adapterType='openclaw_gateway', set agentDefaultsPayload.url to a ws:// or wss:// gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token.",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Runtime-specific adapter config. OpenClaw Gateway agents must include url (ws:// or wss://) and headers.x-openclaw-token. Hermes Gateway agents must include apiBaseUrl, apiKey set to the Hermes API_SERVER_KEY, and toderoApiUrl. A default Hermes dashboard root or /chat URL such as http://127.0.0.1:9119/chat is accepted and maps to /api. Other runtimes should include the config their adapter expects."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Todero hostname is reachable and allowed via `npx todero allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Todero API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "runtime-specific Todero skill location"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    Decide which Todero adapter type matches your runtime.

    Use adapterType only when there is a matching Todero adapter. Put runtime-specific settings in agentDefaultsPayload.
    ...
    OpenClaw Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "toderoApiUrl": "https://todero-hostname-your-agent-can-reach:3100",
    ...
    - Set agentDefaultsPayload.apiBaseUrl to the Hermes gateway URL Todero can reach.
    - Set agentDefaultsPayload.apiKey to the exact same value as API_SERVER_KEY. This is the Hermes gateway key, not the Todero key.
    ...
    - Watch out: /chat and the dashboard root are browser UI routes. Todero tests /api/health and starts runs with /api/v1/runs after mapping them to the API base.
    - Set agentDefaultsPayload.toderoApiUrl to the Todero base URL Hermes can reach.
    - Use hermes_local when Todero should start Hermes on the Todero host.
    - Use hermes_gateway when Todero should call an already-running Hermes API server.
    - After board approval, claim the Todero API key once with the claim endpoint below and save it as PAPERCLIP_API_KEY. Store the parsed token field from the raw HTTP JSON response before printing or summarizing it; do not copy token values from chat, transcript, or tool-output previews. A token value containing literal ... or [redacted] is a masked display preview, not a valid key. Do not rotate or invent a Todero key manually.
    - Hermes-originated Todero API usage means Hermes calls Todero with PAPERCLIP_API_URL and PAPERCLIP_API_KEY after approval/key claim. Do not confuse that with agentDefaultsPayload.apiBaseUrl, which points Todero to Hermes.
    ...
    Hermes Gateway payload example:
    {
      ...
      "agentDefaultsPayload": {
        ...
        "toderoApiUrl": "http://localhost:3100"
      }
    }

    Hermes Gateway network examples:
    - Local loopback API server: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:8642" and agentDefaultsPayload.toderoApiUrl = "http://127.0.0.1:3100".
    - Local loopback dashboard root or chat URL: agentDefaultsPayload.apiBaseUrl = "http://127.0.0.1:9119" or "http://127.0.0.1:9119/chat"; Todero maps either one to "http://127.0.0.1:9119/api".
    - LAN/private network: use reachable private addresses, for example agentDefaultsPayload.apiBaseUrl = "http://192.168.1.25:8642" and agentDefaultsPayload.toderoApiUrl = "http://192.168.1.10:3100".
    - Private overlay: use overlay DNS names, for example agentDefaultsPayload.apiBaseUrl = "http://hermes-host.tailnet-name.ts.net:8642" and agentDefaultsPayload.toderoApiUrl = "http://todero-host.tailnet-name.ts.net:3100".
    - Docker: if Todero runs in Docker and Hermes runs on the host, use agentDefaultsPayload.apiBaseUrl = "http://host.docker.internal:8642"; if both run in Compose, use the Hermes service name.
    - Reverse proxy/TLS: use HTTPS origins, for example agentDefaultsPayload.apiBaseUrl = "https://hermes-gateway.example" and agentDefaultsPayload.toderoApiUrl = "https://todero.example".
    ...
    The board approves the join request in Todero before key claim is allowed.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime and set PAPERCLIP_API_KEY and PAPERCLIP_API_URL for future Todero API calls. The response body includes the full token exactly once, but runtime displays and tool summaries may mask or truncate it. Write the raw response token directly to private storage before logging anything, then verify it with an authenticated Todero API call. Do not persist displayed previews containing literal ... or [redacted].
    ...
    ${
      ...
      "Ensure Todero is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Todero base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.toderoApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - npx todero allowed-hostname <host>
      - then restart Todero and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@todero.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "todero", path: "/api/skills/todero" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "todero-create-agent",
          path: "/api/skills/todero-create-agent"
        },
        {
          name: "todero-converting-plans-to-tasks",
          path: "/api/skills/todero-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "todero",
          path: `/api/invites/${token}/skills/todero`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "todero") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              toderoApiUrl: req.body.toderoApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.toderoApiUrl &&
          !persistedDefaults.toderoApiUrl
        ) {
          missingPersistedFields.push("toderoApiUrl");
        }
