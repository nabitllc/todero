/**
 * A plan that was claimed but never written, and the way back from it.
 *
 * Split out of conversation-outcome.ts, which had grown past the size a file
 * here is allowed to be. Nothing about the behaviour changed: these are the
 * same functions, in a file of their own, because they are one job — reading
 * a reply that asks for approval, counting the tries, and deciding between
 * one more go and handing the task to the person.
 */
