...
import type { QuotaWindow } from "@todero/adapter-utils";
...
describe("readClaudeToken", () => {
  ...
  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `todero-test-claude-${Date.now()}`);
...
describe("readCodexAuthInfo", () => {
  ...
  it("returns null for malformed JSON", async () => {
    const tmpDir = path.join(os.tmpdir(), `todero-test-codex-${Date.now()}`);
    await import("node:fs/promises").then((fs) =>
      fs.mkdir(tmpDir, { recursive: true }).then(() =>
