...
rl.on("line", (line) => {
  ...
  if (method === "environmentExecute") {
    const invocationId = message.toderoInvocation && message.toderoInvocation.id;
    ...
    if (oversizedChars > 0) {
      send({
        jsonrpc: "2.0",
        method: "execute.log",
        toderoInvocationId: invocationId,
    ...
    for (const entry of logs) {
      ...
      if (tag === "echo") {
        note.toderoInvocationId = invocationId;
      } else if (tag === "unknown") {
        note.toderoInvocationId = "unknown-invocation";
      } else if (tag === "forge-previous") {
        note.toderoInvocationId = forgeableId;
      }
