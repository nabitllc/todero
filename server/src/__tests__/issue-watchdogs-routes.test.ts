...
describeEmbeddedPostgres("issue watchdog routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("todero-issue-watchdogs-routes-");
  ...
  async function seedCompany(name = "Todero") {
  ...
  it("routes watchdog-discovered product bugs outside the watched source tree with evidence links", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...
        description: "Watchdog found a Todero follow-up routing bug.",
