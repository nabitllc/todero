...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "todero", LOGNAME: "todero", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "todero" }),
      },
    );

    expect(tokens).toEqual(["todero", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "todero" }, {
        userInfo: () => ({ username: "todero" }),
      });

      expect(tokens).toEqual(["todero", "custom-token"]);
    } finally {
