...
describe("project workspace skill discovery", () => {
  ...
  it("finds bounded skill roots under supported workspace paths", async () => {
    const workspace = await makeTempDir("todero-skill-workspace-");
    ...
    await writeSkillDir(path.join(workspace, "skills", ".system", "todero"), "Todero");
    ...
    expect(discovered).toEqual([
      ...
      {
        skillDir: path.resolve(workspace, "skills", ".system", "todero"),
        directoryRoot: "skills/.system",
        relativePath: "skills/.system/todero",
        inventoryMode: "full",
      },
      {
  ...
  it("limits root SKILL.md imports to skill-related support folders", async () => {
    const workspace = await makeTempDir("todero-root-skill-");
  ...
  it("rejects symlinks reachable from a project-scanned skill", async () => {
    const workspace = await makeTempDir("todero-linked-skill-file-");
    const skillDir = path.join(workspace, ".codex", "skills", "linked-file");
    const outsideFile = path.join(await makeTempDir("todero-linked-skill-outside-"), "outside.md");
  ...
  it("parses inline object array items in skill frontmatter metadata", async () => {
    const workspace = await makeTempDir("todero-inline-skill-yaml-");
  ...
  it("parses folded and literal block scalar descriptions in skill frontmatter", async () => {
    const foldedWorkspace = await makeTempDir("todero-folded-skill-yaml-");
    ...
    const literalWorkspace = await makeTempDir("todero-literal-skill-yaml-");
    ...
    const stripWorkspace = await makeTempDir("todero-strip-skill-yaml-");
  ...
  it("parses YAML block-scalar chomping variants from SKILL.md frontmatter", async () => {
    const workspace = await makeTempDir("todero-block-scalar-chomp-skill-");
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(
      path.join(workspace, "SKILL.md"),
      [
...
describe("missing local skill reconciliation", () => {
  it("flags local-path skills whose directory was removed", async () => {
    const workspace = await makeTempDir("todero-missing-skill-dir-");
  ...
  it("flags local-path skills whose SKILL.md file was removed", async () => {
    const workspace = await makeTempDir("todero-missing-skill-file-");
    const skillDir = path.join(workspace, "skills", "ghost");
    await writeSkillDir(skillDir, "Ghost");
