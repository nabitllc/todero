...
describe("llm routes", () => {
  ...
  it("documents timer heartbeats as opt-in for new hires", async () => {
    ...
    expect(res.text).toContain("Use the todero-create-agent skill for end-to-end hiring");
  ...
  it("serves static Hermes Gateway configuration docs before the plugin is installed", async () => {
    ...
    expect(res.text).toContain("agentDefaultsPayload.toderoApiUrl");
    expect(res.text).toContain("hermes_local");
    expect(res.text).toContain("Hermes-originated Todero API usage");
