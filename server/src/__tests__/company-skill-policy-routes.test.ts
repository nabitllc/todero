...
import { activityLog, agents, companies, companySkillPolicies, createDb } from "@todero/db";
...
describeEmbeddedPostgres("company skill policy routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("todero-skill-policy-routes-");
